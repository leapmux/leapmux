...
// EnforceWorkerSharingChange terminates workspaces of users who lost access
// to a worker due to a sharing change. If affectedUserIDs contains "*",
// all non-owner workspaces on this worker are terminated.
func (n *Notifier) EnforceWorkerSharingChange(ctx context.Context, workerID string, affectedUserIDs []string) error {
	var allWorkspaceIDs []string

	// Check for wildcard: terminate all non-owner workspaces.
	wildcard := false
	...
	if wildcard {
		// Get the worker to find its owner.
		worker, err := n.queries.GetWorkerByIDInternal(ctx, workerID)
		if err != nil {
			return fmt.Errorf("get worker: %w", err)
		}

		wsIDs, err := n.queries.ListWorkspaceIDsByWorker(ctx, workerID)
		if err != nil {
		...
		for _, wsID := range wsIDs {
			...
			ws, err := n.queries.GetWorkspaceByIDInternal(ctx, wsID)
			if err != nil {
				continue
			}
			if ws.CreatedBy == worker.RegisteredBy {
				continue // Don't terminate owner's workspaces.
			}
		...
		for _, userID := range affectedUserIDs {
			wsIDs, err := n.queries.ListWorkspaceIDsByWorkerAndCreator(ctx, db.ListWorkspaceIDsByWorkerAndCreatorParams{
				WorkerID:  workerID,
				CreatedBy: userID,
			})
			if err != nil {
				slog.Error("failed to list workspaces for affected user", "worker_id", workerID, "user_id", userID, "error", err)
				continue
			}
	...
	payload, _ := json.Marshal(map[string]any{"workspace_ids": allWorkspaceIDs})
	msg := &leapmuxv1.ConnectResponse{
		Payload: &leapmuxv1.ConnectResponse_TerminateWorkspaces{
			TerminateWorkspaces: &leapmuxv1.TerminateWorkspacesNotification{
				...
				Reason:       "worker sharing changed",
			},
		},
	}

	return n.SendOrQueue(ctx, workerID, leapmuxv1.NotificationType_NOTIFICATION_TYPE_TERMINATE_WORKSPACES, string(payload), msg)
}
