...
For the content that lives inside tabs, see [Coding Agents](/docs/09-coding-agents/) and [Terminals](/docs/11-terminals/). For the git-aware file tree and inline diffs, see [File Browser](/docs/12-file-browser/). For the tiling canvas the tabs live in, see [Tabs & Layout](/docs/08-tabs-and-layout/).
...
- An **unborn HEAD** — a freshly-initialized repo with no commits yet, or a tab that has not been git-stamped — has no resolvable branch. It is labelled **`(no branch)`** (internally its branch is `null`).
- A **detached HEAD that has at least one commit** is labelled with its **7-character short commit SHA** (e.g. `a1b2c3d`). This is a real, non-null label, not the `(no branch)` bucket.
...
The menu is hidden when the row is read-only, when the group has no branch at all (the `(no branch)` group — an unborn HEAD or an un-stamped tab), or when the tab has not yet been stamped with a git toplevel path. It is **not** hidden for a detached-HEAD-with-commits row, since that row carries a short-SHA label rather than a null branch — its menu stays available even though **Delete branch** will fail there (see the note above).
...
> **Note:** Branch names follow git's `check-ref-format` rules. LeapMux rejects empty names, names over 256 characters, control characters, the characters space `~ ^ : ? * [ ] \`, names that start with `/ . - @`, names that end with `/`, `.`, or `.lock`, and names containing `..`, `//`, or `/.`.
...
The sidebar branch-group header also carries a diff-stats badge (`+N -M *U`) so you can see at a glance which branches have changes. For the full meaning of those badges and the per-file git status colors, see [File Browser](/docs/12-file-browser/).
...
For more on workers and how they are selected, see [Managing Workers](/docs/19-managing-workers/). For the run modes that host workers, see [Running LeapMux](/docs/17-running-leapmux/).
