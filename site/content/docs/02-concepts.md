...
- **Worker** — a Go process that actually runs your coding agents, PTY/terminal sessions, file browsing, and git operations. Each Worker keeps its own local SQLite database and auto-reconnects to the Hub if the connection drops. **Everything you care about — agent transcripts, terminal output, file contents — exists only on the Worker.**
...
> **Note:** The native desktop app is a packaged Frontend plus an embedded LeapMux binary. It can run a local Hub+Worker (solo) or connect its WebView to a remote Hub. See [Installation](/docs/03-installation/) and [Running LeapMux](/docs/17-running-leapmux/).
...
`leapmux solo` is the zero-config, single-user setup. The Hub and Worker run **in the same process**, bound to loopback only at `127.0.0.1:4327`. There is **no login** — the UI opens straight into your workspace because every request is auto-authenticated as the admin.
...
The default TCP port everywhere is **4327**. For run modes, ports, data directories, Docker, and reverse-proxy setup, see [Running LeapMux](/docs/17-running-leapmux/) and [Configuration](/docs/18-configuration/). For the full command surface, see the [CLI Reference](/docs/24-cli-reference/).
...
Org membership and workspace access are *separate* layers: being in an org does not by itself grant you access to any workspace in it. See [Organizations & Members](/docs/06-organizations-and-members/).
...
**Sharing** (distributed mode only — never solo) grants other org members read access. Crucially, a share only authorizes the Hub to *route* that user's traffic to the workspace's Workers; to actually read agent or terminal content, the recipient must open their own encrypted channel to each Worker. See [Workspaces](/docs/07-workspaces/) and [Collaboration & Presence](/docs/13-collaboration-and-presence/).
...
You can split, grid, resize, pop tabs out and back in, and drag tabs between tiles or even between workspaces. See [Tabs & Layout](/docs/08-tabs-and-layout/) for the full set of controls, the responsive behavior, and the close dialogs.
...
For registering, approving, pinning, and selecting Workers, see [Managing Workers](/docs/19-managing-workers/).
...
- The encryption is a hybrid post-quantum handshake (X25519 + ML-KEM-1024 for key exchange, SLH-DSA-SHAKE-256f for the Worker's static-key authentication, ChaCha20-Poly1305 for transport encryption, and BLAKE2b for hashing/key derivation). A **classic** (X25519-only) mode is also available; the default is **post-quantum**.
...
The principle in one line: the Hub sees *who talks to whom* plus the control-plane metadata it needs to do its job — account records, org/workspace/membership data, workspace titles and layout geometry, Worker public keys and online status, and per-message metadata like channel IDs, ciphertext sizes, and timing (so **traffic analysis is in scope**). It never sees the *content*: agent transcripts, tool calls, terminal I/O, file contents, or even Worker hostnames and filesystem paths all travel inside the encrypted channel.

This is what "the Hub is an **authenticated relay, not a trusted peer**" means in practice. It is load-bearing in distributed mode, where the Hub may be operated by someone other than you. For the authoritative breakdown of exactly what the Hub can and cannot see, see [Security & Threat Model](/docs/23-security-and-threat-model/).
...
For the full protocol, threat model, and trust boundaries, see [Security & Threat Model](/docs/23-security-and-threat-model/). For operator-side encryption at rest (a separate keystore protecting Hub-stored secrets like OAuth tokens), see [Encryption & Data](/docs/22-encryption-and-data/).
...
In distributed mode, several people — or several browser tabs/devices of one person — can open the **same workspace** at once. What stays synchronized is the **workspace layout**: the tiling tree, the tabs (their order, which tile they're in, which Worker hosts them), splits, grids, floating windows, and workspace lifecycle (created / renamed / deleted). When one participant opens, moves, closes, resizes, or renames something, the others see it within roughly a frame to a network round-trip.

This sync is a **CRDT** (a conflict-free replicated data type) carried over a single per-org WebSocket. Concurrent edits to the same thing resolve automatically — last write wins by a Hub-assigned clock — with no merge dialog and no error; the losing edit is silently superseded, and every client converges to the same result. You only ever see workspaces you have read access to.

Be clear about what collaboration here is **not**: there are **no avatars, no "who is viewing" badges, no remote cursors, and no shared text caret/selection**. The single presence signal that exists is a per-workspace "active client" used for one purpose — to make sure only the most recently active client plays the agent turn-end notification sound, so every open tab doesn't ding at once.

See [Collaboration & Presence](/docs/13-collaboration-and-presence/) for exactly what does and doesn't sync, and [Settings & Preferences](/docs/14-settings-and-preferences/) for the turn-end sound preference.
...
From here, jump to whichever piece you need: get LeapMux running in [Installation](/docs/03-installation/) and [Quick Start](/docs/04-quick-start/), or dig into any concept via its dedicated chapter linked above. New terms are collected in the [Glossary](/docs/27-glossary/).
