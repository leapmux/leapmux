...
For task-oriented walkthroughs rather than reference tables, see [Running LeapMux](/docs/17-running-leapmux/) (run modes, ports, data dirs, Docker), [Configuration](/docs/18-configuration/) (full config-key reference and storage backends), [Admin CLI](/docs/20-admin-cli/), and [Remote Control CLI](/docs/16-remote-control-cli/).
...
|---------|--------------|-----------|
| `solo` | Hub + Worker in one process, loopback only, no login | [Running LeapMux](/docs/17-running-leapmux/) |
| `hub` | Hub service only; workers connect separately | [Running LeapMux](/docs/17-running-leapmux/) |
| `worker` | Worker only; connects out to a Hub | [Managing Workers](/docs/19-managing-workers/) |
| `dev` | Hub + Worker in one process with real auth (development) | [Running LeapMux](/docs/17-running-leapmux/) |
| `admin` | Manage hub data directly against the database | [Admin CLI](/docs/20-admin-cli/) |
| `remote` | Drive a running Hub over RPC (scripts / spawned agents) | [Remote Control CLI](/docs/16-remote-control-cli/) |
...
Run a Hub and a Worker in one process on loopback, with no login (every request is auto-authenticated as the admin). See [Running LeapMux](/docs/17-running-leapmux/#solo-mode) for details.
...
Run only the Hub: authentication, workspace management, Worker registration, and the encrypted relay. Binds all interfaces by default and requires a real login. See [Running LeapMux](/docs/17-running-leapmux/#hub-mode) and [Configuration](/docs/18-configuration/) for the complete key reference.

```bash
leapmux hub [flags]
```

This table lists the most common flags. The full set — including all PostgreSQL/MySQL/CockroachDB/YugabyteDB/TiDB pool-tuning flags — is in [Configuration](/docs/18-configuration/).
...
> **Note:** Two hub config keys have **no** CLI flag and are set only via YAML or env var: `secure_cookies` (`LEAPMUX_HUB_SECURE_COOKIES`) and `encryption_key_path` (`LEAPMUX_HUB_ENCRYPTION_KEY_PATH`, default `<data-dir>/encryption.key`). See [Configuration](/docs/18-configuration/) and [Encryption & Data](/docs/22-encryption-and-data/).

## worker

Run a Worker that connects out to a Hub. Workers do not serve an inbound HTTP port; they register with the Hub using a key minted in the Hub UI. See [Managing Workers](/docs/19-managing-workers/).
...
A local-only utility for inspecting the Worker's TOFU pin store. It runs entirely against local files — no Worker process starts. See [Security & Threat Model](/docs/23-security-and-threat-model/) for what these pins protect.
...
> **Note:** The binary's own help text for this flag mentions `LEAPMUX_DATA_DIR`, but that variable is **not** read anywhere — it has no effect. The variable that actually overrides the data dir is `LEAPMUX_WORKER_DATA_DIR`.

## dev

Run a Hub and a Worker in one process with **real** password authentication — the same program as `solo` but with login enabled, binding all interfaces, and the first admin bootstrapped through the `/setup` flow. See [Running LeapMux](/docs/17-running-leapmux/#dev-mode).
...
`leapmux admin` manages a Hub's persistent data **directly against its database and on-disk encryption key** — no running Hub or network call required. It is a tree of command groups. For full per-command flags and behavior, see [Admin CLI](/docs/20-admin-cli/).
...
The `LEAPMUX_REMOTE_*` variables prefixed `_SOCK`/`_TOKEN`/`_*_ID`/etc. are injected automatically by the Worker into agents and terminals it spawns; you do not set them by hand. See [Remote Control CLI](/docs/16-remote-control-cli/) for how they drive entity-ID resolution.
...
In `solo` and `dev`, the data directory is split into `<data-dir>/hub` and `<data-dir>/worker` subdirectories. See [Running LeapMux](/docs/17-running-leapmux/) and [Configuration](/docs/18-configuration/) for the full layout and resolution rules.
