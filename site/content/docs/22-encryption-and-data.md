...
- **End-to-end encryption (E2EE)** — all Frontend-to-Worker traffic is encrypted so the Hub can route it but never read it. That protocol is covered in [Security & Threat Model](/docs/23-security-and-threat-model/); this chapter only touches the Worker key material you must back up.

For where these settings live and how to set them, see [Configuration](/docs/18-configuration/). For the full `leapmux admin` command surface, see [Admin CLI](/docs/20-admin-cli/).
...
> **Note:** Agent chat transcripts, tool calls, terminal output, file contents, and diffs **never** reach the Hub in readable form and are **not** stored in the Hub database at all. They live only in the Worker's local database and are end-to-end encrypted in transit. See [Security & Threat Model](/docs/23-security-and-threat-model/).

### Encryption at rest details

The keystore encrypts secrets with **XChaCha20-Poly1305** (a 256-bit key, 24-byte random nonce per ciphertext). Each ciphertext records which key version produced it, so the Hub can always pick the right key to decrypt.
...
**How encryption at rest fits together:**

```text
   ┌──────────────────┐
   │  encryption.key  │   versioned key ring;
   │  (file, 0600)    │   highest version = active
   └────────┬─────────┘
            │ loads keys
            ▼
   ┌──────────────────┐   XChaCha20-Poly1305
   │     Keystore     │   (256-bit key,
   │  (key ring)      │    24-byte nonce)
   └────────┬─────────┘
            │ encrypt / decrypt
            ▼
┌──────────────────────────────────────────────┐
│            Hub database (hub.db)             │
│                                              │
│  Ciphertext (encrypted with the key):        │
│    • oauth_providers.client_secret           │
│    • oauth_tokens.access_token               │
│    • oauth_tokens.refresh_token              │
│                                              │
│  Stored without the key (plaintext/hashed):  │
│    • accounts, orgs, workspaces, Workers     │
│    • API-token secrets (HMAC-SHA256 hashes)  │
│    • Worker public keys                      │
└──────────────────────────────────────────────┘
```

The key lives in a separate file from the database, so a copy of the database alone reveals only ciphertext — which is why the two must be backed up together.

API-token and delegation-token secrets are **not** encrypted — they are HMAC-SHA256 hashed with a dedicated, stable pepper that is independent of the encryption key ring, so the database never contains a recoverable token. See [Accounts & Authentication](/docs/05-accounts-and-authentication/) and [Admin CLI](/docs/20-admin-cli/) for token management.
...
Token hashes are one-way, so regenerating the pepper cannot migrate existing tokens — it invalidates them all at once, and they must be reissued (`leapmux admin api-token issue`) or re-authenticated. The command requires `--yes` and takes effect on the next Hub restart. See [Admin CLI](/docs/20-admin-cli/) for `api-token` and `delegation-token` management.

## Databases

### Hub database backends

The Hub stores all relational data in one of six interchangeable SQL backends, selected by `storage.type`:

| `storage.type` | Driver / notes | DSN required? |
| --- | --- | --- |
| `sqlite` (default; also used when empty) | Embedded file DB (`modernc.org/sqlite`, pure Go) | No |
| `postgres` | PostgreSQL (`pgx/v5`) | Yes — `storage.postgres.dsn` |
| `mysql` | MySQL (`go-sql-driver/mysql`) | Yes — `storage.mysql.dsn` |
| `cockroachdb` | Reuses the PostgreSQL driver | Yes — `storage.cockroachdb.dsn` |
| `yugabytedb` | Reuses the PostgreSQL driver | Yes — `storage.yugabytedb.dsn` |
| `tidb` | Reuses the MySQL driver | Yes — `storage.tidb.dsn` |
...
For the full set of connection-pool, cache, and DSN options for each backend, see [Configuration](/docs/18-configuration/). The full storage-key reference (max-conns, lifetimes, etc.) lives there and is not repeated here.

> **Note:** Use a config file (or CLI flags) to set storage options. Because of how environment variables are mapped, the nested `storage.*` keys are most reliably set via YAML or flags rather than env vars. See [Configuration](/docs/18-configuration/).

### Worker database

Each Worker keeps its **own** separate SQLite database at `{data_dir}/worker.db` (default Worker data dir: `~/.config/leapmux/worker`). It holds transient agent and session state — agents, messages, terminals, worktrees, todos, control requests, and so on. It uses the same WAL/foreign-key settings as the Hub's SQLite and is chmod'd to `0600`. Worker SQLite tuning flags (`--db-max-conns`, `--db-cache-size`, `--db-mmap-size`) are documented in [Configuration](/docs/18-configuration/) and [Running LeapMux](/docs/17-running-leapmux/).
...
| --- | --- | --- |
| `leapmux admin db path` | Print the database path | Prints the SQLite path (`hub.db` or your configured SQLite path). It does **not** consult `storage.type`, so it always prints the SQLite path even when a SQL backend is configured. |
...
Like `encryption-key reencrypt`, both `admin db version` and `admin db migrate` open the store, so both accept `--data-dir` and `--config`; pass `--config` so the command targets the same backend the Hub uses. `admin db path` does not open the store, so it accepts `--data-dir` only. See [Admin CLI](/docs/20-admin-cli/).
...
3. **`state.json`** (default `~/.config/leapmux/worker/state.json`). This holds the Worker's identity: its `worker_id`, Hub auth token, and its private E2EE keys (X25519, ML-KEM-1024, SLH-DSA). Losing it forces re-registration with a new registration key and a new key identity — and because the Worker's public keys are registered in the Hub, a new identity will trip the Frontend's "Worker public key changed" pin-mismatch dialog. See [Managing Workers](/docs/19-managing-workers/) and [Security & Threat Model](/docs/23-security-and-threat-model/).
...
- **Lost a Worker's `state.json`:** re-register the Worker (a fresh registration key from the Hub UI). The first Frontend to reconnect after the identity changes sees the **"Worker public key changed"** dialog and must explicitly Accept the new key. See [Managing Workers](/docs/19-managing-workers/).
...
Separate from the at-rest keystore, each Worker negotiates its **transport** encryption mode for Frontend-to-Worker channels. This is set with the Worker's `--encryption-mode` flag (or `encryption_mode` in YAML / `LEAPMUX_WORKER_ENCRYPTION_MODE`):
...
Aliases `pq`, `post_quantum`, and an empty value all resolve to `post-quantum`; any unrecognized value also falls back to `post-quantum` (fail-safe). The trade-off is purely classic vs. post-quantum: post-quantum costs slightly larger handshakes and a little more CPU per channel open in exchange for resistance to future quantum attacks. Unless you have a specific reason to choose `classic`, leave it at the default.

The handshake primitives, Worker identity pinning (TOFU), and the key-change dialog are covered in [Security & Threat Model](/docs/23-security-and-threat-model/). Configuration precedence and the full flag list are in [Configuration](/docs/18-configuration/).

## See also

- [Configuration](/docs/18-configuration/) — config precedence, the full storage-key reference, data directories, and listen addresses.
- [Security & Threat Model](/docs/23-security-and-threat-model/) — the E2EE protocol, the Hub-as-relay trust boundary, Worker TOFU pinning, and the solo-mode caveat.
- [Admin CLI](/docs/20-admin-cli/) — the complete `leapmux admin` reference, including `encryption-key`, `db`, and token commands.
- [Managing Workers](/docs/19-managing-workers/) — registering and approving Workers, registration keys, and key-pin handling.
