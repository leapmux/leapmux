...
If you are looking for *how to launch* each mode (solo, hub, worker, dev) and what each is for, see [Running LeapMux](/docs/17-running-leapmux/). For key management, encryption at rest, and database operations, see [Encryption & Data](/docs/22-encryption-and-data/).

> **Note:** Everything here applies to the long-running daemons. `solo` and `dev` modes reuse the Hub's configuration loader with a restricted flag set; the differences are called out where relevant. The desktop app and the `remote`/`admin` CLIs are configured separately — see [Running LeapMux](/docs/17-running-leapmux/), [Remote Control CLI](/docs/16-remote-control-cli/), and [Admin CLI](/docs/20-admin-cli/).
...
> **Warning:** Solo mode auto-authenticates every request as the admin. If you bind it to a non-loopback address, anyone who can reach the port has full admin access without credentials, and the Hub logs a warning to that effect. Restrict access externally (firewall, Tailscale/WireGuard, SSH tunnel) or run `leapmux hub` for real authentication.
...
> **Note:** `public_url` must be an absolute `http`/`https` URL with a host and **nothing else** — no userinfo, no path (sub-path proxying is rejected), no query, no fragment. One trailing slash is trimmed. It is **not supported in solo mode**, where setting it fails with `public_url is not supported in solo mode`. See [Running LeapMux](/docs/17-running-leapmux/) for reverse-proxy setup.
...
See [Accounts & Authentication](/docs/05-accounts-and-authentication/) for the sign-up/verification flows, and [Authentication Providers](/docs/21-authentication-providers/) for OAuth/OIDC.
...
See [Encryption & Data](/docs/22-encryption-and-data/) for the encryption key ring, rotation, and what is encrypted at rest.
...
Env prefix: `LEAPMUX_WORKER_`. A Worker connects to a Hub over a URL; it does not serve HTTP itself. See [Managing Workers](/docs/19-managing-workers/) for registration and approval. Each key's CLI flag is `--` followed by the key with underscores replaced by hyphens (for example, `db_max_conns` → `--db-max-conns`).
...
> **Note:** `registration_key` is required on first run, is never persisted to disk, and is ignored once the Worker is registered. For the registration flow and the exact error messages, see [Managing Workers](/docs/19-managing-workers/).
...
After registration, a Worker persists its identity to `<data_dir>/state.json` (mode `0600`): its worker ID, Hub auth token, who registered it, and its base64-encoded composite E2EE keypair (X25519 + ML-KEM-1024 + SLH-DSA). This keypair is auto-generated on first run if absent.
...
Any unrecognized value falls back to `post-quantum` (fail-safe). For what each mode protects and the underlying primitives, see [Security & Threat Model](/docs/23-security-and-threat-model/).
...
> **Note:** Configure storage via the YAML file or the dedicated CLI flags, not env vars (see the warning under [Environment variable mapping](#environment-variable-mapping)). For backups, key/DB interplay, and the `leapmux admin db` / `leapmux admin encryption-key` commands, see [Encryption & Data](/docs/22-encryption-and-data/).
...
Solo mode needs no config file at all — it defaults to loopback TCP and a local SQLite database. To pin the data directory and turn on debug logging:
...
leapmux worker --registration-key "rk_..."
...
Every mode supports the standard help and version tokens:

- `-h`, `-help`, `--help`, or `help` prints categorized usage to stdout.
- `--version` (or `-version`, or the `version` subcommand) prints the build version and exits.

Unexpected positional arguments are rejected with `unexpected argument: "<arg>" (use --help for usage)`.

## Related chapters

- [Running LeapMux](/docs/17-running-leapmux/) — run modes, ports, data dirs, Docker, reverse proxy.
- [Encryption & Data](/docs/22-encryption-and-data/) — encryption key ring, rotation, DB migrations, backup/restore.
- [Managing Workers](/docs/19-managing-workers/) — registration keys, approval, worker selection.
- [Authentication Providers](/docs/21-authentication-providers/) — configuring OAuth/OIDC as an operator.
- [Security & Threat Model](/docs/23-security-and-threat-model/) — E2EE protocol, encryption modes, trust boundaries.
- [CLI Reference](/docs/24-cli-reference/) — consolidated command and flag cheat-sheet.
