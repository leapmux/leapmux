...
This prints an `access_token` of the form `lmx_a<id>_<secret>` exactly once. Supply it to the CLI by setting it as the bearer for the hub transport. Issuing, listing, and revoking these tokens is covered in [Admin CLI](/docs/20-admin-cli/).
...
> **Note:** There is no "remote-enabled" flag or checkbox. Terminals and agents receive `LEAPMUX_REMOTE_*` automatically whenever the worker has remote control enabled. Inherited `LEAPMUX_REMOTE_*` values are stripped before re-injection, so a worker spawned from inside another agent gets a fresh context rather than its parent's. See [Terminals](/docs/11-terminals/) for the terminal side of this.
...
| `workspace rename` | `--workspace-id`, `--title` (required) | The renamed workspace |
...
`--worktree` is **required** when the close would remove the last tab for a worktree, or close the last tab on a non-worktree branch that has uncommitted or unpushed changes — omitting it then returns an `invalid_request` with the details. `--worktree push` runs `git push` and fails with `invalid_request` if the branch isn't pushable. The command emits `{tab_id, tab_type, tombstoned, worktree?, worker_close_error?}`. File tabs skip worktree inspection entirely. See [Worktrees and Branches](/docs/10-worktrees-and-branches/) for the disposition rules.
...
The `tile` group mutates the tile tree one operation at a time; the `layout` group reads or replaces the whole tree at once. See [Tabs and Layout](/docs/08-tabs-and-layout/) for the conceptual model of splits and grids.
...
```bash
leapmux remote layout get --workspace-id "$WS" > layout.json
# edit layout.json ...
leapmux remote layout set --workspace-id "$WS" --file layout.json
```

- `layout get` emits `{workspace_id, root_node_id, tree, tabs_by_tile}` — the same shape `layout set` accepts.
- `layout set` requires exactly one of `--file PATH` or `--stdin`. It rewrites the entire tree in one batch and repoints every live tab onto the new tree's first leaf; the root node id never changes.
...
If a tab races in during the rewrite, `layout set` retries up to twice; persistent contention yields `{"error":{"code":"concurrent_modification",...}}`. The success envelope includes `attempts` (`1` normally, `2` after a retry).
...
Pins are stored at `<ConfigDir>/<hub-host>/pins.json` with mode `0644` (they are not secrets). For the worker registration and approval lifecycle, see [Managing Workers](/docs/19-managing-workers/).
...
- `agent set` applies model/effort/permission-mode and repeatable `--extra-setting key=value` provider options. Changing model or effort restarts the agent process; for Claude Code, permission-mode changes apply live. See [Coding Agents](/docs/09-coding-agents/) for the per-provider settings.
...
```bash
leapmux remote layout get --workspace-id "$WS" > before.json
...
For the full trust model, what the Hub can and cannot see, and the encryption primitives, see [Security & Threat Model](/docs/23-security-and-threat-model/).

## See also

- [Coding Agents](/docs/09-coding-agents/) — providers, models, effort, control prompts, and resume that `agent` commands drive.
- [Terminals](/docs/11-terminals/) — PTY sessions, shells, and the automatic `LEAPMUX_REMOTE_*` injection.
- [Managing Workers](/docs/19-managing-workers/) — worker registration, approval, and TOFU pinning.
- [Admin CLI](/docs/20-admin-cli/) — `leapmux admin api-token` for headless service-account tokens.
- [Tabs and Layout](/docs/08-tabs-and-layout/) — the tile/split/grid model that `tile` and `layout` manipulate.
- [Worktrees and Branches](/docs/10-worktrees-and-branches/) — the worktree dispositions used by `tab close --worktree`.
