...
This is the `app.openPreferences` command (default binding `$mod+Comma`). See [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/) for the full shortcut system and how to customize bindings.
...
What you see in the Preferences dialog depends on whether your Hub runs in distributed mode or [solo mode](/docs/17-running-leapmux/).
...
Controls the color scheme of terminal tabs (see [Terminals](/docs/11-terminals/)).
...
Controls how file diffs render in chat tool results and the file viewer (see [File Browser](/docs/12-file-browser/)).
...
The sound is intentionally restrained: it is active-client gated and suppressed for trivial or rapid turns, so you do not hear duplicate chimes across open tabs or devices. See [Collaboration & Presence](/docs/13-collaboration-and-presence/) for why and how that gating works.
...
Each font-list editor works the same way. Your custom fonts are tried first, in order, and the bundled defaults are appended as a fallback.
...
The Profile dialog manages your account identity. It is **not available in solo mode**. For the broader account lifecycle — sign-up, login, OAuth, email verification, sessions — see [Accounts & Authentication](/docs/05-accounts-and-authentication/). Each section below shows inline success and error messages and saves independently.
...
- If the Hub requires email verification, you will see **"Verification email sent. Check your inbox."** and must confirm the new address (see [Accounts & Authentication](/docs/05-accounts-and-authentication/)).
...
If you have linked one or more OAuth/OIDC providers (GitHub, Google, Apple, or a generic OIDC provider configured by your operator — see [Authentication Providers](/docs/21-authentication-providers/)), each appears in **Linked Accounts** with an **Unlink** button. This section is hidden when you have no linked providers.
...
The composer toggle reads **Enter sends** when plain Enter sends a message, and otherwise shows the modifier-Enter label for your platform: **⌘⏎ sends** on macOS, **Ctrl+⏎ sends** on Windows/Linux. The Enter-key send mode interacts with the chat editor's send keys; see [Coding Agents](/docs/09-coding-agents/) and [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/). The terminal renderer preference has no UI control — it defaults to automatic selection (and avoids WebGL on Linux desktop).

## Custom keyboard shortcuts

Custom keybindings are stored **account-level** (as JSON on the Hub), not in the Preferences dialog and not per-browser. There is no graphical keybinding editor. See [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/) for the default bindings and how overrides work.

## How preferences persist

### Per-browser (This Browser)

Browser overrides live entirely on the device you are using, in the browser's local storage, under a single consolidated key (`leapmux:browser-prefs`). The value carries a 1-year expiry that is refreshed every time you open the app, so a browser you use within any rolling one-year window keeps its overrides indefinitely. Setting an option to **Use account default** removes that override so the account default takes over. Theme changes also propagate instantly to your other open tabs in the same browser.

The fields stored per-browser are: `theme`, `terminalTheme`, `diffView`, `turnEndSound`, `turnEndSoundVolume`, `debugLogging`, plus the browser-only preferences above (`expandAgentThoughts`, `showHiddenMessages`, `enterKeyMode`, `terminalRenderer`, `revealAfterDownload`).
...
Account defaults are stored on the Hub and loaded when the app starts. The account-level fields are: `theme`, `terminalTheme`, custom UI/monospace font toggles and lists, `diffView`, `turnEndSound`, `turnEndSoundVolume`, `debugLogging`, and your custom keybindings. They follow you to every device where you sign in.
...
- [Accounts & Authentication](/docs/05-accounts-and-authentication/) — sign-up, login, OAuth, email verification, sessions.
- [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/) — the shortcut system, default bindings, and customization.
- [Running LeapMux](/docs/17-running-leapmux/) — solo vs. distributed mode.
- [File Browser](/docs/12-file-browser/) and [Terminals](/docs/11-terminals/) — features the Diff View and Terminal Theme settings affect.
