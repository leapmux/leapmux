...
For the bigger picture of how workspaces fit alongside Hubs, Workers, tiles, and tabs, see [Concepts & Architecture](/docs/02-concepts/). For everything you do *inside* a workspace — tabs, splits, grids, and floating windows — see [Tabs & Layout](/docs/08-tabs-and-layout/).
...
- Organization membership alone does **not** grant access to a workspace. You see a workspace only if you own it or the owner has explicitly shared it with you. See [Organizations & Members](/docs/06-organizations-and-members/).
- Agent and terminal state for a workspace lives only in the Worker's local database, end-to-end encrypted. The Hub stores the workspace's title, tab positions, and layout geometry, but never the content. See [Security & Threat Model](/docs/23-security-and-threat-model/).
...
2. Alternatively, trigger the keyboard shortcut bound to the *New workspace dialog* action (see [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/)).

The dialog is titled **New workspace** and contains these fields:

| Field | What it does |
| --- | --- |
| **Worker** | Pick which Worker (machine) hosts the workspace's first agent. See [Managing Workers](/docs/19-managing-workers/). |
| Agent provider | Choose the agent backend (for example Claude Code or Codex). A refresh control re-queries the Worker for available providers. See [Coding Agents](/docs/09-coding-agents/). |
...
| Session ID | Optional agent session ID to resume an existing agent session (right column). See [Coding Agents](/docs/09-coding-agents/). |
| Git options | Once a Worker is chosen, choose the git mode for the working directory — for example opening directly or in a worktree. See [Worktrees & Branches](/docs/10-worktrees-and-branches/). |
...
> **Tip:** If anything fails after the workspace row is created — for example the agent cannot start — LeapMux rolls the creation back automatically, so you are never left with an empty orphan workspace.
...
> **Note:** Custom workspace sections exist in the data model and are honored everywhere a section can appear — the **Move to** submenu lists them by name, and workspaces dragged into one stay there across reloads. However, the current UI does not expose controls to **create**, **rename**, or **delete** a custom section: the **+** button on a section header creates a *workspace*, not a section, and there is no "new section" affordance in the sidebar. Until those controls ship, you cannot add a custom section from the app, and the only sections you will see are the three defaults.
...
- A branch with no current branch (detached HEAD or an unborn ref) is labeled **(no branch)** and offers no branch actions.
- Branch rows expose a **Change branch** and **Delete branch** menu when the workspace is writable and the branch has a real name. See [Worktrees & Branches](/docs/10-worktrees-and-branches/).
...
Sharing lets you give other members of your organization access to one of your workspaces. It is available only to the **owner**, and only when LeapMux is running in distributed (multi-user) mode — the **Share…** menu item does not appear in [solo mode](/docs/17-running-leapmux/).
...
That is all the Hub can do, because **the Hub never sees workspace content**. All agent transcripts, tool calls, terminal I/O, file contents, and git status are end-to-end encrypted between the Frontend and the Worker; the Hub is a blind relay that forwards opaque ciphertext.

So to actually *read* a shared workspace, the invited member must:

- Open their **own** end-to-end-encrypted channel directly to each Worker that hosts the workspace's tabs (the hybrid post-quantum Noise channel — see [Security & Threat Model](/docs/23-security-and-threat-model/)), and
...
The first time a member connects to a Worker, that Worker's identity is pinned (trust on first use). For the detailed encryption model, key pinning, and fingerprint verification, see [Security & Threat Model](/docs/23-security-and-threat-model/) and [Managing Workers](/docs/19-managing-workers/).
...
> **Note:** Removing a member from the organization also cuts off their access — but by a different mechanism than the sharing dialog. Org removal does **not** delete the workspace's share grant; instead the removed user loses access because they are no longer an org member (the org no longer resolves for them, and the workspace drops out of their accessible list) and their Worker access grants for that org are revoked. The workspace's share-grant row itself remains until the owner edits sharing. See [Organizations & Members](/docs/06-organizations-and-members/).

## Switching workspaces

Click any workspace row in the sidebar to switch to it. This closes any open sidebars and navigates to `/o/{orgSlug}/workspace/{workspaceId}`.
...
To switch to a workspace in a different organization, use the **Switch Organization** list in the user menu first, then pick the workspace. See [Organizations & Members](/docs/06-organizations-and-members/).

## Live updates across clients

Workspace lifecycle changes — create, rename, delete — are broadcast to all of your connected clients in real time over the organization event stream, so the sidebar stays in sync without a manual refresh. The tiling layout *inside* a workspace also syncs live across collaborators; see [Collaboration & Presence](/docs/13-collaboration-and-presence/).

## Related chapters

- [Tabs & Layout](/docs/08-tabs-and-layout/) — working inside a workspace: tabs, splits, grids, floating windows.
- [Coding Agents](/docs/09-coding-agents/) — opening and using agents in a workspace.
- [Worktrees & Branches](/docs/10-worktrees-and-branches/) — the git side of workspace tabs.
- [Organizations & Members](/docs/06-organizations-and-members/) — orgs, roles, members, and switching orgs.
- [Collaboration & Presence](/docs/13-collaboration-and-presence/) — live layout sync and who-is-here signals.
- [Security & Threat Model](/docs/23-security-and-threat-model/) — the encryption model behind the sharing caveat.
