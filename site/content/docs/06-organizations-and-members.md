...
- **Personal org for everyone.** When your account is created, LeapMux automatically creates a **personal** org named after your username, and makes you its **Owner**. This is your default home org. See [Accounts & Authentication](/docs/05-accounts-and-authentication/) for account creation.
...
For how orgs fit into the broader architecture (Hub, Worker, Frontend, workspaces, tiles), see [Concepts](/docs/02-concepts/).
...
Every membership carries exactly one of three roles. The role gates which org-management actions you can perform.

| Role | Typical use |
| --- | --- |
| **Owner** | Full control, including deleting the org. New orgs start with their creator as the sole Owner. |
| **Admin** | Day-to-day org administration: rename, manage members and roles — but cannot delete the org. |
| **Member** | Participates in the org and can view it, but cannot administer it. |
...
| Be switched to as your current org | Yes | Yes | Yes |
...
- **Only Owners delete.** Deleting an org requires Owner role on that org. (A system-wide platform administrator can also delete an org regardless of role; that operator-level flag is separate from org roles and is covered in [Admin CLI](/docs/20-admin-cli/).)
...
The exact validation messages you may see (with the field name substituted) are:

- "Organization name must not be empty"
- "Organization name must be at most 32 characters"
- "Organization name must contain only letters, numbers, and hyphens"
- "Organization name must not start with a hyphen"
- "Organization name must not end with a hyphen"
- "Organization name must not contain consecutive hyphens"

If the name is already in use, creation fails with `organization name "<name>" is already taken`.
...
- For a **team org**, the name is an editable text input with a **Save** button. Save is disabled while saving, when the field is empty, or when the value is unchanged; it shows "Saving..." while in flight. On success you see "Organization name updated." Saving runs the new name through the slug rules above.
...
- An **Invite** button (shows "Inviting..." while in flight). On success you see a message naming the user and the role you chose, like "Invited alice as member." (or "...as admin." / "...as owner." if you picked a higher role).
...
Loading and empty states show "Loading members..." and "No members found." respectively.

### Create Organization

An **Organization name** text input plus a **Create** button (shows "Creating..." while busy). This is the card you use to create new team orgs (see [Creating an organization](#creating-an-organization)).

### Danger Zone

Shown only for team orgs (never for personal orgs). It explains: "Permanently delete this organization and all its data. This action cannot be undone." and offers a **Delete Organization** button (danger styling, shows "Deleting..." while busy).
...
If no user with that username exists, you get "user not found". If you omit a role, the new member is added as a Member. (A user already in the org cannot be invited again.)
...
Click **Remove** on the member's row and confirm. Removing a member also **revokes that user's worker-access grants in this org** (their access grants in *other* orgs are preserved). For more on worker access, see [Managing Workers](/docs/19-managing-workers/).
...
In other words, org membership defines *who can be granted access*; the workspace owner decides *who actually gets it*, one user at a time. The workspace owner manages this from the **Workspace sharing** dialog. See [Workspaces](/docs/07-workspaces/) for the full sharing workflow and [Collaboration & Presence](/docs/13-collaboration-and-presence/) for what live state syncs between collaborators.
...
> **Note:** A share grant authorizes the Hub to *route* a recipient's traffic to the workspace's workers; it does not by itself decrypt anything. To actually read agent and terminal content, the recipient still opens their own end-to-end-encrypted channel to each worker. The Hub is a blind relay and never sees plaintext. See [Security & Threat Model](/docs/23-security-and-threat-model/).
...
The output columns are `ID`, `NAME`, `PERSONAL` (yes/no), and `CREATED`; it prints "No organizations found." when there are none. There is **no** admin CLI command to create or delete orgs or to manage membership and roles — use the web app for those. See [Admin CLI](/docs/20-admin-cli/) for the complete reference.

## Related chapters

- [Accounts & Authentication](/docs/05-accounts-and-authentication/) — sign up, log in, profile, and personal-org creation.
- [Workspaces](/docs/07-workspaces/) — creating, sharing, and managing workspaces.
- [Collaboration & Presence](/docs/13-collaboration-and-presence/) — what syncs live between collaborators.
- [Managing Workers](/docs/19-managing-workers/) — worker access grants (revoked on member removal).
- [Running LeapMux](/docs/17-running-leapmux/) — solo vs distributed mode.
- [Security & Threat Model](/docs/23-security-and-threat-model/) — the routing-vs-reading distinction and E2EE.
- [Admin CLI](/docs/20-admin-cli/) — the read-only `org list` command and operator-level deletion.
