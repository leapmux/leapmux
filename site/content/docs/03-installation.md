...
> **Tip:** If you just want to run LeapMux, the pre-built Docker images, desktop app installers, and standalone binaries are all you need. You only need the source build if you intend to modify LeapMux.
...
For more on what each run mode does (solo vs. hub vs. worker vs. dev) and how to operate them, see [Running LeapMux](/docs/17-running-leapmux/). For your very first session once LeapMux is installed, see [Quick Start](/docs/04-quick-start/).
...
> **Note:** The macOS desktop app ships for Apple Silicon (`arm64`) only. The standalone server binary for macOS is also distributed as `darwin_arm64` only.

## Desktop app

The desktop app is a self-contained application that bundles the LeapMux frontend and a local service. On first launch it presents a **LeapMux** launcher asking how you want to connect — **Solo** (a Hub and Worker run together on this machine; no network setup, your data stays local) or **Distributed** (connect to a remote Hub by URL). See [Quick Start](/docs/04-quick-start/) for what to do after the app opens.
...
> **Tip:** On macOS the desktop app can install a `leapmux` command-line symlink at `/usr/local/bin/leapmux` so you can use the [Remote control CLI](/docs/16-remote-control-cli/) from a terminal. When you enter a Solo workspace and the CLI isn't on your `PATH`, LeapMux offers to install it. This integration is macOS-only.
...
Pre-built images are published to [GHCR](https://github.com/leapmux/leapmux/pkgs/container/leapmux) (`ghcr.io/leapmux/leapmux`) in two variants:
...
The Alpine variant is the default; append `-ubuntu` to any tag to get the Ubuntu build instead. Release tags (`:latest`, `:<version>`, `:<major>`) come from the release workflow; the `:dev` tag (and `:dev-ubuntu`) is rebuilt on every push to `main`. Both variants are multi-arch (`linux/amd64` + `linux/arm64`).

> **Tip:** Pin a specific version (e.g. `:1.0.0`) for production so upgrades are deliberate. Use `:latest` to track the newest stable release, or `:dev` only if you want bleeding-edge builds.

### How the image works

The image is supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) and is configured through environment variables and the `/data` volume:

- **`LEAPMUX_MODE`** — **required.** Selects the subcommand and must be one of `hub`, `worker`, `dev`, or `solo`. If it's unset or invalid, the container prints `error: LEAPMUX_MODE must be one of: hub, worker, dev, solo` and exits.
...
- State and configuration live under `/data/<mode>/` — e.g. `/data/hub/` for the Hub database, encryption key ring, and config file (`/data/hub/hub.yaml`). Mount a volume at `/data` so this survives container recreation.

On startup the container runs `leapmux <mode> -config /data/<mode>/<mode>.yaml`, creating an empty config file if none exists. Anything beyond `LEAPMUX_MODE` and `LEAPMUX_DATA_DIR` is configured through that YAML file or through `LEAPMUX_HUB_*` / `LEAPMUX_WORKER_*` environment variables — see [Configuration](/docs/18-configuration/).
...
> **Warning:** The Hub does not terminate TLS itself. To serve LeapMux over HTTPS, put a reverse proxy in front of the container and set `public_url` and `secure_cookies` in the Hub config. See [Running LeapMux](/docs/17-running-leapmux/) and [Configuration](/docs/18-configuration/) for reverse-proxy guidance.

### Running a worker container

A `worker` container connects out to a Hub and must be registered with a registration key minted in the Hub UI. The s6 supervisor doesn't pass any Hub URL or key flags, so supply them via the worker config (`/data/worker/worker.yaml`) or via `LEAPMUX_WORKER_HUB` and `LEAPMUX_WORKER_REGISTRATION_KEY` environment variables. An unregistered worker with no key exits with `worker is unregistered: pass --registration-key <key> from the hub UI`. See [Managing Workers](/docs/19-managing-workers/) for the full registration flow.
...
> **Tip:** Put `leapmux` somewhere on your `PATH` (e.g. `/usr/local/bin`) so you can invoke it from any directory and use the [Remote control CLI](/docs/16-remote-control-cli/). To upgrade, replace the binary with the one from a newer release archive; Hub and Worker database migrations run automatically the next time you start.

For run modes, ports, and data directories, see [Running LeapMux](/docs/17-running-leapmux/); for the full subcommand and flag reference, see [CLI Reference](/docs/24-cli-reference/).
...
For the platform-specific dependency install commands (Homebrew, pacman, winget) and the full set of dev workflows, follow the project [`README.md`](https://github.com/leapmux/leapmux). For what each run mode does once built, see [Running LeapMux](/docs/17-running-leapmux/).

## Next steps

- [Quick Start](/docs/04-quick-start/) — launch LeapMux and open your first agent.
- [Running LeapMux](/docs/17-running-leapmux/) — run modes, ports, data directories, Docker, and reverse proxies in depth.
- [Configuration](/docs/18-configuration/) — the full configuration key reference and storage backends.
- [Managing Workers](/docs/19-managing-workers/) — register and approve workers connecting to a Hub.
</content>
</invoke>
