...
If you only want to know what the sign-in experience looks like as an end user (the "Sign in with…" buttons, completing a first OAuth sign-up, linking and unlinking accounts), see [Accounts & Authentication](/docs/05-accounts-and-authentication/). This chapter stays on the operator side: how providers are defined, how callback URLs are derived, and how to troubleshoot setup.

> **Note:** OAuth providers only apply to **Hub** deployments (`leapmux hub`, and `leapmux dev`). In **solo mode** every request is auto-authenticated as the local admin, so there is no login screen and providers do nothing. See [Running LeapMux](/docs/17-running-leapmux/) for the difference between run modes.
...
All provider configuration is done with the `leapmux admin oauth-provider` command group, which operates directly on the Hub's database and encryption key file. There is **no UI** for adding providers — it is an operator task. For the full admin CLI surface, see [Admin CLI](/docs/20-admin-cli/).

> **Note:** The admin CLI reads and writes the Hub's data store on disk. Run it on the machine that hosts the Hub, pointing it at the same data directory (or config file) the Hub uses. By default that is `~/.config/leapmux/hub` — pass `--data-dir` or `--config` to target a different location. See [Admin CLI](/docs/20-admin-cli/) for how the data directory is resolved.
...
Decide which behavior you want and set the flag accordingly. See [Configuration](/docs/18-configuration/) and [Running LeapMux](/docs/17-running-leapmux/) for where flags and config keys live.
...
> **Note:** Only `--client-id` and `--client-secret` are universally required, alongside `--type`. The remaining requirements depend on the preset: a generic `oidc` provider additionally requires `--name`, `--issuer-url`, and `--trust-email`.
...
3. The client secret is encrypted with the Hub's **active encryption key** and stored. (For key rotation and re-encryption, see [Encryption & Data](/docs/22-encryption-and-data/).)
...
Generic OIDC (e.g. Okta, Auth0, Keycloak, Entra ID) — `--name`, `--issuer-url`, and `--trust-email` are all required:
...
All of this UI is covered in detail in [Accounts & Authentication](/docs/05-accounts-and-authentication/); it requires no extra configuration beyond enabling the provider.
...
> **Tip:** If a stored client secret can no longer be decrypted after an encryption-key change, you have rotated keys without re-encrypting secrets. Run the re-encryption step described in [Encryption & Data](/docs/22-encryption-and-data/).

## Related chapters

- [Accounts & Authentication](/docs/05-accounts-and-authentication/) — the end-user sign-in, sign-up, and account-linking experience.
- [Admin CLI](/docs/20-admin-cli/) — full `leapmux admin` reference, including data-directory resolution.
- [Running LeapMux](/docs/17-running-leapmux/) — run modes, listen addresses, `--public-url`, and reverse-proxy setup.
- [Configuration](/docs/18-configuration/) — config precedence and the full key reference (`signup_enabled`, `public_url`, `secure_cookies`, …).
- [Encryption & Data](/docs/22-encryption-and-data/) — how client secrets are encrypted at rest, key rotation, and re-encryption.
