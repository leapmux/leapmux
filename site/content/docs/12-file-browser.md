...
The show/hidden and refresh buttons have keyboard shortcuts that work anywhere in the workspace. See [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/) for the full keybindings table and how to customize them.
...
> **Note:** The tree's expanded/collapsed state and cached listings are remembered per repository, so re-opening a workspace restores your place in the tree.
...
| **Copy relative path** | When a repo root is known | Copies the path relative to the worker's working directory. |
...
File tabs participate in the workspace's tiling and layout system like any other tab, so you can split, float, and rearrange them. See [Tabs & Layout](/docs/08-tabs-and-layout/). The file's path is registered with the worker over the encrypted channel so that collaborators in the same workspace can resolve the same file — see [Collaboration & Presence](/docs/13-collaboration-and-presence/).
...
Images open at a fit-to-view zoom. The image toolbar provides **Zoom out**, a zoom percentage, **Zoom in**, **Fit to view**, and an **Actual size (100%)** button. Zoom ranges from 25% to 500% in 25% steps, and you can pinch-zoom (Ctrl+wheel) toward the cursor. A checkerboard background sits behind transparent images.
...
This is the fastest way to point an agent at a specific span of code without retyping it. See [Coding Agents](/docs/09-coding-agents/) for how quotes appear in the chat.
...
The committed and staged versions are read from git on the worker (`git show HEAD:<file>` and `git show :0:<file>`); the diff itself is computed on your side and rendered in unified or split layout. Entering a diff auto-scrolls to center the first change.
...
When the worker is on another machine, every one of these requests and responses travels over the end-to-end-encrypted worker channel. The Hub relays the encrypted traffic but cannot read your paths or file contents. This is the same channel your agents and terminals use. For how remote workers are registered, approved, and selected, see [Managing Workers](/docs/19-managing-workers/); for the encryption details, see [Security & Threat Model](/docs/23-security-and-threat-model/).

> **Note:** Worker-side git operations run with a fixed English/C locale, so error messages surfaced in the browser (for example, a dubious-ownership warning) are always in English.

## Related chapters

- [Worktrees & Branches](/docs/10-worktrees-and-branches/) — open a tab in a worktree, switch/create/delete branches, push, and the dirty-worktree protections. The file browser reflects the branch and working directory of whatever tab is active, including linked worktrees.
- [Terminals](/docs/11-terminals/) — the "Open a terminal tab here" action opens a terminal rooted at the selected directory.
- [Coding Agents](/docs/09-coding-agents/) — quoting and mentioning files feeds the active agent's chat.
- [Tabs & Layout](/docs/08-tabs-and-layout/) — file tabs tile, split, and float like any other tab.
