...
Each provider exposes its own settings — models and permission modes, and where the CLI supports it, effort/reasoning levels — which you change mid-session from the composer's status-bar chips or its **[+]** menu. For example, Claude Code defaults to the `opus[1m]` model and Codex to `gpt-5.4`. For how to open an agent, chat, answer permission prompts, switch models, and resume a session, see [Coding Agents](/docs/using/coding-agents/).
...
- **Running & operating** — [Running LeapMux](/docs/operating/running-leapmux/), [Configuration](/docs/operating/configuration/), [Managing Workers](/docs/operating/managing-workers/), [Recovery](/docs/operating/recover/), [Authentication Providers](/docs/operating/authentication-providers/), [Encryption & Data](/docs/operating/encryption-and-data/), [Security & Threat Model](/docs/operating/security/), and [Remote Control CLI](/docs/operating/control-cli/), which covers driving a running instance (or scripting an agent) with `leapmux control`.
