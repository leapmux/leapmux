...
Running `leapmux admin` with no group is an error: it writes `error: admin group is required` to stderr, a blank line, then the root usage, and exits non-zero. Clean help (to stdout, exit 0) only appears when you pass an explicit help token — `leapmux admin --help`:

```text
error: admin group is required
...
> **Tip:** If your Hub runs on Postgres or MySQL, always pass `--config /path/to/hub.yaml` so the admin command connects to the same database. Without `--config`, the CLI builds a SQLite-only config and would operate on (or create) `{DataDir}/hub.db` instead of your real backend. See [Configuration](/docs/operating/configuration/) for the config file format and [Encryption & Data](/docs/operating/encryption-and-data/) for storage backends.
...
| `--email-verified` | `true` or `false` (recommended). Parsed by Go's `strconv.ParseBool`, so `1`/`0`/`t`/`f` are also accepted; an unparseable value errors with `must be 'true' or 'false'`. |
...
For the full picture of how Workers register and connect — registration keys, the no-pending-approval model, online/offline status, and TOFU key pinning — see [Managing Workers](/docs/operating/managing-workers/).
...
Hard-delete every key whose expiry is in the past, in batches of 1000 until drained. The criterion is purely expiry: revoked keys that are also expired are included, but a revoked key that has not yet expired is not purged. No command-specific flags. Success: `Purged %d expired registration keys.`

---

## `oauth-provider` — OAuth/OIDC login providers

These commands configure operator-level OAuth/OIDC sign-in. For a step-by-step walkthrough of each provider, see [Authentication Providers](/docs/operating/authentication-providers/).
...
| `--trust-email` | `true` or `false` — trust email from this provider as verified. Parsed by Go's `strconv.ParseBool`, so `1`/`0`/`t`/`f` are also accepted. |
...
> **Warning:** `oauth-provider add` encrypts the client secret with the active encryption key, so the Hub must have an `encryption.key` file. If you have never run the Hub, run it once to auto-generate the key (see [Encryption & Data](/docs/operating/encryption-and-data/)).
...
> **Note on ALTCHA cost semantics:** ALTCHA v2's cost is the per-derivation iteration count, and the browser brute-forces roughly 256 derivations per solve — total work is ~256 × cost. The default (`10000` ≈ 2.6M PBKDF2 iterations) solves in well under a second with native WebCrypto on desktop. Raising the cost multiplies bot cost **and** your users' wait time equally, so values far above the default mostly punish humans.

### `captcha show`

No command-specific flags. Prints the selected provider's effective configuration as JSON: `"provider"`, `"enabled"`, that provider's settings, `"customized": true/false` for whether a stored row exists, and `"configured"` listing every provider that has a row. Secrets are never printed.
...
# Switch to a memory-hard ALTCHA algorithm
leapmux admin captcha set --algorithm ARGON2ID --cost 2 --memory-cost 65536

# Switch to Turnstile (from the Cloudflare dashboard's widget keys)
leapmux admin captcha set --provider turnstile --site-key 0x4AAAA... --secret 0x4AAAA...

# Switch to reCAPTCHA v3 with a stricter threshold (Google admin console keys)
leapmux admin captcha set --provider recaptcha_v3 --site-key 6Lc... --secret 6Lc... --min-score 0.7

# Rotate a provider secret without re-activating
leapmux admin captcha set --secret 0x4AAAA...new

# Back to the built-in provider
leapmux admin captcha set --provider altcha
```

The ALTCHA SHA and PBKDF2 families solve in every browser via native WebCrypto; SCRYPT and ARGON2ID are memory-hard (stronger against GPU farms, slower on low-end mobile) and their solvers load on demand in the web frontend. A browser needs a secure context (HTTPS or localhost) for the WebCrypto solvers, so front the hub with TLS when you reach it from anything other than localhost.

### `captcha enable` / `captcha disable`

No command-specific flags. Toggles token enforcement on the selected provider; the selection and all settings are preserved (a later `enable` restores the same provider), and the honeypot check stays active either way.

### `captcha reset`

`--provider <name>` resets only that provider's row; without it, every row is deleted and the built-in defaults apply again. Deleting the selected provider leaves nothing selected until the Hub's next use, which lazily re-provisions the ALTCHA row (with a fresh signing secret, invalidating challenges in flight — they expire within the configured window anyway).
...
Opening the store auto-applies any pending migrations, so a normal Hub start already migrates the schema to the latest version. These commands let you confirm the schema state, find the database file, or roll to a specific version where supported. See [Encryption & Data](/docs/operating/encryption-and-data/) for the storage backends and migration model.
...
**Add an OAuth provider.** See [`oauth-provider add`](#oauth-provider-add) for the per-type presets and examples; verify with [`oauth-provider list`](#oauth-provider-list).

**Create, re-encrypt under, and rotate an encryption key.** See the full rotation runbook under [`encryption-key rotate-pepper`](#encryption-key-rotate-pepper).
...
**Issue, list, and revoke an API token.** See [`api-token issue`](#api-token-issue), [`api-token list`](#api-token-list), and [`api-token revoke`](#api-token-revoke).
