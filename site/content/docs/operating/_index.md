---
title: "Operating"
description: "Host LeapMux for a team: run modes, configuration, worker and account administration, encryption, the security model, and scripting a running instance."
type: docs
weight: 3
sidebar:
  open: true
---

For operators hosting LeapMux for a team: run modes, configuration, worker and
account administration, encryption, the security model, and scripting a running
Hub.

{{< cards >}}
  {{< card link="/docs/operating/running-leapmux/" title="Running LeapMux" icon="server" subtitle="Run modes, ports, data directories, and Docker." >}}
  {{< card link="/docs/operating/configuration/" title="Configuration" icon="cog" subtitle="Every config key, environment variable, and storage backend." >}}
  {{< card link="/docs/operating/managing-workers/" title="Managing Workers" icon="chip" subtitle="Register, approve, pin, and select Workers." >}}
  {{< card link="/docs/operating/sign-in-providers/" title="Sign-in Providers" icon="key" subtitle="Configure OAuth / OIDC sign-in." >}}
  {{< card link="/docs/operating/app-authorization/" title="App Authorization" icon="shield-check" subtitle="Register apps, control what they may ask for, and vouch for the ones you trust." >}}
  {{< card link="/docs/operating/encryption-and-data/" title="Encryption & Data" icon="lock-closed" subtitle="The encryption key ring, rotation, and what is stored." >}}
  {{< card link="/docs/operating/security/" title="Security & Threat Model" icon="shield-check" subtitle="Trust boundaries, end-to-end encryption, and worker pinning." >}}
  {{< card link="/docs/operating/control-cli/" title="Control CLI" icon="terminal" subtitle="Drive a running Hub or script an agent with leapmux control." >}}
  {{< card link="/docs/operating/admin-cli/" title="Admin CLI" icon="terminal" subtitle="Administer a running Hub: users, sessions, settings, and tokens." >}}
  {{< card link="/docs/operating/recover/" title="Recovery" icon="support" subtitle="Offline break-glass: first-admin bootstrap, password reset, keys, database." >}}
{{< /cards >}}
