...
**In-band rekey** rotates ChaCha20-Poly1305 transport keys without closing the channel: the initiator sends a `RekeyRequest`, holds outbound traffic until `RekeyAck` or `RekeyReject`, and only then switches keys. Reject leaves both CipherStates unchanged. This is shared by the Frontend, `leapmux remote`, cross-worker links, and the desktop app's tunnels — so port-forwards and SOCKS sessions survive hourly key rotation. Rotation uses `HKDF(k)` rather than a fresh Diffie–Hellman handshake: the Hub still never learns keys, but compromise of the current epoch key can derive the next. Fresh-DH rotation while keeping conns alive is tracked in [issue #321](https://github.com/leapmux/leapmux/issues/321).
