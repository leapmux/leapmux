...
```bash
leapmux control admin oauth-provider add \
...
> **Tip:** If you are not sure which flags a preset requires or what the exact help text is, run `leapmux control admin oauth-provider add --help`.
...
1. Create the provider with `leapmux control admin oauth-provider add` (note the printed `id`).
...
```bash
leapmux control admin oauth-provider list
```
...
```bash
leapmux control admin oauth-provider disable --id <providerID>
leapmux control admin oauth-provider enable  --id <providerID>
```

### Remove

```bash
leapmux control admin oauth-provider remove --id <providerID>
```
...
```bash
leapmux control admin oauth-provider remove --id <providerID> --force
```
...
|---|---|---|
| Provider's page shows "redirect URI mismatch" / "invalid redirect_uri" | The redirect URI registered with the provider doesn't exactly match `<base-url>/auth/oauth/<id>/callback`. | Run `leapmux control admin oauth-provider list` to get the `id`, rebuild the callback URL, and register it verbatim (scheme, host, port, path all matter). |
...
| Button doesn't appear | Provider is disabled, or not created. | `leapmux control admin oauth-provider list`; if `enabled` is `false`, run `enable --id <id>`. |
