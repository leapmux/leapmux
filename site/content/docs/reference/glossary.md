...
A coding agent: a CLI assistant (Claude Code, Codex, Cursor, GitHub Copilot, OpenCode, Pi, Kilo, Goose, Reasonix, ZCode, Codewhale, Kimi Code, MiMo Code, Qwen Code, Oh My Pi, Grok Build, Kiro, Amp, Cline, CodeBuddy Code, Junie, Letta Code, Dirac, Qoder CLI, Factory Droid, Fast Agent) that LeapMux launches and hosts on a Worker, one process per agent tab. You chat with it, watch its tool calls render inline, set its model and effort, and resume it across restarts. See [Coding Agents](/docs/using/coding-agents/).
