...
2. Launch the app. On first launch it shows a launcher titled **"LeapMux"** with the subtitle **"Choose how you'd like to connect"** and two cards:
   ...
   - **Distributed** — connect to a remote Hub by URL (covered in [Running LeapMux](/docs/17-running-leapmux/)).
...
If you installed the `leapmux` binary (see [Installation](/docs/03-installation/)), start solo mode from a terminal:
...
A few useful flags (full reference in [Configuration](/docs/18-configuration/) and [CLI Reference](/docs/24-cli-reference/)):
...
> **Warning:** Solo mode trusts every request as the admin. If you change `-listen` to a non-loopback address, anyone who can reach that port has full admin access without credentials. Keep solo on loopback, or run `leapmux hub` for real authentication. See [Security & Threat Model](/docs/23-security-and-threat-model/).
...
A **workspace** is the top-level container that holds your tiling layout. To create one, click the **"+"** button in a sidebar section header (the one in **In progress** has the test id `sidebar-new-workspace`). This opens the **"New workspace"** dialog:
...
Click **"Create"**. LeapMux creates the workspace, spawns its first agent, and navigates to it. To work in an existing workspace instead, click its row in the sidebar. Full workspace management — rename, move, archive, delete, share — is covered in [Workspaces](/docs/07-workspaces/).
...
> **Note:** The New agent dialog does **not** include model, effort, or permission-mode fields. A new agent starts with the provider's defaults — for Claude Code that's the **Opus (1M context)** model with effort **Auto** and the **Default** permission mode. You change all of these mid-session from the in-chat settings dropdown. See [Coding Agents](/docs/09-coding-agents/) for the full settings reference.
...
Worktrees keep each agent's changes on their own branch and directory, so several agents can work on the same repo at once without stepping on each other. The full set of modes, the worktree path layout, and branch operations — switching, pushing (**"Push"** / **"Commit and Push"**), and deletion — are covered in [Worktrees & Branches](/docs/10-worktrees-and-branches/).
...
- [Concepts & Architecture](/docs/02-concepts/) — how the Hub, Worker, and Frontend fit together, and what solo vs. distributed means.
- [Workspaces](/docs/07-workspaces/) — create, rename, move, archive, delete, and share workspaces.
- [Tabs & Layout](/docs/08-tabs-and-layout/) — tiling, splits, grids, floating windows, and drag-and-drop.
- [Coding Agents](/docs/09-coding-agents/) — providers, models, effort, control prompts, tool rendering, and resuming sessions.
- [Worktrees & Branches](/docs/10-worktrees-and-branches/) — branch and worktree workflows in depth.
- [Terminals](/docs/11-terminals/) — shell selection, persistence, and remote terminals.
- [File Browser](/docs/12-file-browser/) — the file tree, git status, and diffs.
- [Keyboard Shortcuts](/docs/15-keyboard-shortcuts/) — speed up everything above.
- [Running LeapMux](/docs/17-running-leapmux/) — move beyond solo mode to a shared Hub and remote workers.
