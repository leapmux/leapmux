...
func (tc *Conn) onStreamMessage(msg *leapmuxv1.InnerStreamMessage) {
	...
	if len(event.GetData()) > 0 {
		...
		// otherwise bounded only by the channel's negotiated reassembled limit
		// (default ~16 MiB + 64 KiB headroom; up to MaxConfigurableMessageSize +
		// headroom when operators raise max_message_size), and readBuf holds
		// ReadBufFrames of them: 256 × that ceiling can pin multi-GiB in the
