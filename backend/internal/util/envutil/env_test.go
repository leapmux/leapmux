...
func TestStripByPrefix_CaseSensitive(t *testing.T) {
...
func TestStripByPrefix_MatchesNameNotValue(t *testing.T) {
...
func TestStripByPrefix_EmptyPrefix_PassThrough(t *testing.T) {
	// An empty prefix matches every name (strings.HasPrefix(_, "") == true)
	// so all entries are dropped. Document the behavior so callers don't
	// accidentally pass "".
	got := StripByPrefix([]string{"A=1", "B=2"}, "")
	assert.Empty(t, got)
}

func TestStripByPrefix_EmptyInputs(t *testing.T) {
	assert.Empty(t, StripByPrefix(nil, "X_"))
