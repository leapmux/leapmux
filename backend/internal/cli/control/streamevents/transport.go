...
//
//   - hub-bound: open an E2EE channel via `*control.Client.OpenE2EEChannel`,
//     then `SendRPCNoWait` with atomically registered response and stream handlers.
//   - local-IPC: call `ControlIPCService.StreamInner` with method
...
// opened via `*control.Client.OpenE2EEChannel`.
//
...
// (`LEAPMUX_CONTROL_SOCK`). The router on the worker side proxies the
// stream to the appropriate inner-RPC handler.
type LocalIPCTransport struct {
	client leapmuxv1connect.ControlIPCServiceClient
...
func NewLocalIPCTransport(client leapmuxv1connect.ControlIPCServiceClient, targetWorkerID string, logger *slog.Logger) *LocalIPCTransport {
...
func (t *LocalIPCTransport) OpenWatchEvents(parentCtx context.Context, req *leapmuxv1.WatchEventsRequest,
	...
	if t.client == nil {
		return nil, errors.New("nil ControlIPCService client")
	}
	payload, err := proto.Marshal(req)
	if err != nil {
		return nil, err
