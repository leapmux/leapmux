...
// LEAPMUX_CONTROL_*_ID env-var defaults) -- best-effort preflights the
...
func withResolvedAgent(rawCtx any, args []string, opts agentScaffoldOpts) error {
	...
	if !hasAnyEntityInput(in) {
		return control.EmitError("invalid_request", "missing required ID(s): pass --tab-id (or set LEAPMUX_CONTROL_TAB_ID); --worker-id / --workspace-id / --tile-id cannot name an agent on their own")
	}
