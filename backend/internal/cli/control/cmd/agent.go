...
func RunAgentSend(rawCtx any, args []string) error {
	...
	return withResolvedAgent(rawCtx, args, agentScaffoldOpts{
		...
		validate: func() error {
			...
			if message == "" {
				return control.EmitError("invalid_request", "--message or --stdin is required")
			}
			return nil
		},
	...
	return withResolvedAgent(rawCtx, args, agentScaffoldOpts{
		body: func(ctx context.Context, c *control.Client, workerID, agentID string) error {
			...
			for _, a := range resp.GetAgents() {
				if a.GetId() == agentID {
					return control.EmitData(agentInfoToMap(a))
				}
			}
			return control.EmitError("not_found", "agent not found or not accessible: "+agentID)
		},
	})
}

// RunAgentProviders lists the agent providers a worker supports.
// Worker selection takes --worker-id (or $LEAPMUX_CONTROL_WORKER_ID)
...
// pre-filled from $LEAPMUX_CONTROL_AGENT_PROVIDER) into an
