...
func RunTabMove(rawCtx any, args []string) error {
	...
	if destTileID == "" && destWorkspaceID == "" && !specReferencesTile {
		return control.EmitError("invalid_request", "--target-tile-id, --target-workspace-id, or --before/--after is required")
	}
	...
	if destTileID == "" && destWorkspaceID != "" && !specReferencesTile {
		...
		if !ok || ws == nil {
			return control.EmitError("not_found", "no such workspace: "+destWorkspaceID)
		}
		leaf := firstLiveLeaf(cc.bs.State, ws.GetRootNodeId())
		if leaf == "" {
			return control.EmitError("not_found", "workspace "+destWorkspaceID+" has no live leaf tile")
		}
		destTileID = leaf
	}
