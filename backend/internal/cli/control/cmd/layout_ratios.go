...
func RunTileSetRatios(rawCtx any, args []string) error {
	...
	if ratiosCSV == "" {
		return control.EmitError("invalid_request", "--ratios is required")
	}
	...
	return control.EmitData(map[string]any{"split_tile_id": splitID, "ratios": ratios})
}
...
func RunTileSetGridRatios(rawCtx any, args []string) error {
	...
	if rowRatiosCSV == "" && colRatiosCSV == "" {
		return control.EmitError("invalid_request", "at least one of --row-ratios / --col-ratios is required")
	}
	var rowRatios, colRatios []float64
	if rowRatiosCSV != "" {
		r, err := parseRatiosCSV(rowRatiosCSV)
		if err != nil {
			return control.EmitErrorWith("invalid_request", fmt.Errorf("--row-ratios: %w", err))
		}
		rowRatios = r
	}
	if colRatiosCSV != "" {
		...
		if err != nil {
			return control.EmitErrorWith("invalid_request", fmt.Errorf("--col-ratios: %w", err))
		}
		colRatios = c
	}
