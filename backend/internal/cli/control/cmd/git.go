...
func workingDirEnv() string {
	return os.Getenv("LEAPMUX_CONTROL_WORKING_DIR")
}
...
func RunGitRead(rawCtx any, args []string) error {
	...
	if f.Path == "" {
		...
		// $LEAPMUX_CONTROL_WORKING_DIR isn't a useful default —
		// keep the flag explicitly required.
		return control.EmitError("invalid_request", "--path is required (must be a file path; the working-dir default only applies to git status/branches/worktrees)")
	}
	...
	default:
		return control.EmitError("invalid_request", "--ref must be head or staged")
	}
	c, workerID, err := resolveWorker(f.Hub, f.In)
	if err != nil {
		return err
