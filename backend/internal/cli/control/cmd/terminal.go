...
func RunTerminalSend(rawCtx any, args []string) error {
	...
	if stdinFlag {
		...
		if err != nil {
			return control.EmitErrorWith("stdin_read_failed", err)
		}
		payload = buf
	} else {
	...
	if len(payload) == 0 {
		return control.EmitError("invalid_request", "--data or --stdin (with non-empty input) is required")
	}
	return resolveAndEmit(hub, resolve.Need{TabID: true, WorkerID: true}, in, func(ctx context.Context, c *control.Client, got resolve.Resolved) error {
		...
		return control.EmitData(map[string]any{"tab_id": got.TabID, "bytes_sent": len(payload)})
	})
}
	...
	return resolveAndEmit(hub, resolve.Need{TabID: true, WorkerID: true}, in, func(ctx context.Context, c *control.Client, got resolve.Resolved) error {
		...
		for _, t := range resp.GetTerminals() {
			if t.GetTerminalId() == got.TabID {
				if screenOnly {
					_, _ = control.Out.Write(t.GetScreen())
					return nil
				}
				return control.EmitData(terminalInfoToMap(t))
			}
		}
		return control.EmitError("not_found", "terminal not found or not accessible: "+got.TabID)
	})
}
...
// comes from --worker-id (or $LEAPMUX_CONTROL_WORKER_ID), or from
...
func renameTerminalTab(ctx context.Context, c *control.Client, got resolve.Resolved, title string) error {
