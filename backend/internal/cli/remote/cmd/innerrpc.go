...
func withWorkerChannel(ctx context.Context, c *remote.Client, workerID string, body func(w workerCall) error) error {
	if c.IsLocal() {
...
func withBestEffortWorkerChannel(ctx context.Context, c *remote.Client, workerID string, body func(w workerCall) error) error {
...
// bearer-scope check when one is present (remoteipc.Router.CallInner) -- but a
...
func localIPCCallInnerBest(ctx context.Context, c *remote.Client, workerID, method string, payload []byte, out proto.Message) error {
	...
	if err != nil {
		...
		// remoteipc.relayError to have preserved the code -- a flat
