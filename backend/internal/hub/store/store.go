...
// Credential lifecycle event kinds. revocation_events.kind stores these
// ordinals, and its CHECK refuses the UNSPECIFIED zero, so an event that never
// stated its kind fails the insert rather than reaching a consumer that cannot
// classify it.
const (
	...
	RevocationEventKindSession = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_SESSION
	...
	RevocationEventKindSessionRevoked   = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_SESSION_REVOKED
	RevocationEventKindAPIToken         = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_API_TOKEN
	RevocationEventKindAPITokenRotation = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_API_TOKEN_ROTATION
	RevocationEventKindDelegationToken  = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_DELEGATION_TOKEN
	RevocationEventKindUserTokens       = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_USER_TOKENS
	...
	RevocationEventKindUserInfo = leapmuxv1.RevocationEventKind_REVOCATION_EVENT_KIND_USER_INFO
)
...
type WebAuthnSessionStore interface {
	...
	ConsumeCeremony(ctx context.Context, id string, kind leapmuxv1.WebAuthnSessionKind, now time.Time) (int64, error)
	DeleteAllByUser(ctx context.Context, userID string) error
	// DeleteByUserAndKind removes open ceremony rows for one user and kind
	// (for example, replacing a prior elevation Begin).
	DeleteByUserAndKind(ctx context.Context, userID string, kind leapmuxv1.WebAuthnSessionKind) error
}
