...
func seedAPITokenForList(t *testing.T, st store.Store, userID, clientID, installationName string) string {
	t.Helper()
	tokenID := id.Generate()
...
func (s *Suite) testTokenListing(t *testing.T) {
	...
	t.Run("api tokens list all joins owner and pages through", func(t *testing.T) {
		...
		t1 := seedAPITokenForList(t, st, alice.ID, oauthapp.ControlCLIClientID, "alice-cli")
		t2 := seedAPITokenForList(t, st, bob.ID, oauthapp.ControlCLIClientID, "bob-cli")
		t3 := seedAPITokenForList(t, st, bob.ID, oauthapp.ServiceAccountClientID, "bot")
	...
	t.Run("api tokens list all client_id filter", func(t *testing.T) {
		st := s.NewStore(t)
		user := SeedUser(t, st, "filter-user")
		seedAPITokenForList(t, st, user.ID, oauthapp.ControlCLIClientID, "a")
		seedAPITokenForList(t, st, user.ID, oauthapp.ServiceAccountClientID, "b")

		page, err := st.APITokens().ListAll(ctx, store.ListAllAPITokensParams{ClientID: oauthapp.ControlCLIClientID, PageParams: store.PageParams{Limit: 10}})
		require.NoError(t, err)
		require.Len(t, page.Rows, 1)
		assert.Equal(t, oauthapp.ControlCLIClientID, page.Rows[0].ClientID)
	})

	t.Run("api tokens list all client_id filter composes with pagination", func(t *testing.T) {
		st := s.NewStore(t)
		user := SeedUser(t, st, "filter-page-user")
		c1 := seedAPITokenForList(t, st, user.ID, oauthapp.ControlCLIClientID, "c1")
		i1 := seedAPITokenForList(t, st, user.ID, oauthapp.ServiceAccountClientID, "i1")
		c2 := seedAPITokenForList(t, st, user.ID, oauthapp.ControlCLIClientID, "c2")
		i2 := seedAPITokenForList(t, st, user.ID, oauthapp.ServiceAccountClientID, "i2")
		c3 := seedAPITokenForList(t, st, user.ID, oauthapp.ControlCLIClientID, "c3")
		// Interleave the two APPS in time so the cursor from page one
		// (at c2) straddles the other app's rows on both sides: i2 is newer and
		// i1 is older than the cursor, and neither may leak into the control
		// CLI's page -- the filter conjunct and the keyset conjunct must
		// compose.
		base := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
		...
		page, err := st.APITokens().ListAll(ctx, store.ListAllAPITokensParams{ClientID: oauthapp.ControlCLIClientID, PageParams: store.PageParams{Limit: 2}})
		require.NoError(t, err)
		require.Len(t, page.Rows, 2)
		...
		page, err = st.APITokens().ListAll(ctx, store.ListAllAPITokensParams{ClientID: oauthapp.ControlCLIClientID, PageParams: store.PageParams{Cursor: page.NextCursor, Limit: 2}})
		require.NoError(t, err)
		require.Len(t, page.Rows, 1)
	...
	t.Run("api tokens list all filters by user and paginates", func(t *testing.T) {
		...
		a1 := seedAPITokenForList(t, st, alice.ID, oauthapp.ControlCLIClientID, "a1")
		b1 := seedAPITokenForList(t, st, bob.ID, oauthapp.ControlCLIClientID, "b1")
		a2 := seedAPITokenForList(t, st, alice.ID, oauthapp.ControlCLIClientID, "a2")
		a3 := seedAPITokenForList(t, st, alice.ID, oauthapp.ControlCLIClientID, "a3")
		base := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
	...
	t.Run("api tokens list all surfaces soft-deleted owner as (deleted)", func(t *testing.T) {
		...
		token := seedAPITokenForList(t, st, dead.ID, oauthapp.ControlCLIClientID, "dead-cli")
