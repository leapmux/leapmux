...
SELECT * FROM captcha_config WHERE selected = 1;
...
SET settings = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE provider = ?;
...
INSERT INTO altcha_used_salts (salt, expires_at) VALUES (?, ?) ON CONFLICT (salt) DO NOTHING;
