...
WHERE revoked_at IS NULL AND client_id IN (sqlc.slice(client_ids))
GROUP BY client_id;
...
DELETE FROM device_authorizations WHERE client_id = ?;
...
-- every boot: the update list below names ONLY the columns that are constants
-- of the build (see internal/hub/oauthapp). Everything else is deliberately
