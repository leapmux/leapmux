...
// the hub to mint a short-lived delegation token bound to user_id alone. The
// minted bearer is what spawned agents present when
...
// backoff used when the (user_id, tab_id, worker_id) row hasn't yet
// appeared in workspace_tab_owned. Worker spawn-time minting can race
...
func (h *WorkerDelegationHandler) handleMint(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserID == "" || req.IssuedForTabID == "" {
		http.Error(w, "user_id, issued_for_tab_id are required", http.StatusBadRequest)
		return
	}
	...
	// point (the registrant check runs after it).
	//
	...
	// is "no access", the same shape the registrant check below returns.
	...
	// calling worker's own registrant. That is the whole tenancy check now that
	// the bearer carries no workspace pin: a worker may only mint a bearer for
	// the one user it belongs to.
	//
	...
	// a blank req.UserID -- a raw != would fail OPEN on exactly the input this
	// check exists to refuse, minting a bearer that impersonates another user.
	//
	// Runs BEFORE the ownership poll below, and the order is load-bearing rather
	// than tidy. The poll binds the CALLER-supplied user_id and waits up to
	// DefaultMintTabPropagationTimeout, so a request naming a foreign user_id --
	// which can never match -- used to pin a hub goroutine and a stream of point
	// lookups for the full timeout before this O(1) comparison refused it.
	// Anything holding a worker auth_token could repeat that cheaply. Both
	// checks answer 403 and the poll is read-only, so hoisting it changes no
	// outcome except which message a doubly-invalid request gets.
	if !uid.Matches(worker.RegisteredBy) {
		http.Error(w, "user is not the worker's registrant", http.StatusForbidden)
		return
	}
	// Verify the (user_id, tab_id, worker_id) triple exists in
	...
	tabFound, lookupErr := h.waitForTabOwnership(r.Context(), uid, req.IssuedForTabID, worker.ID)
	if lookupErr != nil {
...
// waitForTabOwnership polls workspace_tab_owned for the (user, tab,
...
func (h *WorkerDelegationHandler) waitForTabOwnership(ctx context.Context, userID userid.UserID, tabID, workerID string) (bool, error) {
	...
	params := store.GetOwnedTabParams{UserID: userID, TabID: tabID}
