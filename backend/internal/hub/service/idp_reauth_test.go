...
func beginReauth(t *testing.T, server *httptest.Server, providerID string, cookie *http.Cookie, redirect string) (state string, nonce *http.Cookie, authURL *url.URL) {
	t.Helper()
	target := server.URL + "/auth/idp/" + providerID + "/reauth"
	if redirect != "" {
...
func TestOAuthReauth_RequiresASession(t *testing.T) {
	...
	resp, err := noRedirectClient().Get(server.URL + "/auth/idp/" + providerID + "/reauth")
	require.NoError(t, err)
...
func TestOAuthReauth_RecordsThePurposeAndTheSessionAndForcesAPrompt(t *testing.T) {
	...
	state, nonce, authURL := beginReauth(t, server, providerID, cookie, "/oauth/authorize?state=s")
	...
	assert.Equal(t, "/oauth/authorize?state=s", row.RedirectURI)
}
...
func TestOAuthReauth_ElevatesTheSessionAndCreatesNothing(t *testing.T) {
	...
	state, nonce, _ := beginReauth(t, server, providerID, cookie, "/oauth/authorize?state=s")
	...
	loc, err := resp.Location()
	require.NoError(t, err)
	assert.Equal(t, "/oauth/authorize", loc.Path, "the browser returns to where it came from")
