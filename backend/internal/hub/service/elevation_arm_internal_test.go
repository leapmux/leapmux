...
// TestAccountElevatesOnlyThroughAProvider pins the step-up arm rule at its one
// source of truth.
//
...
// of passkeyManagementAuth reads it to know an account has nothing to elevate
// WITH, and GetCurrentUser REPORTS it as may_elevate so the step-up form
// offers exactly the arms the hub accepts. The form used to spell the rule
...
func TestAccountElevatesOnlyThroughAProvider(t *testing.T) {
	...
	// A password disqualifies the arm. That is the one shape where the
	// account chose a secret and the provider arm would let somebody past it
	// without knowing it.
	t.Run("a password disqualifies the arm", func(t *testing.T) {
		st := hubtestutil.OpenTestStore(t)
	...
	t.Run("a passkey disqualifies the arm", func(t *testing.T) {
		st := hubtestutil.OpenTestStore(t)
	...
	// A registered passkey disqualifies the arm just as a password does, and
	...
	// HOLDS one.
	t.Run("a registered passkey disqualifies the provider arm", func(t *testing.T) {
		st := hubtestutil.OpenTestStore(t)
		user := newUser(t, st, "haspasskey", false)
		require.NoError(t, st.PasskeyCredentials().Create(ctx, store.CreatePasskeyCredentialParams{
			ID:           id.Generate(),
