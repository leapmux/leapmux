...
func TestProviderCacheHoldsOneEntryPerProvider(t *testing.T) {
	...
	for _, redirect := range []string{
		"http://localhost:4327/auth/idp/gh/callback",
		"https://hub.example.com/auth/idp/gh/callback",
		"https://hub2.example.com/auth/idp/gh/callback",
	} {
	...
	other := providerCacheKey{providerID: "okta", redirectURL: "https://hub2.example.com/auth/idp/okta/callback"}
	h.cacheProvider(other, stub, h.providerGeneration("okta"))
	assert.Len(t, h.providers, 2)
}

// newProviderCacheTestHandler builds the cache halves of an IdPHandler,
// which is all the cache tests below touch.
func newProviderCacheTestHandler() *IdPHandler {
	return &IdPHandler{
...
func TestCacheProviderRefusesAProviderInvalidatedMidBuild(t *testing.T) {
	...
	stub := huboauth.NewGitHubProvider("id", "secret", "http://localhost/cb", nil)
	key := providerCacheKey{providerID: "gh", redirectURL: "https://hub.example.com/auth/idp/gh/callback"}
...
func TestBuildProviderDoesNotCacheADeletedProvider(t *testing.T) {
	...
	h := NewIdPHandler(st, &config.Config{}, set, auth.NewCredentialLifecycleEffects(nil, nil, nil), ks)
...
func TestLoginOAuthUser_UsesConfiguredSessionDuration(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/auth/idp/test/callback", nil)
	before := time.Now()
