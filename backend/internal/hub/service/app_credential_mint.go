...
//
// A mint the CONSENT flow performs is not. /oauth/token carries no session
...
//
// A SESSION actor, a SOLO actor and a consent grant clamp nothing. All three
// stand on a factor a human proved -- in a browser, or by reaching a solo
...
type apiTokenMint struct {
	...
	ClientID string
	// InstallationName is which copy of that app this credential belongs to.
	InstallationName string
	// GrantedScopes is the canonical scope string, already closed and narrowed
	// by the caller. Build it with authscope.ScopeSet.Storable, which refuses
	// the unscoped grant.
...
// credentialNotice is what the issuance email says about one credential.
//
...
type credentialNotice struct {
	AppName          string
	InstallationName string
	// Scopes are the granted tokens, sorted, as the message lists them.
	Scopes []string
	// IssuedByAdmin says the OWNER did not perform this. The two mint surfaces
	// differ on exactly that, and the message has to as well: a consent the
	// recipient gave and an administrator acting for them cannot share the
	// sentence "if this was you, nothing more is needed".
	IssuedByAdmin bool
}

// notifyCredentialIssued emails the account owner that an app credential was
// minted. It is how somebody learns that a credential they did not create
// exists -- the one signal that does not depend on them opening Preferences.
//
...
// human waits for. context.WithoutCancel for the same reason handleTokenRefresh
// uses it: the notice must not die because the client disconnected.
//
...
// it is a delivery to a stranger. Silent when the hub has no relay, because
// ErrEmailDisabled is a configuration state and not a failure -- logging it
// would print a warning on every app authorization of every hub without SMTP.
//
...
func notifyCredentialIssued(
	...
	msg := renderer.AppCredentialIssuedEmail(
		user.Email, notice.AppName, notice.InstallationName, notice.Scopes, notice.IssuedByAdmin)
	userID := user.ID
	...
	go func() {
		...
		if err := sender.Send(sendCtx, msg); err != nil {
			...
			slog.WarnContext(sendCtx, "could not send the app credential notice", "user_id", userID, "err", err)
		}
	}()
}
