...
func TestInterceptor_SoloMode_AutoAuthenticated(t *testing.T) {
	...
	interceptors := connect.WithInterceptors(interceptor)
	authSvc := service.NewAuthService(service.AuthServiceDeps{Store: st, Config: &config.Config{SoloMode: true}, Lifecycle: auth.NewCredentialLifecycleEffects(nil, nil, nil), Keystore: nil, Mail: mail.NewStubSender(), Renderer: mail.Renderer{}, Captcha: nil})
	path, handler := leapmuxv1connect.NewAuthServiceHandler(authSvc, interceptors)
	mux.Handle(path, handler)
...
func TestInterceptor_LeapMuxBearer_CacheEvictedOnRevoke(t *testing.T) {
	...
	t.Cleanup(sc.Stop)
	interceptors := connect.WithInterceptors(interceptor)
	authSvc := service.NewAuthService(service.AuthServiceDeps{Store: st, Config: &config.Config{}, Lifecycle: auth.NewCredentialLifecycleEffects(sc, nil, nil), Keystore: nil, Mail: mail.NewStubSender(), Renderer: mail.Renderer{}, Captcha: nil})
	path, handler := leapmuxv1connect.NewAuthServiceHandler(authSvc, interceptors)
	mux.Handle(path, handler)
	server := httptest.NewServer(mux)
	t.Cleanup(server.Close)
