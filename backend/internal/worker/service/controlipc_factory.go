...
//
// It is part of the ControlIPCFactory contract rather than an implementation
...
// ControlIPCFactory abstracts the per-agent local-IPC server lifecycle so
...
// in the actual controlipc.Server + crossworker.Client.
type ControlIPCFactory interface {
...
// scope its bearer / socket name appropriately. UserID / WorkerID /
// TabID / WorkingDir / AgentProvider feed into LEAPMUX_CONTROL_* env
// vars (TAB_ID, TAB_TYPE=agent, USER_ID, WORKER_ID, WORKING_DIR,
// AGENT_PROVIDER) so child CLI invocations can default flags. Neither
// workspace id nor tile id is here -- the CLI derives both from the tab
// id at command time via the hub's LocateTab RPC.
type AgentSpawnInfo struct {
	...
	WorkerID      string
	TabID         string // The spawned agent's id (becomes LEAPMUX_CONTROL_TAB_ID).
	WorkingDir    string
