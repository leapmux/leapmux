...
func TestAllDatetimeColumnsStoreCanonicalLayout(t *testing.T) {
	...
	// agent_background_tasks: created_at/updated_at via Upsert's DEFAULT;
	// ended_at via CloseAgentBackgroundTask's strftime. Agent agent-1 (root)
	// already exists; insert a running row then close it.
