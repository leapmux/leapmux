...
func (h *OutputHandler) applyGoalUpdateFromRow(agentID string, provider leapmuxv1.AgentProvider, row db.GetAgentGoalRow, update agent.GoalUpdate) {
	...
	goal := goalProto(GoalColumns{NativeID: nativeID, Objective: update.Objective, Status: update.Status, StatusDetail: update.StatusDetail, CreatedAt: createdAt})
...
func (h *OutputHandler) GoalSnapshotFrom(cols GoalColumns) GoalSnapshot {
	...
	status, statusDetail := cols.Status, cols.StatusDetail
	...
	if goalPresent(cols.Objective) && !h.agentAlive(cols.AgentID) {
		status, statusDetail = agent.GoalStatusDormant, ""
	}
	cols.Status, cols.StatusDetail = status, statusDetail
	return GoalSnapshot{
		Goal:      goalProto(cols),
...
func (s *agentOutputSink) UpdateGoalStatus(expected, status agent.GoalStatus) {
	...
	if !goalPresent(row.GoalObjective) || agent.GoalStatus(row.GoalStatus) != expected {
		return
	}
