...
// createTestAgent seeds an agent row whose title NOBODY CHOSE, which is what
// every plan case below needs unless it says otherwise: plan-mode auto-rename
// overwrites only such a title. A case about a title the user typed calls
// createTestAgentWithChosenTitle.
func createTestAgent(t *testing.T, queries *db.Queries, agentID, title string) {
	t.Helper()
	createAgentWithTitleProvenance(t, queries, agentID, title, true)
}

// createTestAgentWithChosenTitle seeds an agent row whose title a USER typed.
func createTestAgentWithChosenTitle(t *testing.T, queries *db.Queries, agentID, title string) {
	t.Helper()
	createAgentWithTitleProvenance(t, queries, agentID, title, false)
}

func createAgentWithTitleProvenance(t *testing.T, queries *db.Queries, agentID, title string, auto bool) {
	t.Helper()
	autoValue := int64(0)
	if auto {
		autoValue = 1
...
func TestUpdatePlan_FirstWrite_PreservesUserSetAgentTitle(t *testing.T) {
	...
	h, queries, _ := newPlanHandler(t)
	createTestAgentWithChosenTitle(t, queries, "agent-1", "My custom title")
...
func TestUpdatePlan_NotificationEmittedWithoutAutoRename(t *testing.T) {
	...
	h, queries, _ := newPlanHandler(t)
	createTestAgentWithChosenTitle(t, queries, "agent-1", "User Set Title")
...
// TestUpdatePlan_PreservesATypedTitleThatLooksGenerated is the case the regex
// could not answer.
//
// Auto-rename used to decide "may I overwrite this?" by matching the RENDERED
// title against `^Agent [A-Z][A-Za-z]+$`. A user who typed `Agent Bob`, or who
// edited the pre-filled `Agent Gabe` to another single word -- the likely edit,
// now that the dialog pre-fills exactly that shape and sends it -- matched, and
// their title was silently replaced. The row records the answer instead.
func TestUpdatePlan_PreservesATypedTitleThatLooksGenerated(t *testing.T) {
	t.Parallel()

	for _, title := range []string{
		"Agent Bob",      // a pooled name the user happened to type
		"Agent Refactor", // the pre-fill with its name half edited
		"Agent Smith",
	} {
		t.Run(title, func(t *testing.T) {
			t.Parallel()
			h, queries, _ := newPlanHandler(t)
			createTestAgentWithChosenTitle(t, queries, "agent-1", title)
			...
			assert.Equal(t, title, row.Title,
				"a title the user typed must survive, whatever shape it happens to have")
			assert.Equal(t, "Rendering fixes", row.PlanTitle, "the plan itself is still recorded")
		})
	}
}

// The mirror: a title of ANY shape that nobody chose is overwritable, so the
// pool is no longer tied to the pattern a regex demanded.
func TestUpdatePlan_OverwritesAnyGeneratedTitle(t *testing.T) {
	t.Parallel()

	for _, title := range []string{
		"Agent Gabe", // the pooled shape
		"Agent 2",    // a shape the old regex refused
		"agent lowercase",
		"Prior plan title from an earlier rename",
	} {
		t.Run(title, func(t *testing.T) {
			t.Parallel()
			h, queries, _ := newPlanHandler(t)
			createTestAgent(t, queries, "agent-1", title)

			updatePlanHelper(t, h, "agent-1", "Rendering fixes", []byte("# Rendering fixes\n"))

			row, err := queries.GetAgentByID(context.Background(), "agent-1")
			require.NoError(t, err)
			assert.Equal(t, "Rendering fixes", row.Title)
		})
	}
}

// A plan rename leaves the row auto-generated, so a LATER plan title replaces
// it too. Losing that would freeze the tab on the first plan's name.
func TestUpdatePlan_AutoRenameKeepsTheRowOverwritable(t *testing.T) {
	t.Parallel()

	h, queries, _ := newPlanHandler(t)
	createTestAgent(t, queries, "agent-1", "Agent Gabe")

	updatePlanHelper(t, h, "agent-1", "First plan", []byte("# First plan\n"))
	updatePlanHelper(t, h, "agent-1", "Second plan", []byte("# Second plan\n"))

	row, err := queries.GetAgentByID(context.Background(), "agent-1")
	require.NoError(t, err)
	assert.Equal(t, "Second plan", row.Title)
	assert.NotZero(t, row.TitleAutoGenerated, "the model chose it, so it stays overwritable")
}

// RenameAgent is a user typing a title, so it must end the overwriting.
func TestRenameAgent_StopsPlanModeOverwritingTheTitle(t *testing.T) {
	t.Parallel()
	...
	row, err := queries.GetAgentByID(context.Background(), "agent-1")
	require.NoError(t, err)
	assert.Equal(t, "My custom title", row.Title)
	assert.Zero(t, row.TitleAutoGenerated, "a rename records that a user chose the title")
}
