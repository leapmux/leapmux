...
func TestGetAgentSpanMessagesSeparatesProviderSessions(t *testing.T) {
	t.Parallel()
	service, dispatcher, _ := setupTestService(t)
	require.NoError(t, service.Queries.CreateAgent(t.Context(), db.CreateAgentParams{
		ID: "agent", AgentProvider: leapmuxv1.AgentProvider_AGENT_PROVIDER_CLAUDE_CODE,
	...
	for _, sessionID := range []string{"old", "new", ""} {
		for _, side := range []string{"request", "result"} {
			_, err := createMessageRow(t.Context(), service.Queries, db.CreateMessageParams{
				...
				Content: []byte(`{"native":"unchanged"}`), CreatedAt: sqltime.NewSQLiteTime(time.Now()),
			})
			require.NoError(t, err)
		}
	}
	for _, sessionID := range []string{"old", "new", "", "missing"} {
		writer := newTestWriter()
		dispatch(dispatcher, contracts.RPCMethodGetAgentSpanMessages, &leapmuxv1.GetAgentSpanMessagesRequest{
			AgentId: "agent", SpanId: "same-tool", AgentSessionId: sessionID,
		}, writer)
		require.Empty(t, writer.errors)
		...
		require.Equal(t, sessionID+"-request", response.Messages[0].Id)
		require.Equal(t, sessionID+"-result", response.Messages[1].Id)
		for _, message := range response.Messages {
			require.Equal(t, sessionID, message.AgentSessionId)
		}
	}
}
