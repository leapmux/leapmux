...
func TestBgTask_UpsertCreatesRowAndBroadcasts(t *testing.T) {
	...
	assert.Equal(t, int64(bgtask.KindSubagent), rows[0].Kind)
	assert.Equal(t, "build feature", rows[0].Title)
	assert.Equal(t, int64(bgtask.StatusRunning), rows[0].Status)
}
...
func TestBgTask_UpdateStatusFinalStampsEndedAt(t *testing.T) {
	...
	rows := listRows()
	require.Len(t, rows, 1)
	assert.Equal(t, int64(bgtask.StatusCompleted), rows[0].Status)
...
func TestBgTask_UpdateStatusMonotonicOnFinal(t *testing.T) {
	...
	rows := listRows()
	require.Len(t, rows, 1)
	assert.Equal(t, int64(bgtask.StatusCompleted), rows[0].Status, "finished row stays finished")
...
func TestBgTask_UpsertMonotonicOnFinal(t *testing.T) {
	...
	rows := listRows()
	require.Len(t, rows, 1)
	assert.Equal(t, int64(bgtask.StatusInterrupted), rows[0].Status, "replayed running upsert does not resurrect a finished row")
	assert.True(t, rows[0].EndedAt.Valid, "ended_at stays stamped")
}
...
func TestBgTask_PartialUpsertPreservesExistingFields(t *testing.T) {
	...
	assert.Equal(t, int64(bgtask.KindShell), rows[0].Kind, "kind preserved")
...
func TestBgTask_CloseIsIdempotentOnFinishedRow(t *testing.T) {
	...
	rows = listRows()
	require.Len(t, rows, 1)
	assert.Equal(t, int64(bgtask.StatusCompleted), rows[0].Status, "first final status wins")
...
func TestBgTask_RenameRekeysRowAndPreservesStatus(t *testing.T) {
	...
	assert.Equal(t, int64(bgtask.StatusCompleted), rows[0].Status, "status preserved across the rename")
...
func TestBgTask_RenameOntoOccupiedKeyDropsTheDuplicate(t *testing.T) {
	...
	assert.Equal(t, "sess-stable", rows[0].RowKey)
	assert.Equal(t, int64(bgtask.StatusCompleted), rows[0].Status, "the surviving row keeps its final status")
}
...
func TestBgTask_LoadSeedsCacheFromDB(t *testing.T) {
	...
	require.NoError(t, svc.Queries.UpsertAgentBackgroundTask(ctx, db.UpsertAgentBackgroundTaskParams{
		...
		Kind: int64(bgtask.KindSubagent), Title: "seeded row", Status: int64(bgtask.StatusRunning),
	}))
...
func TestBgTask_MarkExitedStoppedVsInterrupted(t *testing.T) {
	...
	for _, r := range rows {
		assert.Equal(t, int64(bgtask.StatusStopped), r.Status, "stopped=true maps to StatusStopped")
	...
	assert.Equal(t, int64(bgtask.StatusInterrupted), cRow.Status, "stopped=false maps to StatusInterrupted")
}
...
func TestUpsertBackgroundTaskDerivesAKeyForAnUnusableOne(t *testing.T) {
	...
	for _, tc := range []struct{ name, key string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, int64(bgtask.StatusCompleted), row.Status, "the close must find the row the upsert opened")
...
func TestBgTask_ReplayedRunningUpsertCannotResurrectARetainedRow(t *testing.T) {
	...
	assert.Equal(t, int64(bgtask.StatusCompleted), row.Status, "the replay must not resurrect the row")
...
func TestBgTask_CloseReachesARetainedRowAndEndsItsTranscript(t *testing.T) {
	...
	row := storedRow(t, svc, ownerID, "task-1")
	assert.Equal(t, int64(bgtask.StatusCompleted), row.Status)
...
func TestBgTask_ReviveReachesARetainedRow(t *testing.T) {
	...
	row := storedRow(t, svc, ownerID, "task-1")
	assert.Equal(t, int64(bgtask.StatusRunning), row.Status, "the subagent runs again")
...
func TestBgTask_ANoOpMutationOnARetainedRowDeletesNothing(t *testing.T) {
	...
	assert.Equal(t, int64(bgtask.StatusCompleted), storedRow(t, svc, ownerID, "unlinked-1").Status,
		"the unlinked row an eviction would have destroyed is still there")
	assert.Equal(t, int64(bgtask.StatusCompleted), storedRow(t, svc, ownerID, "task-1").Status,
