...
// exitTerminalAndWait sends `exit<exitArg>` followed by Enter and waits
// for the PTY to register as exited. Use exitArg="" for a clean exit
// (code 0) or " 42" for a non-zero exit. The returned writer captures
// the SendInput response so callers can assert against it if needed.
func exitTerminalAndWait(t *testing.T, svc *Context, d *channel.Dispatcher, terminalID, exitArg string) *testResponseWriter {
	...
	dispatch(d, "SendInput", &leapmuxv1.SendInputRequest{
		...
		Data:       []byte("exit" + exitArg + testutil.TestShellEnter()),
	}, w)
	require.Empty(t, w.errors)
