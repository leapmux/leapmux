...
func TestTokenStore_RegisterLookup(t *testing.T) {
	store := controlipc.NewTokenStore()
...
func TestTokenStore_LookupUnknownReturnsErr(t *testing.T) {
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, controlipc.ErrUnknownToken))
}

func TestTokenStore_RevokeRemoves(t *testing.T) {
	store := controlipc.NewTokenStore()
	store.Register("t-1", controlipc.TokenInfo{UserID: userid.MustNew("u-1")})
...
func TestTokenStore_DistinctTokensResolveDistinctly(t *testing.T) {
	s := controlipc.NewTokenStore()
	tokA := "A" + strings.Repeat("X", 32) + "A"
	tokB := "B" + strings.Repeat("X", 32) + "B"
	infoA := controlipc.TokenInfo{UserID: userid.MustNew("user-A")}
	infoB := controlipc.TokenInfo{UserID: userid.MustNew("user-B")}
