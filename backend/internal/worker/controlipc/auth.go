// Package controlipc implements the worker's local-IPC ConnectRPC server
...
type TokenInfo struct {
	...
	// Context snapshot baked into LEAPMUX_CONTROL_* env vars so the
	...
	// identity that IS baked in (emitted as LEAPMUX_CONTROL_USER_ID): it
	...
	// resolver (see cli/control/resolve.Resolve): the tenant is implied by
	// the authenticated session, so no hub RPC takes one. It is shipped
	// because the LEAPMUX_CONTROL_*_ID env contract is documented surface
	// that agent scripts read to learn who they are running as.
	WorkingDir    string
	AgentProvider string // Empty for terminals.
}

// TokenStore maps LEAPMUX_CONTROL_TOKEN values to their TokenInfo. The
...
// in the spawned process's environment as LEAPMUX_CONTROL_TOKEN. We hash
...
// registered.
var ErrUnknownToken = errors.New("controlipc: unknown token")
