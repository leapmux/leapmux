...
// yields "". Used to derive a concise registry title from a multi-line prompt.
...
func (a *ClaudeCodeAgent) handleClaudeTaskNotification(ev *claudeTaskEnvelope) {
	...
	a.mu.Lock()
	if tuid, ok := a.taskToolUse[ev.TaskID]; ok {
		delete(a.taskToolUse, ev.TaskID)
...
func (a *ClaudeCodeAgent) routeSubagentMessage(content []byte, msgType string, env *messageEnvelope) {
	...
	childID, err := a.sink.EnsureChildAgent(spawnSpanID, taskID, title)
	if err != nil {
...
// taskTitle returns the description recorded at task_started for a task_id, for
// seeding the child tab title. Returns "" when unknown.
func (a *ClaudeCodeAgent) taskTitle(taskID string) string {
	// The title is carried by the EnsureChildAgent call at task_started; nothing
	// extra to look up here. Kept as a hook for future enrichment.
	return ""
}
