...
func (a *CursorCLIAgent) handleCursorExtension(method string, params json.RawMessage) bool {
	switch method {
...
func cursorExtensionSupplement(original []byte, method string, params json.RawMessage) ([]byte, error) {
	...
	supplement := newACPToolSupplement(frame)
...
//
// It DELEGATES to the embedded acpProvider for every other message. Cursor speaks ACP,
...
func (p cursorProvider) ExtractTodoEvent(spanType string, content []byte, paired func() []byte) (todoevents.Event, bool) {
	...
	if !ok {
		return p.acpProvider.ExtractTodoEvent(spanType, content, paired)
	}
...
//
// It DELEGATES to the embedded acpProvider and then adds Cursor's own key, so an ACP
...
func (p cursorProvider) ResolveProviderData(content MessageContent) []byte {
	resolved := p.acpProvider.ResolveProviderData(content)
