...
const (
	// openCodeQuestionToolEnv turns ON the daemon's own question tool, and
	// kiloQuestionToolEnv is Kilo's spelling of the same flag.
	//
	...
	kiloQuestionToolEnv     = "KILO_ENABLE_QUESTION_TOOL"
	...
	openCodeEventRoute   = "/event"
	openCodeQuestionRoot = "/question"
...
// openCodeACPArgs builds the daemon's arguments with its own server enabled.
//
...
func (q *openCodeQuestions) servesQuestions(ctx context.Context, base string) bool {
	...
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, base+openCodeQuestionRoot, nil)
	if err != nil {
		return false
	}
...
func (q *openCodeQuestions) configure(sink ControlServices) {
	q.sink = sink
...
func (q *openCodeQuestions) consume(ctx context.Context) error {
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, q.base()+openCodeEventRoute, nil)
	if err != nil {
		return err
	}
...
func (q *openCodeQuestions) publishPending(ctx context.Context) {
	...
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, q.base()+openCodeQuestionRoot, nil)
	if err != nil {
		return
	}
	response, err := q.client.Do(request)
...
func (q *openCodeQuestions) publish(ctx context.Context, properties json.RawMessage) string {
	...
	q.mu.Unlock()
	if err := q.sink.PublishControlRequest(ControlRequest{
		RequestID:      requestID,
		Payload:        payload,
...
func (q *openCodeQuestions) answer(ctx context.Context, raw []byte) (bool, error) {
	...
	_, requestID, ok := ExtractJSONRPCID(raw)
	if !ok || requestID == "" {
		return false, nil
	}
	q.mu.Lock()
...
func (q *openCodeQuestions) post(ctx context.Context, questionID, action string, payload any) error {
	...
	url := q.base() + openCodeQuestionRoot + "/" + questionID + "/" + action
