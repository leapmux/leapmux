...
func (s *enrichHookSink) EnrichMessage(change MessageEnrichment) (bool, error) {
	if s.hook != nil && s.fired.CompareAndSwap(false, true) {
		s.hook()
	}
	return s.testSink.EnrichMessage(change)
}
...
func newHookedToolTranscript(t *testing.T, record []byte) (*toolTranscript, *enrichHookSink) {
	t.Helper()
	sink := &enrichHookSink{testSink: &testSink{}}
	...
	return newToolTranscript(t.Context(), sink, source), sink
}
...
func persistClosingToolRow(t *testing.T, transcript *toolTranscript, original []byte) {
	t.Helper()
	require.NoError(t, transcript.PersistMessage(leapmuxv1.MessageSource_MESSAGE_SOURCE_AGENT,
		MessageContent{Original: original}, SpanInfo{SpanID: "call", Closing: true}))
}
...
func TestEnrichToolSpanRefusesAWriteARacingRevisionOvertook(t *testing.T) {
	...
	sink.hook = func() {
		...
		written, err := sink.testSink.EnrichMessage(MessageEnrichment{
