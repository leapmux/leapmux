...
func reasonixDefaultToolStorePath(q StoredSessionQuery, sessionID, workingDir string) string {
...
// the session.
func reasonixToolStorePath(q StoredSessionQuery, sessionID, workingDir string) string {
	if !reasonixSessionIDIsSafe(sessionID) {
		return ""
	}
	roots := reasonixSessionRoots(q, workingDir)
...
func newReasonixToolTranscript(ctx context.Context, services ProviderServices, sessionID func() string, query StoredSessionQuery, workingDir string) *toolTranscript {
...
func (r *reasonixToolSource) locate(string) toolTranscriptLocation {
	id := r.sessionID()
	path := reasonixDefaultToolStorePath(r.query, id, r.workingDir)
	return toolTranscriptLocation{sessionKey: id, path: path, ready: path != ""}
}

func (r *reasonixToolSource) toolCallID(original []byte) string { return acpToolCallID(original) }
...
// file.
func (r *reasonixToolSource) readSupplements(ctx context.Context, _ string, pending map[string]MessageContent, _ bool) (map[string][]byte, error) {
...
func readReasonixToolSupplements(ctx context.Context, path string, pending map[string]MessageContent, cache *reasonixEventCache) (map[string][]byte, error) {
