...
func newOpenCodeAgentWithSink(sink agent.ProviderServices) *Agent {
	a := &Agent{
		Base: acp.Base{
...
func TestHandleOpenCodeOutput_AgentThoughtChunk(t *testing.T) {
	...
	agent.HandlePromptResponseForTest(resp)
...
func TestHandleOpenCodeOutput_AgentThoughtChunk_DoesNotInferAParagraph(t *testing.T) {
	...
	agent.HandlePromptResponseForTest(json.RawMessage(`{"stopReason":"end_turn"}`))
...
func TestHandleOpenCodeOutput_TrailingThoughtFlushedBeforeReply(t *testing.T) {
	...
	agent.TurnAssistantTextForTest().WriteString("Here is the answer.")
	agent.HandlePromptResponseForTest(json.RawMessage(`{"stopReason":"end_turn"}`))
...
func TestHandleOpenCodePromptResponse_PersistsAssistantText(t *testing.T) {
	...
	agent.Mu.Lock()
	require.Equal(t, "", agent.TurnAssistantTextForTest().String())
	agent.Mu.Unlock()
}
...
func TestHandleOpenCodeOutput_ToolCallUpdateInProgress(t *testing.T) {
	...
	ag.HandleOutput([]byte(statusOnly))
	...
	ag.HandleOutput([]byte(first))
	ag.HandleOutput([]byte(second))
	...
	require.Equal(t, agent.OutputTailProgress("tc-1", "line1\n", false), updates[1])
...
func TestHandleOpenCodeOutput_ToolCallUpdateCompleted(t *testing.T) {
	...
	assert.Contains(t, sink.ProgressUpdates(), agent.CompleteOutputProgress("tc-1"))
...
func TestHandlePromptResponse_WrappedFormat(t *testing.T) {
	...
	sink := &agenttest.Sink{}
	agent := newOpenCodeAgentWithSink(agent.NewProviderServices(sink))
	agent.SetTurnToolUsesForTest(2)
	...
	assert.Equal(t, 0, agent.TurnToolUsesForTest())
}
...
func TestHandleOpenCodeOutput_ToolCallUpdateCompletedIncrementsToolUses(t *testing.T) {
	...
	agent.Mu.Lock()
	count := agent.TurnToolUsesForTest()
	agent.Mu.Unlock()
...
func acpMessageChunk(text string) []byte {
	return acptest.Chunk("test-session", "agent_message_chunk", text)
}

func acpThoughtChunk(text string) []byte {
	return acptest.Chunk("test-session", "agent_thought_chunk", text)
}
...
func TestHandleACPOutput_ThoughtChunkAccumulatesThinkingTokens(t *testing.T) {
	...
	assert.Equal(t, []interface{}{int64(2), int64(4)}, sink.SessionInfoValues("thinking_tokens"),
...
func TestHandleACPOutput_AssistantTextAfterReasoningResetsViaFlush(t *testing.T) {
	...
	assert.Equal(t, int64(2), sink.LastThinkingTokens(), "assistant text restarts from zero after the reasoning flush")
}
...
func TestHandleACPOutput_ToolCallResetsThinkingTokens(t *testing.T) {
	...
	for _, src := range []struct {
		...
		t.Run(src.name, func(t *testing.T) {
			...
			assert.Equal(t, int64(2), sink.LastThinkingTokens(), "the next phase restarts at 8/4")
		})
	}
}
...
func TestHandleACPOutput_TurnEndResetsThinkingTokens(t *testing.T) {
	...
	agent.HandleOutput(acpMessageChunk("abcdefgh"))
	assert.Equal(t, int64(2), sink.LastThinkingTokens(), "the next turn restarts the estimate")
}
...
func TestHandleACPOutput_ReasoningAfterAssistantTextStartsFreshPhase(t *testing.T) {
	...
	assert.Equal(t, []interface{}{int64(4), int64(0), int64(2)}, sink.SessionInfoValues("thinking_tokens"),
...
func TestHandleACPOutput_NilResultResetsThinkingTokens(t *testing.T) {
	...
	agent.HandlePromptResponseForTest(nil)
	assert.Equal(t, int64(0), sink.LastThinkingTokens(), "the abort broadcasts an explicit clear")

	agent.HandleOutput(acpMessageChunk("abcdefgh"))
	assert.Equal(t, int64(2), sink.LastThinkingTokens(), "a nil-result turn end restarts the estimate")
	assert.Equal(t, []interface{}{int64(4), int64(0), int64(2)}, sink.SessionInfoValues("thinking_tokens"),
...
func TestHandleACPOutput_NilResultDropsBufferedAssistantText(t *testing.T) {
	...
	ag.HandleOutput(acpMessageChunk("STALE-TURN-1"))
	ag.HandlePromptResponseForTest(nil)

	// Turn 2 streams its own assistant text and ends normally.
	ag.HandleOutput(acpMessageChunk("turn-2-text"))
	ag.HandlePromptResponseForTest(json.RawMessage(`{"stopReason":"end_turn"}`))

	// The aborted turn keeps its own partial text, marked interrupted.
	assert.Equal(t, "STALE-TURN-1", persistedACPAssistantText(t, sink, agent.MessageCompletionInterrupted),
	...
	assert.Equal(t, "turn-2-text", persistedACPAssistantText(t, sink, agent.MessageCompletionComplete),
...
func TestHandleACPPrompt_FailedRequestMarksTheBufferedTextFailed(t *testing.T) {
	...
	ag.HandleOutput(acpThoughtChunk("weighing the two call sites"))
	ag.HandleOutput(acpMessageChunk("The change is safe because"))
	ag.FinishPromptRequestForTest("test-session", nil, errors.New("transport closed"))

	assert.Equal(t, "The change is safe because", persistedACPAssistantText(t, sink, agent.MessageCompletionError),
	...
	assert.Equal(t, "weighing the two call sites",
		persistedACPText(t, sink, contracts.AssembledMessageKindReasoning, agent.MessageCompletionComplete),
...
func TestHandleACPOutput_NilResultPersistsInterruptedToolOutput(t *testing.T) {
	...
	ag.HandleOutput([]byte(`{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"test-session","update":{"sessionUpdate":"tool_call","toolCallId":"tc-1","title":"command","kind":"execute","status":"pending"}}}`))
	ag.HandleOutput([]byte(`{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"test-session","update":{"sessionUpdate":"tool_call_update","toolCallId":"tc-1","status":"in_progress","content":[{"type":"content","content":{"type":"text","text":"partial output"}}]}}}`))
	ag.HandlePromptResponseForTest(nil)
...
func TestHandleACPOutput_NilResultClosesAToolWithoutAnUpdate(t *testing.T) {
	...
	a.HandleOutput([]byte(`{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"test-session","update":{"sessionUpdate":"tool_call","toolCallId":"tc-1","title":"command","kind":"execute","status":"pending","rawInput":{"command":"printf partial"}}}}`))
	a.HandlePromptResponseForTest(nil)
...
func TestHandleACPOutput_CompletedPromptClosesAToolWithoutAFinalUpdate(t *testing.T) {
	...
	a.HandleOutput([]byte(`{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"test-session","update":{"sessionUpdate":"tool_call","toolCallId":"tc-1","title":"command","kind":"execute","status":"pending"}}}`))
	a.HandlePromptResponseForTest(json.RawMessage(`{"stopReason":"end_turn"}`))
...
func TestHandleACPOutput_FinalToolCallClosesItsEarlierSpan(t *testing.T) {
	...
	assert.Equal(t, 1, agent.TurnToolUsesForTest())
	agent.HandlePromptResponseForTest(nil)
...
func persistedACPAssistantText(t *testing.T, sink *agenttest.Sink, completion agent.MessageCompletion) string {
	t.Helper()
...
// separate the rows.
func persistedACPText(t *testing.T, sink *agenttest.Sink, kind string, completion agent.MessageCompletion) string {
	t.Helper()
	var found []string
	for _, m := range sink.Messages() {
		...
		if agent.MessageCompletion(parsed.Completion) == completion {
...
func TestHandleACPOutput_PermissionRequestResetsThinkingTokens(t *testing.T) {
	...
	agent.HandleOutput(acpMessageChunk("abcdefgh"))
	assert.Equal(t, int64(2), sink.LastThinkingTokens(), "a permission prompt restarts the estimate")
}
...
func TestHandleACPOutput_UnknownMethodResetsThinkingTokens(t *testing.T) {
	...
	agent.HandleOutput(acpMessageChunk("abcdefgh"))
	assert.Equal(t, int64(2), sink.LastThinkingTokens(), "an unknown AGENT-message method restarts the estimate")
}
...
func TestOpenCodeControlRequestsKeepNumericAndStringIdentitiesSeparate(t *testing.T) {
	sink := &agenttest.ControlSink{}
	agenttest.AssertControlIdentitiesStaySeparate(t, sink, newOpenCodeAgentWithSink(agent.NewProviderServices(sink)).HandleOutput,
