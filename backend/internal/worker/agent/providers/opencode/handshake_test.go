...
func TestHelperProcessOpenCodeACP(t *testing.T) {
	...
	for scanner.Scan() {
		...
		case acp.MethodSessionSetModel:
			...
			writeResult(req.ID, `{}`)
		case acp.MethodSessionSetMode, acp.MethodSessionPrompt:
			writeResult(req.ID, `{}`)
		}
	}
	os.Exit(0)
...
func TestStartOpenCode_NewSessionHandshakeReadsConfigOptionModels(t *testing.T) {
	...
	assert.Equal(t, "opencode-new", a.SessionIDForTest())
	...
	assert.Equal(t, "Claude Sonnet 4", a.AvailableModelsForTest()[0].DisplayName)
...
func TestStartOpenCode_HandshakeSurfacesGenericConfigOption(t *testing.T) {
	...
	current := agent.CurrentOptions(groups)
	assert.Equal(t, "high", current["thoughtLevel"])
	assert.Equal(t, PrimaryAgentBuild, current[agent.OptionIDPrimaryAgent])
}
...
func TestStartOpenCode_RejectedModelIsNonFatal(t *testing.T) {
	...
	provider, err := Start(context.Background(), agent.Options{
		...
		WorkingDir:    t.TempDir(),
		Options:       map[string]string{agent.OptionIDModel: "made-up/model"},
		Shell:         testutil.TestShell(),
		LoginShell:    false,
		AgentProvider: leapmuxv1.AgentProvider_AGENT_PROVIDER_OPENCODE,
	}, agent.NewProviderServices(&agenttest.Sink{}))
	...
	assert.Equal(t, "anthropic/claude-sonnet-4", agent.ModelForTest())
...
func TestStartOpenCode_ModelChangeSurfacesEffort(t *testing.T) {
	...
	require.True(t, ag.UpdateSettings(optionmap.Map{agent.OptionIDModel: "openai/gpt-5.5"}).AppliedLive)
	...
	assert.Equal(t, "medium", agent.CurrentOptions(groups)[agent.OptionIDEffort])
