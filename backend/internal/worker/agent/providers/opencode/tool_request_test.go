...
func TestACPToolRequestReceivesLateInputBeforeCompletion(t *testing.T) {
	...
	agent.HandleToolCallForTest(json.RawMessage(`{"sessionUpdate":"tool_call","toolCallId":"command","title":"bash","kind":"execute","status":"pending","rawInput":{}}`))
	agent.HandleToolCallUpdateForTest(json.RawMessage(`{"sessionUpdate":"tool_call_update","toolCallId":"command","title":"Run checks","rawInput":{"command":"npm test"},"status":"in_progress"}`))
	...
	agent.HandleToolCallUpdateForTest(originalResult)
...
func TestACPToolResultPreservesOriginalTerminalReference(t *testing.T) {
	...
	sink := &agenttest.Sink{}
	agent := newOpenCodeAgentWithSink(agent.NewProviderServices(sink))
	agent.HandleToolCallForTest(json.RawMessage(`{"sessionUpdate":"tool_call","toolCallId":"command","kind":"execute","rawInput":{"command":"printf output"}}`))
	exitCode := 0
	agent.SetCompletedTerminalsForTest(map[string]contracts.ACPTerminalResult{"terminal": {Output: "output", ExitCode: &exitCode}})
	...
	agent.HandleToolCallUpdateForTest(original)
	require.Len(t, sink.Messages(), 2)
