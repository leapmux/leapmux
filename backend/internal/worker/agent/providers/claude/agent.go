...
// output from Claude.
func Start(ctx context.Context, opts agent.Options, sink agent.ProviderServices) (*Agent, error) {
	...
	launchSpec, err := providerkit.ResolveLaunch(ctx, opts, leapmuxv1.AgentProvider_AGENT_PROVIDER_CLAUDE_CODE, claudeLocator)
	if err != nil {
		cancel()
		return nil, err
	}
