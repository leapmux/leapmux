...
// agent_test.go's `//go:build unix` so they run on every platform. Nothing
...
func TestClaudeCodePermissionModeOptions_AllDescriptionsPopulated(t *testing.T) {
	...
	for _, g := range groups {
		if g.GetId() == agent.OptionIDPermissionMode {
			permGroup = g
			break
		}
	}
...
func TestFilterPermissionModeGroup_AutoAvailable(t *testing.T) {
	staticGroup := Registration().OptionGroups[0]
...
func TestLivePermissionModeGroup_EmptyCurrentFallsBackToDefault(t *testing.T) {
	...
	// blank selection (delegated to providerkit.LiveGroup's empty-current fallback). The default is
...
func TestClaudeCodeAgent_AvailableOptionGroupsFiltersAutoWhenUnavailable(t *testing.T) {
	a := &Agent{
		Process:           providerkit.NewProcessFrom(providerkit.ProcessConfig{AgentID: "t"}),
	...
	permGroup := optionids.GroupByID(groups, agent.OptionIDPermissionMode)
...
func TestLaunchOmitsEffort(t *testing.T) {
	...
	assert.True(t, static.launchOmitsEffort("opus", agent.EffortAuto), "auto omits --effort")
...
// tolerate nil entries (matching FindAvailableModel/agent.ModelOptionGroup), so a
// nil-bearing catalog can't panic the effort/ultracode lookups.
func TestEffortResolver_NilGuard(t *testing.T) {
	r := newEffortResolver([]*agent.ModelInfo{nil, {Id: "opus", SupportedEfforts: claudeEffortXHighMax}, nil})
...
func TestEffortResolver_StaticFallbackTrustsFilteredModel(t *testing.T) {
	...
	filtered := &Agent{availableModels: convertClaudeModels([]claudeCodeModelInfo{
	...
	dropped := &Agent{availableModels: convertClaudeModels([]claudeCodeModelInfo{
...
func TestEffortResolver_ContextWindowFallsBackToStatic(t *testing.T) {
	...
	r := (&Agent{availableModels: convertClaudeModels([]claudeCodeModelInfo{
	...
	assert.Equal(t, int64(0), r.contextWindow(agent.DefaultModelSentinel),
...
func TestEffortResolver_SentinelIsUnresolved(t *testing.T) {
	r := (&Agent{}).effortResolver() // static fallback only
	_, known := r.definedEfforts(agent.DefaultModelSentinel)
	assert.False(t, known, "the sentinel is reported as unresolved, not known-with-empty-efforts")
	assert.Equal(t, "max", r.resolveEffort(agent.DefaultModelSentinel, "max"), "effort passes through, not clamped to high")
	assert.False(t, r.unsupportedUltracode("max", agent.DefaultModelSentinel))
	...
	assert.Nil(t, r.updateFlagSettings(agent.DefaultModelSentinel, "max", "high"))
}
...
func TestEffortResolver_EmptyDynamicEntryDefersToFallback(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			dynSonnet := agent.FindAvailableModel(dyn, "sonnet")
			require.NotNil(t, dynSonnet)
			require.Empty(t, dynSonnet.SupportedEfforts)

			r := (&Agent{availableModels: dyn}).effortResolver()
...
func TestEnsureSettledModelListed_InjectsResolvedDefault(t *testing.T) {
	...
	require.Nil(t, agent.FindAvailableModel(a.availableModels, "opus[1m]"),
	...
	assert.NotNil(t, agent.FindAvailableModel(a.availableModels, agent.DefaultModelSentinel))
	assert.NotNil(t, agent.FindAvailableModel(a.availableModels, "sonnet[1m]"))
	...
	assert.Equal(t, []string{agent.DefaultModelSentinel, "fable[1m]", "opus[1m]", "sonnet", "sonnet[1m]", "haiku"}, ids,
...
func TestEnsureSettledModelListed_AppendsLowestRankAtEnd(t *testing.T) {
	...
	assert.Equal(t, []string{agent.DefaultModelSentinel, "fable[1m]", "opus[1m]"}, ids,
...
func TestEnsureSettledModelListed_NoOps(t *testing.T) {
	t.Run("already listed", func(t *testing.T) {
		a := &Agent{model: "sonnet", availableModels: convertClaudeModels(realCLIModelsNoOpus(), nil)}
	...
	for _, tc := range []struct{ name, model string }{
		{"empty model", ""},
		{"unresolved sentinel", agent.DefaultModelSentinel},
	} {
		t.Run(tc.name, func(t *testing.T) {
			a := &Agent{model: tc.model, availableModels: convertClaudeModels(realCLIModelsNoOpus(), nil)}
			n := len(a.availableModels)
			a.ensureSettledModelListed()
	...
	t.Run("empty dynamic list (old CLI)", func(t *testing.T) {
		a := &Agent{model: "opus[1m]", availableModels: nil}
		a.ensureSettledModelListed()
...
func TestEnsureSettledModelListed_UnknownModelNotInjected(t *testing.T) {
	...
	assert.Nil(t, agent.FindAvailableModel(a.availableModels, "mystery[1m]"),
...
func claudeEffortIDs(m *agent.ModelInfo) []string {
	return claudeEffortIDsFromList(m.SupportedEfforts)
}

func claudeEffortIDsFromList(efforts []*agent.EffortInfo) []string {
...
func TestConvertClaudeModels_ReproducesStaticCatalog(t *testing.T) {
	...
	for _, want := range claudeCodeAvailableModels {
		...
		if want.Id == agent.DefaultModelSentinel {
			continue
		}
...
func TestClaudeAvailableModels_DynamicFirstStaticFallback(t *testing.T) {
	...
	thirdParty := &Agent{thirdPartyFromSettings: true, availableModels: dynamic}
...
func TestBuildStartupFlagSettings_DynamicCatalogUltracode(t *testing.T) {
	...
	a := &Agent{model: "mythos", effort: EffortUltracode, availableModels: dynamic}
...
func TestBuildStartupFlagSettings_DowngradesWhenDynamicDropsXHigh(t *testing.T) {
	...
	a := &Agent{model: "opus[1m]", effort: EffortUltracode, availableModels: dynamic}
	fs := a.buildStartupFlagSettings(nil)
...
func TestBuildStartupFlagSettings_SentinelEmitsNoEffort(t *testing.T) {
	a := &Agent{model: agent.DefaultModelSentinel, effort: EffortUltracode}
	fs := a.buildStartupFlagSettings(nil)
...
func TestBuildStartupFlagSettings_FilteredModelHonorsLaunchVerdict(t *testing.T) {
	...
	opus := &Agent{model: "opus[1m]", effort: EffortUltracode, availableModels: dynamic}
	fs := opus.buildStartupFlagSettings(nil)
	assert.Equal(t, agent.EffortXHigh, fs["effortLevel"], "filtered ultracode model completes the combo (xhigh base)")
	...
	sonnet := &Agent{model: "sonnet", effort: "max", availableModels: dynamic}
	...
	unknown := &Agent{model: "claude-future-preview", effort: EffortUltracode, availableModels: dynamic}
...
func TestBuildStartupFlagSettings_ThirdPartyEmitsNoEffort(t *testing.T) {
	...
	a := &Agent{model: "opus[1m]", effort: EffortUltracode, availableModels: dynamic, thirdPartyFromSettings: true}
	fs := a.buildStartupFlagSettings(nil)
	_, hasLevel := fs["effortLevel"]
	_, hasUltra := fs["ultracode"]
...
func TestBuildStartupFlagSettings_AppliesEffortWhenDynamicAddsEfforts(t *testing.T) {
	...
	a := &Agent{model: "haiku", effort: agent.EffortHigh, availableModels: dynamic}
	fs := a.buildStartupFlagSettings(nil)
	assert.Equal(t, agent.EffortHigh, fs["effortLevel"], "launch omitted --effort (static effort-less); startup applies the dynamic level")
	_, hasUltra := fs["ultracode"]
	...
	autoAgent := &Agent{model: "haiku", effort: agent.EffortAuto, availableModels: dynamic}
...
func TestModelGroupDefault_ClaudeDefaultSentinel(t *testing.T) {
	...
	withSentinel := []*agent.ModelInfo{
		{Id: "default", DisplayName: "Default (recommended)"},
		{Id: "claude-fable-5[1m]", DisplayName: "Fable"},
		{Id: "sonnet", DisplayName: "Sonnet"},
	}
	assert.Equal(t, "default", agenttest.DerivedModelDefault(t, claudeTestRegistry, withSentinel, claude))

	// The static catalog carries the sentinel, so it is marked there too.
	assert.Equal(t, "default", Registration().DefaultModel())
	assert.Equal(t, "default", agenttest.DerivedModelDefault(t, claudeTestRegistry, claudeCodeAvailableModels, claude))
	...
	noSentinel := []*agent.ModelInfo{
		{Id: "opus[1m]", DisplayName: "Opus (1M context)"},
		{Id: "sonnet", DisplayName: "Sonnet"},
	}
	assert.Equal(t, "opus[1m]", agenttest.DerivedModelDefault(t, claudeTestRegistry, noSentinel, claude))

	// Operator override wins over the CLI sentinel.
	t.Setenv("LEAPMUX_CLAUDE_DEFAULT_MODEL", "sonnet")
	assert.Equal(t, "sonnet", agenttest.DerivedModelDefault(t, claudeTestRegistry, withSentinel, claude))
}
...
func TestUpdateSettings_SwitchToDefaultRestarts(t *testing.T) {
	a := &Agent{model: "claude-fable-5[1m]", effort: agent.EffortHigh}
	assert.False(t, a.UpdateSettings(map[string]string{agent.OptionIDModel: agent.DefaultModelSentinel}).AppliedLive)
}
...
func TestClaudeOptionGroups_HiddenModelEffortUISurfacesReadOnly(t *testing.T) {
	...
	assert.Equal(t, agent.EffortHigh, eg.GetCurrentValue())
}
...
func TestClaudeOptionGroups_HiddenUIAutoEffortOmitsEffortGroup(t *testing.T) {
	a := &Agent{thirdPartyFromSettings: true, model: "tp", effort: agent.EffortAuto}

	groups := a.OptionGroups()

	assert.NotNil(t, optionids.GroupByID(groups, agent.OptionIDModel))
	assert.Nil(t, optionids.GroupByID(groups, agent.OptionIDEffort), "auto effort is not surfaced")
}
...
func TestClaudeCatalogCarriesTheAccountDefaultEntry(t *testing.T) {
	t.Parallel()

	row := agent.FindAvailableModel(claudeCodeAvailableModels, agent.DefaultModelSentinel)
