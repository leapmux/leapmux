...
func TestAgent_StartTimeoutCleansUpProcess(t *testing.T) {
	...
	startUnresponsive := func(ctx context.Context, opts agent.Options, sink agent.ProviderServices) (*Agent, error) {
		...
		mode := cmp.Or(opts.PermissionMode(), contracts.ClaudeModeDefault)
		requestID := id.Short()
		...
		case <-ctx2.Done():
			...
			cleanup()
			return nil, ctx2.Err()
		case <-time.After(opts.EffectiveStartupTimeout()):
			a.unregisterPendingControl(requestID)
			cleanup()
...
func TestAgent_EarlyExitDetected(t *testing.T) {
	...
	startEarlyExit := func(ctx context.Context, opts agent.Options, sink agent.ProviderServices) (*Agent, error) {
		...
		cmd.Stderr = a.StderrWriterForTest()
		a.SkipStderr() // stderr is captured synchronously via cmd.Stderr
		...
		if _, err := a.sendControlAndWait(ctx2, `{"subtype":"initialize"}`, opts.EffectiveStartupTimeout()); err != nil {
			cleanup()
...
func TestAgent_PreambleSkipping(t *testing.T) {
	...
	// Drain stderr in background. The drain closes stderrDone when the pipe ends.
