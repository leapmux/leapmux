...
// claudeMetaCanChangeModelAndEffort is the preamble metadata key through which the
// shell wrapper reports whether the user's shell sets up a third-party provider:
// "false" when it does, so the session hides the model/effort UI.
const claudeMetaCanChangeModelAndEffort = "can_change_model_and_effort"

// claudeModelEffortGate gates Claude's --model/--effort arguments on the environment
// the user's shell sets up. detectThirdPartyProvider reads only the settings files
// and the worker's own environment, and a provider exported by a shell profile is
// visible to neither. So the check runs inside the shell, and a default-model launch
// with no arguments to send still asks for it: its answer is what decides whether
// the model/effort UI shows.
func claudeModelEffortGate(modelEffortArgs []string) *launch.EnvGatedArgs {
	return &launch.EnvGatedArgs{
		EnvVars: thirdPartyProviderEnvVars,
		MetaKey: claudeMetaCanChangeModelAndEffort,
		Args:    modelEffortArgs,
	}
}
