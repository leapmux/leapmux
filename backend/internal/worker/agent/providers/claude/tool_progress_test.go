...
// the same flags Start passes (--output-format stream-json --verbose),
...
func runningToolUpdate(t *testing.T, sink *outputTestSink) map[string]interface{} {
	t.Helper()
	v, ok := sink.LastSessionInfoValue(contracts.SessionInfoKeyRunningTool)
...
func TestHandleOutput_ToolProgressHeartbeatsRaiseTheElapsedTime(t *testing.T) {
	...
	broadcasts := sink.SessionInfoCount()
...
func TestHandleOutput_ToolProgressKeepsParallelToolsApart(t *testing.T) {
	...
	first := infos[0][contracts.SessionInfoKeyRunningTool].(map[string]interface{})
	second := infos[1][contracts.SessionInfoKeyRunningTool].(map[string]interface{})
...
func TestHandleOutput_ToolProgressDropsWhatNoCardCanCarry(t *testing.T) {
	...
	for name, line := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			_, ok := sink.LastSessionInfoValue(contracts.SessionInfoKeyRunningTool)
...
// replacement still forwarded, and it is unreachable anyway: Start
