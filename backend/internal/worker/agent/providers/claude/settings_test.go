...
// TestClaudeModelEffortGate pins Claude's half of the shell wrapper's env gate:
// its three third-party variables, the metadata key hidesModelEffortUI reads, and
// the launch arguments it withholds when one of the variables is set.
func TestClaudeModelEffortGate(t *testing.T) {
	gate := claudeModelEffortGate([]string{"--model", "opus"})
	assert.Equal(t, []string{"CLAUDE_CODE_USE_BEDROCK", "CLAUDE_CODE_USE_FOUNDRY", "CLAUDE_CODE_USE_VERTEX"}, gate.EnvVars)
	assert.Equal(t, "can_change_model_and_effort", gate.MetaKey)
	assert.Equal(t, []string{"--model", "opus"}, gate.Args)

	cmd, _, metaPrefix := launch.Wrap(context.Background(), launch.WrapSpec{
		Shell:      "/bin/bash",
		Launch:     launch.Spec{Program: "claude"},
		EnvGated:   gate,
		WorkingDir: "/tmp",
	})
	for _, v := range gate.EnvVars {
		assert.Contains(t, cmd.Args[2], `[ -n "$`+v+`" ]`)
	}
	assert.Contains(t, cmd.Args[2], metaPrefix+"can_change_model_and_effort=false")
	assert.Contains(t, cmd.Args[2], metaPrefix+"can_change_model_and_effort=true")
}
