...
func TestAvailableOptionGroups_IncludesOutputStyles(t *testing.T) {
	...
	styleGroup := optionids.GroupByID(groups, OptionOutputStyle)
...
func TestAvailableOptionGroups_NoOutputStylesWhenEmpty(t *testing.T) {
	...
	groups := a.OptionGroups()
	assert.Nil(t, optionids.GroupByID(groups, OptionOutputStyle), "should not have output style group when empty")
}
...
func TestAvailableOptionGroups_FastModeOn(t *testing.T) {
	...
	fastGroup := optionids.GroupByID(groups, OptionFastMode)
...
func TestAvailableOptionGroups_AlwaysIncludesThinking(t *testing.T) {
	...
	thinkingGroup := optionids.GroupByID(groups, OptionAlwaysThinking)
...
func TestAvailableOptionGroups_ThinkingLabelsByModel(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.model+"/"+tc.current, func(t *testing.T) {
			a := &Agent{model: tc.model, alwaysThinking: tc.current}
			groups := a.OptionGroups()

			g := optionids.GroupByID(groups, OptionAlwaysThinking)
			require.NotNil(t, g)
...
// against synthetic level sets in agent_test.go's convertClaudeModels cases.
func TestOptionGroups_ModelOptionsCarrySubGroups(t *testing.T) {
	t.Parallel()

	a := &Agent{model: "sonnet", alwaysThinking: AlwaysThinkingOn}
	groups := a.OptionGroups()
	...
	opusEffort := subGroup("opus[1m]", agent.OptionIDEffort)
	require.NotNil(t, opusEffort)
	assert.Contains(t, effortIDs(opusEffort), agent.EffortXHigh)
	assert.Contains(t, effortIDs(opusEffort), EffortUltracode)
	assert.Equal(t, agent.EffortXHigh, opusEffort.GetDefaultValue())
	opusThinking := subGroup("opus[1m]", OptionAlwaysThinking)
	...
	sonnetEffort := subGroup("sonnet", agent.OptionIDEffort)
	require.NotNil(t, sonnetEffort)
	assert.Contains(t, effortIDs(sonnetEffort), agent.EffortXHigh)
	assert.Contains(t, effortIDs(sonnetEffort), "max")
	assert.Equal(t, agent.EffortXHigh, sonnetEffort.GetDefaultValue())

	// Haiku: no effort group at all; thinking enabled label is "On".
	assert.Nil(t, subGroup("haiku", agent.OptionIDEffort), "Haiku has no effort group")
	haikuThinking := subGroup("haiku", OptionAlwaysThinking)
...
// (both come from launchModel). StartOverCommand mirrors Start's a.model
...
func TestCurrentSettings_IncludesExtraSettings(t *testing.T) {
	...
	a := &Agent{
		Process:                 providerkit.NewProcessFrom(providerkit.ProcessConfig{AgentID: "test"}),
		model:                   "sonnet",
		effort:                  "low",
	...
	opts := agent.CurrentOptions(a.OptionGroups())
	assert.Equal(t, "sonnet", opts[agent.OptionIDModel])
	assert.Equal(t, "low", opts[agent.OptionIDEffort])
	assert.Equal(t, "Explanatory", opts[OptionOutputStyle])
	assert.Equal(t, "on", opts[OptionFastMode])
	assert.Equal(t, "off", opts[OptionAlwaysThinking])
}
...
func TestUpdateSettings_NothingChanged(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{
		agent.OptionIDModel:  a.model,
		agent.OptionIDEffort: a.effort,
		OptionOutputStyle:    a.outputStyle,
		OptionFastMode:       a.fastMode,
		OptionAlwaysThinking: a.alwaysThinking,
	})
...
func TestUpdateSettings_FlagReadbackFailureIsUnresolved(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{OptionOutputStyle: "Explanatory"})
	require.True(t, result.AppliedLive)
	assert.Equal(t, agent.OptionSettlementUnresolved, result.Settlements[OptionOutputStyle].State)
}
...
func TestUpdateSettings_RespelledModelSendsNoModelFlag(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{
		agent.OptionIDModel: "OPUS[1M]", // normalizes to the running "opus[1m]" -- not a real change
		OptionFastMode:      FastModeOn, // a real change, so an apply_flag_settings is sent
	})
	require.True(t, result.AppliedLive)
	...
	for _, settings := range sent {
		...
		assert.Contains(t, settings, OptionFastMode, "the real fast-mode change is sent")
	}
}
...
func TestUpdateSettings_ConfirmedPermissionSupersedesDeferredRequest(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{agent.OptionIDPermissionMode: contracts.ClaudeModePlan})
...
func TestUpdateSettings_AutoSwitchClearsStaleAutoModeAvailable(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{agent.OptionIDPermissionMode: contracts.ClaudeModeAuto})
...
// time out quickly.
func newTestAgentWithDeferredPermissionMode(t *testing.T) *Agent {
	t.Helper()
	a, err := spawnMockClaudeAgent(context.Background(), "TestHelperProcessWithControlProtocol",
...
func TestUpdateSettings_AutoRequiresRestart(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{agent.OptionIDEffort: "auto"})
...
func TestUpdateSettings_AlwaysThinkingOn(t *testing.T) {
	...
	result := a.UpdateSettings(map[string]string{
		agent.OptionIDModel:  a.model,
		agent.OptionIDEffort: a.effort,
		OptionAlwaysThinking: AlwaysThinkingOn,
	})
	assert.True(t, result.AppliedLive)
...
func TestUpdateSettings_ToggleRoundTripTracksConfirmedValue(t *testing.T) {
	...
	t.Run("fast mode off->on->off->on", func(t *testing.T) {
		...
		for _, step := range []struct{ set, want string }{
			...
			require.True(t, a.UpdateSettings(map[string]string{OptionFastMode: step.set}).AppliedLive)
...
func TestHandlePendingControlResponse_ParsesInitializeFields(t *testing.T) {
	...
	line := &providerkit.ParsedLine{Type: "control_response", Raw: raw}
...
func newTestAgentWithControlProtocol(t *testing.T) *Agent {
...
func newTestAgentWithControlProtocolEnv(t *testing.T, extraEnv ...string) *Agent {
	t.Helper()
	a, err := spawnMockClaudeAgent(context.Background(), "TestHelperProcessWithControlProtocol",
