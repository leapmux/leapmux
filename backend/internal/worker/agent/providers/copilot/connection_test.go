...
func startCopilotConnectionForTest(t *testing.T, opts agent.Options, handle providerkit.LineHandler) (*copilotConnection, error) {
	t.Helper()
...
func TestCopilotNativeConnection(t *testing.T) {
	...
	connection, err := startCopilotConnectionForTest(t, agent.Options{
		AgentID: "copilot-native", WorkingDir: t.TempDir(), Shell: testutil.TestShell(),
		APITimeout: time.Second,
	}, func(line *providerkit.ParsedLine) { notifications <- line.Raw })
...
func TestCopilotNativeConnectionRejectsAnUnknownProtocol(t *testing.T) {
	...
	_, err := startCopilotConnectionForTest(t, agent.Options{
		AgentID: "copilot-native", WorkingDir: t.TempDir(), Shell: testutil.TestShell(),
		APITimeout: time.Second,
	}, func(*providerkit.ParsedLine) {})
...
func TestCopilotNativeConnectionDeliversNothingBeforeItsReaderStarts(t *testing.T) {
	...
	connection.startReading(func(line *providerkit.ParsedLine) { delivered <- line.Raw })
