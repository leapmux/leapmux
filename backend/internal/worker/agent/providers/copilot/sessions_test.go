...
// is a valid YAML double-quoted scalar, so agenttest.JSONString states it.
func writeCopilotSession(t *testing.T, home, id, cwd, name, created, updated string) {
	t.Helper()
	agenttest.RequireHostAbsDir(t, cwd)
	body := fmt.Sprintf(
		"id: %s\ncwd: %s\nclient_name: leapmux\nname: %s\nuser_named: false\nsummary_count: 0\ncreated_at: %s\nupdated_at: %s\n",
		id, agenttest.JSONString(cwd), name, created, updated)
	agenttest.WriteFixtureFile(t, filepath.Join(home, ".copilot", "session-state", id, "workspace.yaml"), body)
}
...
func TestCopilotStoredSessions(t *testing.T) {
	...
	writeCopilotSession(t, home, "sess-elsewhere", testutil.NativeAbsPath("/other/dir"), "Not mine", "2026-08-29T10:00:00.000Z", "2026-08-29T11:00:00.000Z")

	got, err := copilotStoredSessions(context.Background(), agent.StoredSessionQuery{
...
func TestCopilotStoredSessions_FallsBackWhenTimesAreMissing(t *testing.T) {
	...
	agenttest.TouchFixture(t, filepath.Join(home, ".copilot", "session-state", "no-times", "workspace.yaml"),
...
func TestCopilotStoredSessions_OrdersByTheSidecarNotTheDirectory(t *testing.T) {
	...
	agenttest.TouchFixture(t, filepath.Join(stateDir, "sess-recent", "workspace.yaml"),
		time.Date(2026, 8, 28, 12, 0, 0, 0, time.UTC))
	agenttest.TouchFixture(t, filepath.Join(stateDir, "sess-stale", "workspace.yaml"),
		time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC))
	// The DIRECTORY times say the opposite, which is what a migration produces.
	agenttest.TouchFixture(t, filepath.Join(stateDir, "sess-recent"), time.Date(2026, 8, 20, 9, 0, 0, 0, time.UTC))
	agenttest.TouchFixture(t, filepath.Join(stateDir, "sess-stale"), time.Date(2026, 8, 30, 9, 0, 0, 0, time.UTC))

	got, err := copilotStoredSessions(context.Background(), agent.StoredSessionQuery{
		WorkingDir: dir, HomeDir: home, Getenv: agenttest.FixtureEnv(nil), Limit: 1,
	})
	require.NoError(t, err)
	assert.Equal(t, []string{"sess-recent"}, agenttest.Handles(got),
...
func TestCopilotStoredSessions_SkipsUnreadableSidecars(t *testing.T) {
	...
	agenttest.WriteFixtureFile(t, filepath.Join(root, "no-sidecar", "events.jsonl"), "{}\n")
	// A sidecar that is not YAML.
	agenttest.WriteFixtureFile(t, filepath.Join(root, "broken", "workspace.yaml"), "\tid: [unclosed\n  cwd:::\n")
...
// one time, sessionstore.SortAndCapEntries then breaks that tie by NAME, and the cut keeps a
...
func TestCopilotStoredSessions_RespectsTheLimit(t *testing.T) {
	...
	for i := range 6 {
		...
		agenttest.TouchFixture(t, filepath.Join(home, ".copilot", "session-state", id, copilotWorkspaceFileName),
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"sess-5", "sess-4"}, agenttest.Handles(got),
...
func TestCopilotHome(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(home, ".copilot"),
		copilotHome(agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(nil)}))

	assert.Equal(t, filepath.Join(home, "alt"),
		copilotHome(agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(map[string]string{"COPILOT_HOME": "~/alt"})}))
	...
	assert.Equal(t, filepath.Join(home, ".copilot"),
		copilotHome(agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(map[string]string{"XDG_STATE_HOME": testutil.NativeAbsPath("/xdg-state")})}))
}

func TestCopilotReadsItsSessionStore(t *testing.T) {
	t.Parallel()
