...
// ListStoredSessions reads Copilot's own per-session sidecars. See sessions.go.
func (copilotProvider) ListStoredSessions(ctx context.Context, q agent.StoredSessionQuery) ([]agent.StoredSession, error) {
...
//
// The translation lives in control_response.go, which also classifies the
...
func (copilotProvider) ResolveControlResponse(ctx agent.ControlResponseContext) agent.ControlResponseResolution {
...
func (copilotProvider) PlanModePermissionMode(agent.PlanModeControlKind) string { return "" }
