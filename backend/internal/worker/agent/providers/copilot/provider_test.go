...
func copilotPlugin(t *testing.T) agent.Provider {
	t.Helper()
...
func TestCopilotPlanModePermissionModeStatesNoMode(t *testing.T) {
	t.Parallel()
	plugin := copilotPlugin(t)
	for _, kind := range []agent.PlanModeControlKind{
		agent.PlanModeControlNone, agent.PlanModeControlEnter, agent.PlanModeControlExit, agent.PlanModeControlPrompt,
	} {
...
func TestCopilotStaticOptionGroupsMatchTheNativeAxes(t *testing.T) {
	...
	for id, want := range map[string][]string{
		...
		agent.OptionIDPermissionMode:       {contracts.CopilotPermissionModeManual, contracts.CopilotPermissionModeAssisted, contracts.CopilotPermissionModeAllowAll},
	} {
...
func TestCopilotDefaultModelUsesTheEnvironmentOverride(t *testing.T) {
	...
	assert.Equal(t, "gpt-5.4-mini", Registration().DefaultModel())
}

func TestCopilotSessionConfigRequestsTheEventsLeapMuxReads(t *testing.T) {
	t.Parallel()
	config := newCopilotSessionConfig(agent.Options{WorkingDir: "/project"}, "session-1", false)
