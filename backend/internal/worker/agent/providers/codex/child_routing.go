...
func (a *Agent) registerCollabReceiver(threadID, spawnCorrelationID, parentThreadID string) bool {
	if threadID == "" {
		return false
	}
	a.Mu.Lock()
...
func (a *Agent) registerCollabReceivers(
...
// child creation.
func (a *Agent) lookupCodexChildRoute(threadID string) (codexChildRoute, bool) {
...
func (a *Agent) ensureCodexChildRoute(threadID string) (codexChildRoute, bool) {
...
func (a *Agent) codexServicesForThread(threadID string) (agent.ProviderServices, string, bool) {
	a.Mu.Lock()
...
func (a *Agent) rememberCodexChildItemThread(itemID, threadID string) {
	if itemID == "" || threadID == "" {
		return
	}
	a.Mu.Lock()
...
func (a *Agent) enqueuePendingCodexChildEvent(threadID string, event codexPendingChildEvent) {
	if threadID == "" {
		return
	}
...
func (a *Agent) replayPendingCodexChildEvents(threadID string, route codexChildRoute) {
	a.Mu.Lock()
	state := a.collabChildren[threadID]
