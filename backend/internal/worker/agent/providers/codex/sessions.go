...
func codexHome(q agent.StoredSessionQuery) string {
	return sessionstore.HomeDirFromEnv(q, "CODEX_HOME", ".codex")
}
...
func codexStateDBPath(q agent.StoredSessionQuery) string {
...
func codexStoredSessions(ctx context.Context, q agent.StoredSessionQuery) ([]agent.StoredSession, error) {
	return sessionstore.Query(ctx, codexStateDBPath(q), codexSessionsSQL, q, sessionstore.ScanEpochMillisSession)
}
