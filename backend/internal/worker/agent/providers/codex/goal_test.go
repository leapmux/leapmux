...
func TestCodexGoal_UpdatedReportsNeutralGoal(t *testing.T) {
	...
	handleCodexOutput(ag, providerkit.ParseLine([]byte(
...
func TestCodexGoal_OmitsCountersCodexDoesNotReport(t *testing.T) {
	...
	handleCodexOutput(agent, providerkit.ParseLine([]byte(
...
func TestCodexGoal_StatusMapping(t *testing.T) {
	...
	for wire, want := range map[string]agent.GoalStatus{
		...
		"usageLimited":  agent.GoalStatusBlocked,
		"budgetLimited": agent.GoalStatusBlocked,
...
func TestCodexGoal_ControlRefusedWithoutAThread(t *testing.T) {
	...
	for _, action := range []agent.GoalAction{agent.GoalActionSet, agent.GoalActionClear, agent.GoalActionPause, agent.GoalActionResume} {
		_, err := a.PerformGoalAction(action, "anything")
		assert.Error(t, err, action)
	}
}

// Codex completes a goal action itself through its goal RPC, so it is a
// GoalWriter. It has no user-message command for the caller to queue and
// observe, so it is no GoalTextCommander.
func TestCodexSideBandGoal_ReturnsNoQueuedInput(t *testing.T) {
	t.Parallel()

	var codex any = &Agent{}
	_, writer := codex.(agent.GoalWriter)
	_, commander := codex.(agent.GoalTextCommander)
