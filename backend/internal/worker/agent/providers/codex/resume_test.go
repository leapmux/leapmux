...
func TestCodexStartOrResumeThread(t *testing.T) {
	...
	resume := func(t *testing.T, body agenttest.RPCReply) (codexThreadResult, func() []codexRecordedRequest, error) {
		t.Helper()
	...
	t.Run("adopts the resumed thread and sends no thread/start", func(t *testing.T) {
		t.Parallel()
		thread, requests, err := resume(t, agenttest.RPCReply{Result: json.RawMessage(`{"thread":{"id":"thread-old"},"model":"gpt-5.6-sol"}`)})
		require.NoError(t, err)
		assert.Equal(t, "thread-old", thread.ID)
		require.NotNil(t, thread.settings[agent.OptionIDModel])
		assert.Equal(t, "gpt-5.6-sol", *thread.settings[agent.OptionIDModel])
		sent := requests()
	...
	t.Run("pins concrete models and adopts the reported one", func(t *testing.T) {
		...
		for _, test := range []struct {
			...
			{name: "interrupted startup default resolves on resume", storedModel: agent.DefaultModelSentinel, responseModel: "gpt-5.6-sol", wantModelOmitted: true, resumeID: "thread-old", wantMethod: "thread/resume"},
			{name: "new session omits the account default", storedModel: agent.DefaultModelSentinel, responseModel: "gpt-5.6-sol", wantModelOmitted: true, wantMethod: "thread/start"},
			...
			t.Run(test.name, func(t *testing.T) {
				...
				a, _, requests := newCodexAgentForRPC(t, func(string) agenttest.RPCReply {
					return agenttest.RPCReply{Result: json.RawMessage(`{"thread":{"id":"thread-1"},"model":"` + test.responseModel + `"}`)}
				})
				a.model = test.storedModel
				params := codexThreadParams(test.storedModel, "/work", DefaultApprovalPolicy, contracts.CodexOptionDefaultSandboxPolicy, contracts.CodexOptionDefaultServiceTier)
	...
	t.Run("fails with the agent's own reason when the resume is refused", func(t *testing.T) {
		t.Parallel()
		_, requests, err := resume(t, agenttest.RPCReply{Error: json.RawMessage(`{"code":-32000,"message":"thread not found"}`)})
		require.Error(t, err)
	...
	t.Run("fails when the response does not parse", func(t *testing.T) {
		t.Parallel()
		_, requests, err := resume(t, agenttest.RPCReply{Result: json.RawMessage(`not json`)})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "thread-old")
		assert.Contains(t, err.Error(), "/clear")
	...
	t.Run("fails when the response carries no thread ID", func(t *testing.T) {
		t.Parallel()
		_, requests, err := resume(t, agenttest.RPCReply{Result: json.RawMessage(`{"thread":{}}`)})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "thread-old")
		assert.Contains(t, err.Error(), "/clear")
		require.Len(t, requests(), 1)
	...
	t.Run("starts a thread when there is nothing to resume", func(t *testing.T) {
		t.Parallel()
		a, _, requests := newCodexAgentForRPC(t, func(string) agenttest.RPCReply {
			return agenttest.RPCReply{Result: json.RawMessage(`{"thread":{"id":"thread-new"},"model":"gpt-5.4"}`)}
		})
	...
	t.Run("keeps the server reason when thread start fails", func(t *testing.T) {
		t.Parallel()
		a, _, requests := newCodexAgentForRPC(t, func(string) agenttest.RPCReply {
			return agenttest.RPCReply{Error: json.RawMessage(`{"code":-32000,"message":"model unavailable"}`)}
		})
...
func TestCodexClearContextStartsFreshThread(t *testing.T) {
	...
	a, sink, requests := newCodexAgentForRPC(t, func(method string) agenttest.RPCReply {
	...
	a.sandboxPolicy = SandboxDangerFullAccess
	...
	assert.Equal(t, SandboxDangerFullAccess, sent[0].Params["sandbox"])
	assert.Equal(t, ServiceTierFast, sent[0].Params["serviceTier"])
	assert.Equal(t, map[string]interface{}{"model_reasoning_summary": "detailed"}, sent[0].Params["config"])
}
...
func TestCodexClearContextDoesNotRouteLateChildOutputToTheNewRoot(t *testing.T) {
	...
	a.threadID = "thread-old"
	handleCodexOutput(a, providerkit.ParseLine([]byte(`{"method":"item/completed","params":{"threadId":"thread-old","turnId":"root-turn","item":{"type":"subAgentActivity","id":"spawn-call","kind":"started","agentThreadId":"old-child","agentPath":"/root/old_child"}}}`)))

	_, clearErr := a.ClearContext()
	require.NoError(t, clearErr)
	handleCodexOutput(a, providerkit.ParseLine([]byte(`{"method":"item/completed","params":{"threadId":"old-child","turnId":"child-turn","item":{"type":"agentMessage","id":"late-message","text":"LATE_OLD_CHILD_OUTPUT","phase":"final_answer"}}}`)))
...
func TestCodexClearContextRejectsAChildRouteThatFinishesLate(t *testing.T) {
	...
	a, baseSink, _ := newCodexAgentForRPC(t, func(string) agenttest.RPCReply {
		return agenttest.RPCReply{Result: json.RawMessage(`{
			"thread":{"id":"thread-new"},
			"model":"gpt-5.4",
			"reasoningEffort":"high",
	...
	a.sink = agent.NewProviderServices(blockingSink)
...
func TestCodexThreadResultAppliesConfirmedSettings(t *testing.T) {
	...
	assert.Equal(t, DefaultApprovalPolicy, a.approvalPolicy)
	assert.Equal(t, SandboxWorkspaceWrite, a.sandboxPolicy)
}
...
func TestCodexThreadResultAppliesCurrentSandboxObject(t *testing.T) {
	...
	assert.Equal(t, SandboxWorkspaceWrite, a.sandboxPolicy)
	assert.Equal(t, NetworkRestricted, a.networkAccess)
	...
	require.NoError(t, err)
	a.sandboxPolicy = SandboxReadOnly
	a.networkAccess = NetworkRestricted
...
func TestCodexThreadResultNullSettingsUseProviderDefaults(t *testing.T) {
	...
	assert.Equal(t, agent.EffortHigh, a.effort)
...
func TestCodexThreadResultPreservesGranularApprovalPolicy(t *testing.T) {
	...
	a := &Agent{approvalPolicy: DefaultApprovalPolicy}
	a.applyThreadResult(result)
