...
func writeCursorSession(t *testing.T, home, cwd, agentID, metaJSON string, at time.Time) {
	...
	agenttest.TouchFixture(t, dir, at)
}
...
func TestCursorStoredSessions(t *testing.T) {
	...
	writeCursorSession(t, home, testutil.NativeAbsPath("/other/dir"), "agent-elsewhere",
		`{"agentId":"agent-elsewhere","name":"Elsewhere"}`, base.Add(2*time.Hour))

	got, err := cursorStoredSessions(context.Background(), agent.StoredSessionQuery{
		WorkingDir: dir, HomeDir: home, Getenv: agenttest.FixtureEnv(nil),
	})
	require.NoError(t, err)

	assert.Equal(t, []string{"agent-new", "agent-old"}, agenttest.Handles(got),
...
func TestCursorStoredSessions_TimesByStoreDBOnly(t *testing.T) {
	...
	agenttest.WriteFixtureFile(t, wal, "a sidecar the read-only open can create")
	agenttest.TouchFixture(t, wal, footprint)
	agenttest.TouchFixture(t, sessionDir, footprint)
...
func TestCursorStoredSessions_OrdersByStoreDBNotDirectory(t *testing.T) {
	...
	agenttest.TouchFixture(t, filepath.Join(home, ".cursor", "chats", cursorChatsDirFor(dir), "agent-oldest"),
		base.Add(time.Hour))
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-newest", "agent-oldest"}, agenttest.Handles(got))
}
...
func TestCursorStoredSessions_SkipsDirectoriesWithNoStore(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-real"}, agenttest.Handles(got))
}
...
func TestCursorStoredSessions_SkipsUnreadableSessions(t *testing.T) {
	...
	agenttest.WriteFixtureFile(t, filepath.Join(chats, "agent-nodb", "notes.txt"), "x")
	// A database with the tables but no metadata row.
	agenttest.NewFixtureDB(t, filepath.Join(chats, "agent-nometa", "store.db"), cursorStoreDDL)
	// A metadata row whose value decodes to neither hex-JSON nor JSON.
	db := agenttest.NewFixtureDB(t, filepath.Join(chats, "agent-junk", "store.db"), cursorStoreDDL)
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-good"}, agenttest.Handles(got))
}
...
func TestCursorStoredSessions_FallsBackToTheDirectoryName(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-from-dir"}, agenttest.Handles(got),
...
// `keep` filter handed to `sessionstore.NewestEntries`; it moved into the reader's own scan
...
func TestCursorStoredSessions_IgnoresNonDirectoryEntries(t *testing.T) {
	...
	agenttest.WriteFixtureFile(t, stray, `{"not":"a session"}`)
	agenttest.TouchFixture(t, stray, base.Add(time.Hour))
...
func TestCursorStoredSessions_RespectsTheLimit(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-a", "agent-b"}, agenttest.Handles(got))
}
...
func TestCursorStoredSessions_BreaksTimeTiesByName(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"agent-alpha", "agent-zulu"}, agenttest.Handles(got))
}

func TestCursorReadsItsSessionStore(t *testing.T) {
	t.Parallel()
