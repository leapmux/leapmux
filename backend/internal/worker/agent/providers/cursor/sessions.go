...
// override for it.
func cursorHome(q agent.StoredSessionQuery) string {
	home := q.Home()
	if home == "" {
		return ""
	}
...
func cursorStoredSessions(ctx context.Context, q agent.StoredSessionQuery) ([]agent.StoredSession, error) {
	...
	//
	// `sessionstore.NamedFileInside` is what times a candidate by its `store.db` rather than
	...
	// open leaves alone.
	entries, err := sessionstore.NewestEntries(dir, limit, sessionstore.NamedFileInside(cursorStoreFileName))
	if err != nil {
...
func readCursorSession(ctx context.Context, entry sessionstore.Entry) (agent.StoredSession, bool) {
	storePath := filepath.Join(entry.Path, cursorStoreFileName)
	db, err := sessionstore.OpenDB(ctx, storePath)
	if err != nil {
	...
	return agent.StoredSession{
		...
		Title:     sessionstore.TrimTitle(meta.Name),
...
// modification time, which `sessionstore.NamedFileInside(cursorStoreFileName)` already took.
...
// session to the end.
func cursorSessionTime(entry sessionstore.Entry, meta cursorSessionMeta) time.Time {
	if !entry.ModTime.IsZero() {
		return entry.ModTime
	}
	return sessionstore.EpochMillis(meta.CreatedAt)
}
