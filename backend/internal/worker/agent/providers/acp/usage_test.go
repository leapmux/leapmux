...
func acpContextUsage(t *testing.T, sink *agenttest.Sink) map[string]any {
	t.Helper()
	v, ok := sink.LastSessionInfoValue(contracts.SessionInfoKeyContextUsage)
...
func TestACP_UsageUpdateReportsACostOnlyWhenPositive(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			cost, ok := sink.LastSessionInfoValue(contracts.SessionInfoKeyTotalCostUsd)
