...
// options.go holds the optionState config-option state machine -- the bookkeeping for an
// ACP provider's server-driven config options (the selectors the model and mode channels do not
// claim: effort, reasoning_effort, allow_all, ...). Split out of base.go so the option
// lifecycle reads in one place; every method here assumes the caller holds the owning Base.Mu.
...
// thinking_effort, tool_approval, ...). It is a field of Base, NOT a standalone locked
// object: every method here assumes the caller holds the OWNING Base.Mu (they are the
...
// Base.Mu critical section), so the type deliberately has none.
type optionState struct {
	...
	templates map[string]ConfigOption
...
// no lock of its own; the owning Base.Mu guards it like every other optionState field. The
...
// Caller holds the owning Base.Mu.
func (g *optionState) markKnown(option ConfigOption) {
	if g.known == nil {
		g.known = newBoundedIDSet()
	}
	if g.templates == nil {
		g.templates = make(map[string]ConfigOption)
	}
...
// persisted value worth deleting. Caller holds the owning Base.Mu.
...
// Base.Mu.
func (g *optionState) resolveCurrent(option ConfigOption, authority payloadAuthority) (current string, surface bool) {
	g.markKnown(option)
...
// Caller holds the owning Base.Mu.
func (g *optionState) storedIfOffered(option ConfigOption) (string, bool) {
...
func configOptionOffers(option ConfigOption, value string) bool {
...
func configOptionValuesContain(values []ConfigOptionValue, value string) bool {
...
func appendConfigOptionValueIfMissing(options []ConfigOptionValue, value string) []ConfigOptionValue {
	if value == "" || configOptionValuesContain(options, value) {
		return options
	}
	out := make([]ConfigOptionValue, len(options), len(options)+1)
	copy(out, options)
	return append(out, ConfigOptionValue{Value: value})
}
...
//
// effortConfigID is the running provider's declared effort id (Base.effortConfigID).
func buildOptionGroup(option ConfigOption, current, effortConfigID string) *leapmuxv1.AvailableOptionGroup {
	...
	if isEffortConfigOption(option, effortConfigID) {
		providerkit.SortEffortsDescending(opts)
	}
	return &leapmuxv1.AvailableOptionGroup{
		Id:           option.ID,
		Label:        agent.NameOrID(option.Name, option.ID),
		Options:      opts,
		...
		Mutable: true,
		Order:   agent.OptionOrderTrailing,
	}
}
...
func (b *Base) applyOptionGroupsLocked(options []ConfigOption) (valueChanged, listChanged bool) {
	return b.options.apply(options, authoritativePayload, b.hooks.ModeChannel, b.hooks.EffortConfigID)
}
...
func (b *Base) applyOptionGroupsKeepingStoredLocked(options []ConfigOption) (valueChanged, listChanged bool) {
	return b.options.apply(options, preferStoredValue, b.hooks.ModeChannel, b.hooks.EffortConfigID)
}

// apply is the shared core of the two Base wrappers above; authority picks the value
...
// Base.Mu.
func (g *optionState) apply(options []ConfigOption, authority payloadAuthority, modeChannel ModeChannel, effortConfigID string) (valueChanged, listChanged bool) {
	...
	if option, ok := acpConfigOptionByCategory(options, acpConfigOptionCategoryModel, ConfigOptionIDModel); ok {
	...
	if modeChannel != ModeChannelUnmapped {
		if option, ok := acpConfigOptionByCategory(options, acpConfigOptionCategoryMode, ConfigOptionIDMode); ok {
	...
	winner := make(map[string]ConfigOption, len(options))
	...
	for _, id := range order {
		...
		groups = append(groups, buildOptionGroup(option, current, effortConfigID))
		values[id] = current
		g.markSurfaced(id)
	}
	...
	listChanged = !agent.OptionGroupSetEqualExact(g.groups, groups)
...
// (primaryAgent) can never clobber it. The caller must hold the owning Base.Mu.
//
...
// only for an empty value. Caller holds the owning Base.Mu.
func (gs *optionState) recordOptimistic(configID, value, effortConfigID string) {
	if value == "" {
		return
	}
	if gs.values == nil {
	...
	if !ok {
		tmpl = ConfigOption{ID: configID}
	}
	gs.groups = append(gs.groups, buildOptionGroup(tmpl, value, effortConfigID))
...
func isReservedOptionKey(id string) bool {
	return id == agent.OptionIDModel || id == agent.OptionIDPermissionMode || id == agent.OptionIDPrimaryAgent
}
