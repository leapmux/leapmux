...
func TestPiArtifactRecoveryFollowsSessionIdentityAndFinalBoundaries(t *testing.T) {
	...
	for _, scenario := range []string{"first identity", "new session", "cancelled context", "next boundary"} {
		t.Run(scenario, func(t *testing.T) {
			...
			defer cancel()
			sink := &agenttest.Sink{}
			transcript := newPiToolTranscript(ctx, agent.NewProviderServices(sink))
			...
			require.NoError(t, transcript.PersistMessage(leapmuxv1.MessageSource_MESSAGE_SOURCE_AGENT, agent.MessageContent{Original: raw}, agent.SpanInfo{SpanID: "call", Closing: true}))
			...
			require.NoError(t, transcript.PersistTurnEnd(agent.MessageContent{Original: []byte(`{"type":"agent_end"}`)}, agent.SpanInfo{}))
...
func TestPiArtifactRecoveryKeepsTheNativeResultWithinTheCombinedLimit(t *testing.T) {
	...
	padding := agent.LiveMaxMessageSize() - 1024 - remaining - len(raw) - len(`,"padding":""`)
...
func TestRecoverPiToolArtifactsRefusesAMessageThatFillsTheFrame(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(path, []byte("full"), 0o600))
	raw := piArtifactTestEvent(t, map[string]any{"outputGuard": map[string]any{"truncated": true, "fullOutputPath": path}})
	// LiveMaxMessageSize reads a live atomic that a renegotiation can shrink, so
	// the original message alone can already fill the whole frame.
	padding := agent.LiveMaxMessageSize() - len(raw) - len(`,"padding":""`)
	require.Positive(t, padding)
	raw = []byte(string(raw[:len(raw)-1]) + `,"padding":"` + strings.Repeat("x", padding) + `"}`)
	extra, complete, err := recoverPiToolArtifacts(t.Context(), raw, nil)
