...
func TestPiMessageContentConformance(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile(testutil.RepoPath(t, "testdata", "pi_message_content_conformance.json"))
	...
	for _, tc := range fixture.Cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			resolved := piProvider{}.ResolveProviderData(agent.MessageContent{Original: tc.Original, Supplemental: tc.Supplemental})
			assert.JSONEq(t, string(tc.Expected), string(resolved))
			again := piProvider{}.ResolveProviderData(agent.MessageContent{Original: resolved, Supplemental: tc.Supplemental})
