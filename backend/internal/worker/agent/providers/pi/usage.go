...
func piContextUsageFromAssistantUsage(usage piAssistantUsage, contextWindow int64) map[string]any {
	...
	ctx := providerkit.ContextUsageMap(providerkit.ContextTokenCounts{
		Input:      usage.Input,
...
func piAgentEndContent(raw []byte, snap piUsageSnapshot, durationMs *int64) agent.MessageContent {
...
func (a *Agent) canRequestPiSessionStats() bool {
	a.Mu.Lock()
	defer a.Mu.Unlock()
	return a.HasStdin() && !a.StoppedLocked()
}

// snapshotLocked builds a usage snapshot from the agent's current
// state. Caller must hold a.Mu.
func (a *Agent) snapshotLocked() piUsageSnapshot {
...
func (a *Agent) currentPiUsageSnapshot() piUsageSnapshot {
	a.Mu.Lock()
	defer a.Mu.Unlock()
	return a.snapshotLocked()
}

func (a *Agent) currentPiContextWindow() int64 {
	a.Mu.Lock()
	defer a.Mu.Unlock()
...
func (a *Agent) recordPiAssistantUsage(usage piAssistantUsage, contextUsage map[string]any) piUsageSnapshot {
	a.Mu.Lock()
	defer a.Mu.Unlock()
...
func (a *Agent) applyPiSessionStats(stats piSessionStats, generation uint64, expectedSessionID string) (piUsageSnapshot, bool) {
	a.goal.publishMu.Lock()
	defer a.goal.publishMu.Unlock()
	a.Mu.Lock()
	if a.goal.stopping || a.StoppedLocked() || a.IsDiscardingOutput() || (a.sessionID != expectedSessionID && stats.SessionID != a.sessionID) {
		a.Mu.Unlock()
...
func (a *Agent) fetchPiSessionStats(timeout time.Duration) (piSessionStats, bool) {
	var stats piSessionStats
	raw, err := a.sendPiCommand(CommandGetSessionStats, nil, timeout)
	if err != nil {
...
func (a *Agent) refreshPiSessionStats(timeout time.Duration) (piUsageSnapshot, bool) {
...
func (a *Agent) piMessageEndContent(raw []byte) agent.MessageContent {
	content := agent.MessageContent{Original: raw}
	var envelope struct {
		Type    string `json:"type"`
		Message *struct {
...
func (a *Agent) persistPiAgentEnd(content agent.MessageContent, willRetry bool) {
	var err error
	if willRetry {
		err = a.sink.PersistMessage(leapmuxv1.MessageSource_MESSAGE_SOURCE_AGENT, content, agent.SpanInfo{})
	} else {
		err = a.sink.PersistTurnEnd(content, agent.SpanInfo{})
	}
	if err != nil {
