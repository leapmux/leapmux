...
func TestPiMCPApprovalResponse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		result, matched := resolvePiMCPApprovalResponse(agent.ControlResponseContext{RequestPayload: request, ResponseContent: response})
	...
	result, matched := resolvePiMCPApprovalResponse(agent.ControlResponseContext{RequestPayload: request, ResponseContent: []byte(`{"response":{"request_id":"other","response":{"action":"accept"}}}`)})
	require.True(t, matched)
	assert.True(t, result.Withhold)
	_, matched = resolvePiMCPApprovalResponse(agent.ControlResponseContext{RequestPayload: []byte(`{"type":"extension_ui_request","id":"question","method":"select","title":"Choose","options":["Allow once","Allow for session","Deny"]}`)})
...
func TestPiControlSourceRejectsAmbiguousAndChangedTools(t *testing.T) {
	for _, change := range []string{"second tool", "completed", "changed arguments"} {
		t.Run(change, func(t *testing.T) {
			...
			handlePiOutput(a, providerkit.ParseLine([]byte(`{"type":"tool_execution_start","toolCallId":"mcp","toolName":"mcp","args":{"tool":"probe_read","args":{}}}`)))
			switch change {
			case "second tool":
				handlePiOutput(a, providerkit.ParseLine([]byte(`{"type":"tool_execution_start","toolCallId":"other","toolName":"read","args":{"path":"sample.py"}}`)))
			case "completed":
				handlePiOutput(a, providerkit.ParseLine([]byte(`{"type":"tool_execution_end","toolCallId":"mcp","toolName":"mcp","result":{"content":[]}}`)))
