...
// piTestRig wires an Agent to an in-memory pipe pair and a fake peer that
...
func newPiTestRig(t *testing.T, sink agent.ProviderServices) *piTestRig {
	t.Helper()
	...
	// Process.ReadOutput here because it ends with cmd.Wait(), which
...
func TestPi_SendPiCommand_TimeoutReturnsError(t *testing.T) {
	...
	_, err := rig.agent.sendPiCommand(CommandGetState, nil, 50*time.Millisecond)
	require.Error(t, err)
...
func TestPi_HandlePiResponse_IgnoresNonResponseLines(t *testing.T) {
	...
	a := &Agent{Process: providerkit.NewProcessFrom(providerkit.ProcessConfig{AgentID: "test-agent"})}
	consumed := a.handlePiResponse(providerkit.ParseLine([]byte(`{"type":"agent_start"}`)))
...
func TestPi_ApplyAvailableModels_BuildsCatalog(t *testing.T) {
	...
	assert.Contains(t, got, agent.EffortAuto)
	assert.Contains(t, got, ThinkingHigh)
	assert.Contains(t, got, ThinkingXHigh)
	...
	assert.ElementsMatch(t, []string{agent.EffortAuto, ThinkingOff}, got)
}
...
func TestPi_CurrentSettings_ReflectsLocalState(t *testing.T) {
	...
	a := &Agent{
		...
		availableModels: []*agent.ModelInfo{{
			Id:          "gpt-5.5",
			DisplayName: "GPT-5.5",
			SupportedEfforts: []*agent.EffortInfo{
				{Id: "low", Name: "Low"},
	...
	assert.Equal(t, "gpt-5.5", optionids.CurrentValue(groups, agent.OptionIDModel))
	assert.Equal(t, "medium", optionids.CurrentValue(groups, agent.OptionIDEffort))
...
func TestPi_UpdateSettings_EffortAutoRequiresRestart(t *testing.T) {
	...
	ok := a.UpdateSettings(map[string]string{agent.OptionIDEffort: agent.EffortAuto})
...
func TestPi_UpdateSettings_FailedApplyRequestsRestart(t *testing.T) {
	...
	ok := rig.agent.UpdateSettings(map[string]string{agent.OptionIDEffort: "high"})
...
func TestPi_Stop_SendsAbortBeforeClosingStdin(t *testing.T) {
	...
	rig.agent.Mu.Lock()
	stopped := rig.agent.StoppedLocked()
	rig.agent.Mu.Unlock()
...
func TestPiClearContextPreservesTheSessionWhenCancelled(t *testing.T) {
	...
	assert.ErrorIs(t, clearErr, agent.ErrContextClearCancelled)
...
func TestPiAgentStartRecoversAnExtensionSessionReplacement(t *testing.T) {
	...
	rig.agent.Mu.Unlock()
	rig.setResponder(func(req piRecordedRequest) (json.RawMessage, bool, string) {
		if req.Type != CommandGetSessionStats {
...
func TestPiSessionStatsCannotRestoreAReplacedSession(t *testing.T) {
	t.Parallel()
	sink := &agenttest.Sink{}
	a := &Agent{sink: agent.NewProviderServices(sink), sessionID: "new-session", sessionFile: "/project/new.jsonl", sessionCostUsd: 3}
...
func TestPiExtensionCommandPublishesIdleBeforeSendReturns(t *testing.T) {
	...
	rig := newPiTestRig(t, agent.NewProviderServices(sink))
	rig.setResponder(func(req piRecordedRequest) (json.RawMessage, bool, string) {
		if req.Type == CommandGetCommands {
...
func TestPi_UpdateSettings_BroadcastsRefreshedSettings(t *testing.T) {
	...
	assert.Equal(t, "openai-codex", last.Options[OptionProvider])
}
...
func TestPi_UpdateSettings_TakesTheModelFromPiOwnState(t *testing.T) {
	...
	ok := rig.agent.UpdateSettings(map[string]string{agent.OptionIDModel: "gpt-5.5"})
	require.True(t, ok.AppliedLive)
...
func TestPiLaunchDefaults(t *testing.T) {
	...
	t.Run("the fallback catalog describes the default model", func(t *testing.T) {
		...
		assert.Equal(t, DefaultModel, entry.Id)
		assert.True(t, entry.IsDefault)
		assert.Equal(t, DefaultThinkingLevel, entry.DefaultEffort)
	...
	t.Run("the display name identifies the default model", func(t *testing.T) {
		assert.True(t, strings.EqualFold(piDefaultModels[0].DisplayName, DefaultModel),
			"display name %q must identify model %q", piDefaultModels[0].DisplayName, DefaultModel)
	})
	...
	t.Run("a cold agent resolves the default provider", func(t *testing.T) {
		cold := &Agent{Process: providerkit.NewProcessFrom(providerkit.ProcessConfig{AgentID: "test-agent"})}
		assert.Equal(t, DefaultProvider, cold.providerForModel(DefaultModel))
		assert.Equal(t, DefaultProvider, cold.providerForModel("anything-else"))
	})
	...
	t.Run("the reported catalog overrides the default provider", func(t *testing.T) {
		...
		assert.Equal(t, DefaultProvider, warm.providerForModel("absent-model"))
	})
}

func TestPiSessionIdentityReplacesTheFileWhenTheIDChanges(t *testing.T) {
	t.Parallel()
	a := &Agent{sessionID: "old", sessionFile: "/project/old.jsonl"}
	a.Mu.Lock()
	defer a.Mu.Unlock()
