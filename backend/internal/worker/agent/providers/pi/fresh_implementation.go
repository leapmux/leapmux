...
func (a *Agent) notePiPlanFreshApproval() {
	a.Mu.Lock()
...
func (a *Agent) answerPiFreshSettingsDialog(id string, raw []byte) bool {
	...
	match := a.freshImplementationPending &&
		...
		piDialogTitleFirstLine(dialog.Title) == PlanDialogFreshSettingsTitle &&
		slices.Contains(dialog.Options, PlanActionStartFresh)
	if match {
