...
func TestZCodeMessageContentConformance(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile(testutil.RepoPath(t, "testdata", "zcode_message_content_conformance.json"))
	...
	for _, tc := range fixture.Cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			resolved := zcodeProvider{}.ResolveProviderData(agent.MessageContent{Original: tc.Original, Supplemental: tc.Supplemental})
...
func TestZCodeControlInputPreservesEarlierSupplement(t *testing.T) {
	...
	resolved := zcodeProvider{}.ResolveProviderData(agent.MessageContent{Original: message.Content, Supplemental: message.SupplementalContent})
...
func (*zcodeRejectedEnrichmentSink) EnrichMessage(agent.MessageEnrichment) (bool, error) {
	return false, nil
}
...
func (*zcodeFailingToolRequestSink) ReadToolRequest(string) (*agent.StoredMessage, error) {
