...
func TestZCodeStaticOptionGroups_ModeListExcludesAuto(t *testing.T) {
	...
	group := optionids.GroupByID(
		Registration().OptionGroups, agent.OptionIDPermissionMode)
	require.NotNil(t, group)
	assert.Equal(t, ModeLabel, group.GetLabel())
...
func TestApplySettingsSnapshot_ReadsTheAuthoritativeCurrentFields(t *testing.T) {
	...
	ids := agenttest.OptionIDs(a.observedThoughtLevels)
	assert.Equal(t, []string{agent.EffortAuto, "high", "low"}, ids,
...
func TestApplySettingsSnapshot_BuildsTheCurrentProtocolModelCatalog(t *testing.T) {
	...
	assert.Equal(t, []string{agent.EffortAuto, "max", "low"}, agenttest.OptionIDs(models[0].SupportedEfforts))
...
func TestApplySettingsSnapshot_OrdersTheLevelsLikeTheConfiguredCatalog(t *testing.T) {
	...
	a.Mu.Lock()
	defer a.Mu.Unlock()
	fromSnapshot := agenttest.OptionIDs(a.observedThoughtLevels)
	assert.Equal(t, []string{agent.EffortAuto, "max", "high", "low"}, fromSnapshot)
...
func TestApplySettingsSnapshot_AModelWithNoThoughtLevelsClampsToAuto(t *testing.T) {
	...
	for name, settings := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			a.Mu.Lock()
			defer a.Mu.Unlock()
			assert.Equal(t, agent.EffortAuto, a.thoughtLevel)
			assert.Equal(t, agent.EffortAuto, a.observedThoughtDefault)
			require.Len(t, a.observedThoughtLevels, 1)
			assert.Equal(t, agent.EffortAuto, a.observedThoughtLevels[0].GetId())
		})
	}
}
...
func TestZCodeOptionGroups_ExposesModelThoughtLevelAndMode(t *testing.T) {
	...
	modelIDs := agenttest.OptionIDs(modelGroup.GetOptions())
	...
	effortGroup := optionids.GroupByID(groups, agent.OptionIDEffort)
	require.NotNil(t, effortGroup)
	assert.Equal(t, ThoughtLevelLabel, effortGroup.GetLabel(),
...
func TestZCodeModelsForUI_OverridesOnlyTheCurrentModelsEfforts(t *testing.T) {
	...
	a.observedThoughtLevels = []*agent.EffortInfo{zcodeAutoEffort, {Id: "turbo", Name: "Turbo"}}
	...
	liveIDs := agenttest.OptionIDs(live.SupportedEfforts)
	assert.Equal(t, []string{agent.EffortAuto, "turbo"}, liveIDs)
	...
	for _, m := range a.catalog.Models {
		if m.GetId() == "builtin:zai/GLM-5.3" {
			ids := agenttest.OptionIDs(m.SupportedEfforts)
			assert.Equal(t, []string{agent.EffortAuto, "max", "high", "low"}, ids,
...
func TestHandleZCodeStateUpdated_SessionPatchAppliesTopLevelAxes(t *testing.T) {
	...
	a.HandleOutput(zcodeStateLine(t, ScopeSession, "mode_changed", `{"mode":{"current":"plan"}}`))
...
func TestHandleZCodeStateUpdated_NonSessionScopeIsNotSettings(t *testing.T) {
	...
	a.HandleOutput(zcodeStateLine(t, ScopeWorkspace, "catalog_changed", `{"mode":{"current":"yolo"}}`))
...
func TestHandleZCodeStateUpdated_StatusOnlyPatchPersistsNothing(t *testing.T) {
	...
	a.HandleOutput(zcodeStateLine(t, ScopeSession, "prompt_started", `{"status":"running"}`))
...
func TestHandleZCodeStateUpdated_RuntimeInAPatchIsApplied(t *testing.T) {
	...
	a.HandleOutput(zcodeStateLine(t, ScopeSession, "runtime",
...
func TestApplyZCodeModel_OverlayCarriesTheLevelAndAutoResolvesTheDefault(t *testing.T) {
	...
	for level, want := range cases {
		t.Run("level "+level, func(t *testing.T) {
			...
			assert.Equal(t, MethodSetModel, requests[0].Method)
			var params struct {
...
func TestApplyZCodeThoughtLevel_AlwaysSendsTheLevel(t *testing.T) {
	...
	requests := stdin.Requests(t)
	require.Len(t, requests, 1)
	assert.Equal(t, MethodSetThoughtLevel, requests[0].Method)
...
func TestApplyZCodeMode_SendsTheModeAndAppliesNoLocalValue(t *testing.T) {
	...
	requests := stdin.Requests(t)
	require.Len(t, requests, 1)
	assert.Equal(t, MethodSetMode, requests[0].Method)
	var params map[string]any
	require.NoError(t, json.Unmarshal(requests[0].Params, &params))
...
func TestZCodeUpdateSettings_EffortToAutoAsksForARestart(t *testing.T) {
	...
	assert.False(t, a.UpdateSettings(map[string]string{agent.OptionIDEffort: agent.EffortAuto}).AppliedLive)
...
func TestZCodeUpdateSettings_NoChangeSendsNothingAndSucceeds(t *testing.T) {
	...
	a.Mu.Lock()
	a.model, a.thoughtLevel, a.mode = "p/m", "high", contracts.ZCodeModeBuild
	a.unresolvedSettings = map[string]struct{}{agent.OptionIDEffort: {}}
	a.Mu.Unlock()

	result := a.UpdateSettings(map[string]string{
		agent.OptionIDModel:          "p/m",
		agent.OptionIDEffort:         "high",
		agent.OptionIDPermissionMode: contracts.ZCodeModeBuild,
	})
...
func TestZCodeUpdateSettings_ARespelledModelIsNotASwitch(t *testing.T) {
	...
	assert.True(t, a.UpdateSettings(map[string]string{agent.OptionIDModel: `p\m`}).AppliedLive)
	assert.Empty(t, stdin.Frames())
}
...
func TestZCodeUpdateSettings_AFailedApplyRestoresTheCapturedTrio(t *testing.T) {
	...
	assert.False(t, a.UpdateSettings(map[string]string{
		agent.OptionIDModel:          "builtin:zai/GLM-5.3-Flash",
		agent.OptionIDPermissionMode: contracts.ZCodeModeYolo,
	}).AppliedLive)
...
func TestZCodeOptionGroups_LabelsTheLiveAndConfiguredLevelsAlike(t *testing.T) {
	...
	for _, e := range optionids.GroupByID(groups, agent.OptionIDEffort).GetOptions() {
		live[e.GetId()] = e.GetName()
	}
	assert.Equal(t, map[string]string{agent.EffortAuto: "Auto", "low": "Low", "max": "Max"}, live)

	// The catalog path builds the level list for every OTHER model, and reads the same.
	configured := map[string]string{}
	for _, m := range optionids.GroupByID(groups, agent.OptionIDModel).GetOptions() {
		if m.GetId() != "builtin:zai/GLM-5.3" {
			continue
		}
		for _, g := range m.GetSubGroups() {
			if g.GetId() != agent.OptionIDEffort {
				continue
			}
	...
	assert.Equal(t, map[string]string{agent.EffortAuto: "Auto", "low": "Low", "high": "High", "max": "Max"}, configured)
