...
//
// The "no name given" half routes through agent.NameOrID, which every other naming
...
func titleCaseHyphenated(s string) string {
	...
	for i, w := range words {
		words[i] = providerkit.CapitalizeFirst(w)
	}
...
func zcodeProviderLabel(providerID, configuredName string, collides bool) string {
	...
	if builtin == "" {
		return agent.NameOrID(configuredName, providerID)
	}
...
func zcodeModelInfo(composite, providerLabel, modelID string, model zcodeConfigModel) *agent.ModelInfo {
	...
	if r := model.Reasoning; r != nil && r.Enabled && len(r.Variants) > 0 {
		efforts := make([]*agent.EffortInfo, 0, len(r.Variants))
		for _, v := range r.Variants {
...
func markZCodeDefaultModel(models []*agent.ModelInfo) {
...
// Registration.NormalizeModelID function, which is why the two are separate.
...
func (c zcodeCatalog) defaultThoughtLevel(modelID string) string {
	...
	for _, m := range c.Models {
		...
		if level := m.DefaultEffort; level != "" && level != agent.EffortAuto {
			return level
		}
		return ""
	}
