...
func (a *Agent) supplementZCodeControlInput(toolCallID, toolName string, input json.RawMessage) (carriesInput bool, err error) {
	...
	combined, err := tooltranscript.MergeSupplements(stored.Content.Supplemental, supplement)
	if err != nil {
	...
	if agent.JSONCanonicalEqual(combined, stored.Content.Supplemental) {
		return true, nil
	}
	// A refused write leaves the row without the input, and its bool says so.
	return sink.EnrichMessage(agent.MessageEnrichment{
...
func (zcodeProvider) ResolveProviderData(content agent.MessageContent) []byte {
