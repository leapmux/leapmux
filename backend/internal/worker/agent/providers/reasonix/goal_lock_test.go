...
func (p *goalApplyProbe) UpsertGoal(update agent.GoalUpdate) {
	p.onApply()
...
func TestReasonixStatusUpdateAppliesTheGoalUnderTheRevisionLock(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			sink := &agenttest.Sink{}
			a := &Agent{}
			a.SetSessionIDForTest("session")
			applies, heldDuringApply := 0, 0
			a.SetSinkForTest(&goalApplyProbe{ProviderServices: agent.NewProviderServices(sink), onApply: func() {
				applies++
...
func TestReasonixStatusUpdateReportsUsageAndPhaseOutsideTheRevisionLock(t *testing.T) {
	...
	a.SetSessionIDForTest("session")
	free := 0
	a.SetSinkForTest(&sessionInfoProbe{ProviderServices: agent.NewProviderServices(sink), onReport: func() {
		if a.goalStatusMu.TryLock() {
			a.goalStatusMu.Unlock()
			free++
		}
