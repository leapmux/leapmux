...
func TestReasonixToolStoreRecoversTheFullErrorBehindTheACPHeadline(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		require.NoError(t, err)
		records, err := readReasonixToolSupplements(t.Context(), path, map[string]agent.MessageContent{"call": {Original: original}}, &reasonixEventCache{})
		require.NoError(t, err)
		if !tt.want {
			assert.Empty(t, records)
			continue
...
func TestReasonixToolStorePathResolvesAgainstTheQueryHome(t *testing.T) {
	t.Parallel()
	home := t.TempDir()
	query := agent.StoredSessionQuery{HomeDir: home, Getenv: func(string) string { return "" }}
