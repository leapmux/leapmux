...
func seedGooseDB(t *testing.T, path string, rows []gooseSessionRow) {
	t.Helper()
	db := agenttest.NewFixtureDB(t, path, gooseSessionsDDL)
	for _, r := range rows {
		agenttest.RequireHostAbsDir(t, r.workingDir)
		_, err := db.Exec(
...
func TestGooseStoredSessions(t *testing.T) {
	...
	seedGooseDB(t, filepath.Join(home, ".local", "share", "goose", "sessions", "sessions.db"), []gooseSessionRow{
		...
		{id: "20260830_5", name: "Elsewhere", kind: "acp", workingDir: testutil.NativeAbsPath("/other/dir"), createdAt: "2026-08-30 18:00:00", updatedAt: "2026-08-30 18:00:00"},
	})

	got, err := gooseStoredSessions(context.Background(), agent.StoredSessionQuery{
		WorkingDir: dir, HomeDir: home, Getenv: agenttest.FixtureEnv(nil),
	})
	require.NoError(t, err)

	assert.Equal(t, []string{"20260830_2", "20260830_1"}, agenttest.Handles(got),
...
func TestGooseStoredSessions_KeepsACPSessions(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"acp_only"}, agenttest.Handles(got))
}
...
func TestGooseStoredSessions_OrdersByInstantNotByText(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"late", "tform_a"}, agenttest.Handles(got),
...
func TestGooseStoredSessions_AnUnreadableTimeDoesNotTakeASlot(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"real_a", "real_b"}, agenttest.Handles(got),
...
func TestGooseDBCandidates(t *testing.T) {
	...
	// question on Windows.
	root := testutil.NativeAbsPath("/goose-root")
	rooted := agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(map[string]string{"GOOSE_PATH_ROOT": root})}
	...
	relative := agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(map[string]string{"GOOSE_PATH_ROOT": "relative/path"})}
	...
	plain := gooseDBCandidates(agent.StoredSessionQuery{HomeDir: home, Getenv: agenttest.FixtureEnv(nil)})
...
func TestGooseDBPath_PrefersTheCandidateThatExists(t *testing.T) {
	...
	agenttest.WriteFixtureFile(t, legacy, "")
...
func TestGooseReadsItsSessionStore(t *testing.T) {
	t.Parallel()
