...
// PeekForTest returns the prompt held under key WITHOUT spending it, or "" when none
// is held. For assertions and diagnostics; the production paths take().
func (p *PendingPrompts) PeekForTest(key string) string {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.byKey[key]
}

// CountForTest returns how many prompts are held. For assertions and diagnostics: a
// non-zero count after a spawn's lifecycle ended is a retained prompt.
func (p *PendingPrompts) CountForTest() int {
	p.mu.Lock()
	defer p.mu.Unlock()
	return len(p.byKey)
}
