...
func TestDetachedStdinWritesCostOneGoroutineAndKeepTheirOrder(t *testing.T) {
	...
	p := &processBase{agentID: "agent", stdin: nopWriteCloser{writer}}
	...
	// acpBase.Interrupt and CodexAgent.Interrupt both send their answers first.
...
func TestStdinWriteAfterStopStillReportsItsOwnError(t *testing.T) {
	...
	writer := &blockingWriter{release: make(chan struct{})}
	close(writer.release)
	p := &processBase{agentID: "agent", stdin: nopWriteCloser{writer}, processDone: make(chan struct{})}
	close(p.processDone)
	p.Stop()
