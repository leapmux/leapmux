...
func newNativeCopilotForEvents(t *testing.T) (*copilotAgent, *testSink) {
	t.Helper()
	sink := &testSink{}
	a := &copilotAgent{
...
func copilotBackgroundRow(t *testing.T, sink *testSink, rowKey string) (bgtask.Item, bool) {
	t.Helper()
...
func TestNativeCopilotFailedToolResultCarriesTheErrorCompletion(t *testing.T) {
	...
	messages := sink.Messages()
	require.Len(t, messages, 2)
	assert.Equal(t, MessageCompletionError, messages[1].Completion)
}
...
func TestNativeCopilotNestedSubagentBelongsToItsParent(t *testing.T) {
	...
	require.True(t, ok)
	parentSink, ok := sink.ChildSink(parentRow.ChildAgentID).(*testSink)
	require.True(t, ok)
...
func TestNativeCopilotTurnEndClosesAToolThatNeverCompleted(t *testing.T) {
	...
	assert.Equal(t, MessageCompletionInterrupted, closing.Completion)
...
func TestNativeCopilotKeepsTheAnswerAnInterruptedTurnStreamed(t *testing.T) {
	...
	for _, message := range sink.Messages() {
		if kind, completion := MessageMetadata(MessageContent{Original: message.Content, Completion: message.Completion}); kind != leapmuxv1.AssembledMessageKind_ASSEMBLED_MESSAGE_KIND_UNSPECIFIED {
...
func TestNativeCopilotCompletesASegmentTheTurnEndFindsOpen(t *testing.T) {
	...
	found := false
	for _, message := range sink.Messages() {
		kind, completion := MessageMetadata(MessageContent{Original: message.Content, Completion: message.Completion})
