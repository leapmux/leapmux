...
func parseCopilotModels(raw json.RawMessage) ([]*ModelInfo, error) {
	var response struct {
		Models *[]struct {
			ID           string `json:"id"`
	...
	for _, entry := range *response.Models {
		...
		model := &ModelInfo{
			Id: entry.ID, DisplayName: StringOrDefault(entry.Name, entry.ID),
		...
		for _, effort := range entry.Capabilities.Supports.ReasoningEfforts {
			...
			model.SupportedEfforts = append(model.SupportedEfforts, effortTier(effort))
		}
		sortEffortsDescending(model.SupportedEfforts)
