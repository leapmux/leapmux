...
type OutputSink interface {
	PersistMessage(role leapmuxv1.MessageRole, content []byte, threadID string) error
	MergeIntoThread(threadID string, content []byte) bool
...
type Provider interface {
	...
	Wait() error
	Stderr() string
	CurrentSettings() *leapmuxv1.UpdateAgentSettingsRequest
	...
	UpdateSettings(s *leapmuxv1.UpdateAgentSettingsRequest) bool
}
