...
func (piProvider) ExtractTodoEvent(_ string, content []byte, _ func() []byte) (todoevents.Event, bool) {
	...
	for _, task := range *envelope.Result.Details.Tasks {
		...
		items = append(items, todoevents.Item{ID: strconv.FormatInt(task.ID, 10), Content: task.Subject, Description: task.Description, ActiveForm: task.ActiveForm, Status: todoevents.StatusFromProviderWord(task.Status)})
	}
