...
//
// The registry row keys off the SPAWN toolCallId (the same key the spawn
// tool_call upserts), so the terminal close reaches it. The child transcript
// links through ChildAgentKey (the per-request subagent_id), which keeps each
// request's transcript under its own child while the registry row collapses to
// one closeable entry per spawn.
func gooseSubagentFromToolCallUpdate(tcu acpToolCallUpdateEnvelope) *acpSubagentObservation {
	...
	// Key the registry row off the SPAWN toolCallId so the terminal close
	// reaches it. The subagent_id still links the child transcript
	// (ChildAgentKey), but a row keyed only by subagent_id would never close
	// (the terminal update knows only the spawn toolCallId).
	childKey := params.Data.SubagentID
	if childKey == "" {
