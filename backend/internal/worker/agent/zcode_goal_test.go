...
func TestZCodeGoalActionAppliesTheReplyGoal(t *testing.T) {
	...
	outcome, err := a.PerformGoalAction(GoalActionSet, "Ship the parity matrix")
...
func TestZCodeGoalSetSendsReplace(t *testing.T) {
	...
	_, err := a.PerformGoalAction(GoalActionSet, "pause the release")
	require.NoError(t, err)
...
func TestZCodeGoalActionReadsTheDocumentedGoalKey(t *testing.T) {
	...
	_, err := a.PerformGoalAction(GoalActionSet, "Documented shape")
	require.NoError(t, err)

	goals := sink.Goals()
	require.Len(t, goals, 1)
	assert.Equal(t, "Documented shape", goals[0].Objective)
	assert.Equal(t, GoalStatusPaused, goals[0].Status)
}
...
func TestZCodeGoalStatusDetailKeepsOnlyWhatTheStatusLoses(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{zcodeGoalStatusActive, GoalStatusActive, ""},
		{zcodeGoalStatusPaused, GoalStatusPaused, ""},
		{zcodeGoalStatusVerifying, GoalStatusActive, zcodeGoalStatusVerifying},
		{zcodeGoalStatusVerified, GoalStatusDone, zcodeGoalStatusVerified},
		{zcodeGoalStatusNotSatisfied, GoalStatusBlocked, zcodeGoalStatusNotSatisfied},
		{zcodeGoalStatusFailed, GoalStatusBlocked, zcodeGoalStatusFailed},
	} {
...
func TestZCodeGoalActiveCarriesNoRedundantDetail(t *testing.T) {
	...
	_, err := a.PerformGoalAction(GoalActionSet, "Ship it")
	require.NoError(t, err)
