...
func TestNativeCopilotControlResponseRequiresAReceipt(t *testing.T) {
	for _, outcome := range []string{"timeout", "missing-receipt", "protocol-error"} {
		t.Run(outcome, func(t *testing.T) {
			...
			require.Error(t, err)
			require.Equal(t, outcome != "protocol-error", errors.Is(err, ErrDeliveryUncertain))
		})
	}
}
...
func deliverCopilotControl(t *testing.T, a *copilotAgent, sink *recordingControlSink, answer string) string {
	t.Helper()
	identifier := sink.LastPublishedControl().RequestID
...
func startCopilotForControls(t *testing.T, env ...string) (*copilotAgent, *recordingControlSink) {
	t.Helper()
	installFakeACPCLI(t, fakeACPCLISpec{
		binary: "copilot", helperRun: "TestHelperCopilotNativeConnection",
