...
func TestNativeCopilotSessionLifecycle(t *testing.T) {
	...
	provider, err := startNativeCopilot(t.Context(), Options{
		...
		Options: optionmap.Map{
			OptionIDModel: "probe-model", OptionIDEffort: "high",
			copilotOptionSessionMode: "plan", OptionIDPermissionMode: "assisted",
		},
	}, sink)
	require.NoError(t, err)
	t.Cleanup(func() { provider.Stop(); _ = provider.Wait() })
	...
	require.Equal(t, "assisted", a.SettingsSnapshot().SurfacedOptions[OptionIDPermissionMode])
	require.Equal(t, "high", a.SettingsSnapshot().SurfacedOptions[OptionIDEffort])
	...
	require.ErrorIs(t, a.SendInput("second input", nil), ErrAgentBusy)
	...
	for _, failure := range []string{"create", "subscription", "restore"} {
		t.Run(failure, func(t *testing.T) {
			...
			provider, err := startNativeCopilot(t.Context(), Options{
				...
				APITimeout: time.Second,
				Options:    optionmap.Map{copilotOptionSessionMode: "plan", OptionIDPermissionMode: "assisted"},
			}, sink)
			require.NoError(t, err)
			t.Cleanup(func() { provider.Stop(); _ = provider.Wait() })
			a := provider.(*copilotAgent)
...
func TestCopilotNativeSessionRejectsAnEmptyIdentity(t *testing.T) {
	...
	_, err := connection.openSession(Options{}, "", false, time.Second)
...
func TestCopilotUsesNativeSessionProtocol(t *testing.T) {
	...
	sink := &testSink{}
	provider, err := StartCopilotCLI(t.Context(), Options{
		...
		APITimeout: time.Second,
		Options:    map[string]string{OptionIDModel: "probe-model", OptionIDEffort: "high"},
	}, sink)
	require.NoError(t, err)
	t.Cleanup(func() { provider.Stop(); _ = provider.Wait() })
