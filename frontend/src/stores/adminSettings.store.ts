 ...
 * passkey ceremonies and which URL a worker is told to dial, the SMTP key
 * decides whether email affordances appear, and the sign-up and captcha keys
 * decide the sign-in form. An administrator who published the hub's URL
 * watched Add passkey stay disabled with no way to know why, on the very
 * screen that had just accepted the change.
 *
 ...
 * carries, including the ones nobody has added yet.
 *
 ...
 * landing before the reply is merged. A failed refresh keeps the previous
 * snapshot and the next write converges.
 *
 * `loadSystemInfo(true)` rather than `refreshSnapshot()`: this is a KNOWN
 * change, not a suspected staleness, and it happens at human speed. The
 * dedupe window that `refreshSnapshot` holds exists for failure sites that
 * fire in bursts, and it would swallow the second of two writes made inside
 * three seconds -- leaving exactly the stale flag this exists to remove.
 */
