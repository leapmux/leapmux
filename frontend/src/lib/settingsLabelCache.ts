...
const [settingsLabelRevision, setSettingsLabelRevision] = createSignal(0)
 ...
 */
function setWithCap<V>(map: Map<string, V>, key: string, value: V, max: number): boolean {
  const changed = !Object.is(map.get(key), value)
  let evicted = false
  map.delete(key)
  map.set(key, value)
  while (map.size > max) {
