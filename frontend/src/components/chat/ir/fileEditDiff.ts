 ...
 */
export type FileEditDiff = FileEditFacts & FileEditContent

export interface FileEditFacts {
  filePath: string
  previousPath?: string
 ...
 *
 * The key is the SOURCE, which the IR rebuilds whenever the row's revision changes, so
 ...
 * a unified diff at all: `ir/applyPatch.ts` reads it, for Copilot and ZCode both.
 */
