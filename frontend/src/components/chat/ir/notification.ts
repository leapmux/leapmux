 ...
 * cannot reach the screen with no glyph. The IR names the outcome rather than
...
export interface SettingChangeIR {
  label: string
...
export interface CompactionBoundaryMeta {
 ...
 */
export type NotificationEntryIR
   ...
   */
    ...
    | { kind: 'settings-changed', changes: SettingChangeIR[] }
    /**
     ...
     */
    | { kind: 'compaction', phase: CompactionPhase, micro?: boolean, detail?: CompactionBoundaryMeta, error?: string }
...
export interface NotificationThreadIR {
  entries: NotificationEntryIR[]
}
 ...
 */
export function compactionMetaFromRecord(meta: Record<string, unknown> | undefined): CompactionBoundaryMeta {
...
export function compactionMetaFromBoundary(m: Record<string, unknown>): CompactionBoundaryMeta {
