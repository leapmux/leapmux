...
import type { NoneRequest, NoneResult } from './none'
...
/** The REQUEST half of every kind's IR pair. */
...
/** The RESULT half of every kind's IR pair. */
...
// Compile-time coverage. `ToolCallIR` indexes both interfaces for every ToolKind, so a
// MISSING key already fails tsc. These reject an EXTRA key, which the index would not see.
type NoExtraKey<T> = Exclude<keyof T, ToolKind> extends never ? true : never
export const REQUESTS_COVER_TOOL_KINDS: NoExtraKey<ToolRequests> = true
export const RESULTS_COVER_TOOL_KINDS: NoExtraKey<ToolResults> = true
...
type BrandKeyOf<T> = Extract<KeysOf<T>, 'unparsed' | 'failure'>
export const NO_PAYLOAD_RESERVES_A_BRAND: { [K in ToolKind]: BrandKeyOf<ToolResults[K]> }[ToolKind] extends never ? true : never = true

// `DeclinedCall` narrows its typed half with `Extract<ToolResults[K], ProseResult>`,
...
type KindWhoseResultIsProse = { [K in ToolKind]: ToolResults[K] extends ProseResult ? K : never }[ToolKind]
export const PROSE_RESULT_KINDS_MATCH_THE_TYPES: ProseResultKind extends KindWhoseResultIsProse ? true : never = true
export const EVERY_PROSE_RESULT_KIND_IS_LISTED: KindWhoseResultIsProse extends ProseResultKind ? true : never = true
