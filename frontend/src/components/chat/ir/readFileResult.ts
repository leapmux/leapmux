 ...
 *
 * The IR's own vocabulary, not the `Alert` component's `variant`. The two
 * happen to spell the same four words today, and the renderer still maps one
 * onto the other: the IR must not import a component's prop type, or a change
 ...
 */
export function readFileLines(source: ReadFileResult): ParsedCatLine[] | null {
