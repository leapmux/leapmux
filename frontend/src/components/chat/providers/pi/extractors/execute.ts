...
function parsePiCommandMarker(text: string, isError: boolean): PiCommandMarker {
  ...
  if (!m)
    return { output: text, exitCode: null, cancelled: false }
  // The pattern's first group is the whole marker, which a match always carries.
  const head = m[1] ?? ''
  return {
...
export function extractPiCommand(payload: Record<string, unknown> | null | undefined): PiCommand | null {
  ...
  return {
    ...
    fullOutputPath: typeof details.fullOutputPath === 'string' ? details.fullOutputPath : null,
    isError,
  }
}

/**
 * Adapt a PiCommand to the shared CommandResult shape.
 *
 * The outcome words travel on the row's STATUS rather than in this shape, and the
 * row states them from {@link PiCommand.cancelled}: Pi reports a stop and a timeout
 * as errors, so the envelope's own status words both as `failed`. The `execute` entry
 * of `PI_TOOL_READERS` reads that field and sets `statusOverride`. The exit code rides
 * here, and the shared label reads it.
 */
export function piCommandResult(command: PiCommand): CommandResult {
  return {
    output: command.output,
    // Null states an unknown code, which `CommandExit` declares; undefined would state none.
