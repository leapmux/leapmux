...
import { piSubagentNotifications, piVisibleCustomMessage } from './customMessage'
...
describe('pi custom messages', () => {
  ...
  it('does not render hidden or non-custom messages', () => {
    ...
    expect(piSubagentNotifications({ ...payload, message: { ...payload.message, role: 'assistant' } })).toBeNull()
    expect(piSubagentNotifications({ ...payload, type: 'message_start' })).toBeNull()
  })

  it('leaves malformed notification details to the custom text renderer', () => {
    expect(piSubagentNotifications(notification('Preserve this text', { id: null }))).toBeNull()
    expect(piSubagentNotifications(notification('Preserve this text', { status: '' }))).toBeNull()
  })
})
