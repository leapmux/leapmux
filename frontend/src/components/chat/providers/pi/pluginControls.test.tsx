...
describe('pi editor control requests', () => {
  it('disables the native editor while a response is pending', () => {
    ...
    const editor = getByTestId('dialog-editor')
  ...
  it.each([false, true])('places the editor in the request body and respects disabled state: %s', (disabled) => {
    ...
    const editor = getByTestId('dialog-editor') as HTMLTextAreaElement
  ...
  it.each(['  first\n  second\n', ''])('retains the edited value across remount: %j', (value) => {
    ...
    fireEvent.input(first.getByTestId('dialog-editor'), { target: { value } })
    first.unmount()
    const second = renderEditor()
    expect((second.getByTestId('dialog-editor') as HTMLTextAreaElement).value).toBe(value)
  })
  ...
  it('renders a multi-line textarea pre-populated with prefill', () => {
    ...
    const textarea = getByTestId('dialog-editor') as HTMLTextAreaElement
...
// Pi may add a dialog method that LeapMux does not know. It reads as a confirm, so
// the reader answers it with Approve or Deny, and the agent never waits on a dialog
// that has no way out.
describe('pi dialog of an unknown method', () => {
  function makeUnknownRequest(): ControlRequest {
    return {
      requestId: 'req-u',
      agentId: 'agent-1',
      payload: { type: 'extension_ui_request', id: 'req-u', method: 'something_pi_added_later', title: 'Heads up' },
    }
  }

  it.each([
    ['control-allow-btn', 'Approve', true],
    ['control-deny-btn', 'Deny', false],
  ] as const)('answers %s (%s) with confirmed: %s', async (testId, label, confirmed) => {
    const onRespond = vi.fn().mockResolvedValue(undefined)
    const { getByTestId, queryByTestId } = render(() => (
      <ControlRequestActions
        agentProvider={AgentProvider.PI}
        request={makeUnknownRequest()}
        answerState={makeAskState()}
        onRespond={onRespond}
        hasEditorContent={false}
        onTriggerSend={vi.fn()}
    ...
    await vi.waitFor(() => expect(onRespond).toHaveBeenCalledOnce())
    const [bytes] = onRespond.mock.calls[0] ?? []
    expect(JSON.parse(new TextDecoder().decode(bytes as Uint8Array))).toEqual({ type: 'extension_ui_response', id: 'req-u', confirmed })
  })
})

describe('pi dialog content', () => {
  it('states the deadline of a dialog that Pi resolves by itself', () => {
    const request: ControlRequest = {
      requestId: 'req-t',
      agentId: 'agent-1',
      payload: { type: 'extension_ui_request', id: 'req-t', method: 'confirm', title: 'Continue?', timeout: 30000 },
    }
    const { getByText } = render(() => (
      <ControlRequestContent agentProvider={AgentProvider.PI} request={request} answerState={makeAskState()} />
    ))
    expect(getByText('Auto-resolves in 30s if no response.')).toBeVisible()
  })

  it('states the default title of a dialog that states none', () => {
    const request: ControlRequest = {
      requestId: 'req-n',
      agentId: 'agent-1',
      payload: { type: 'extension_ui_request', id: 'req-n', method: 'confirm' },
    }
    const { getByText } = render(() => (
      <ControlRequestContent agentProvider={AgentProvider.PI} request={request} answerState={makeAskState()} />
    ))
    expect(getByText('Approval Required')).toBeVisible()
  })
})
