...
describe('pi editor control requests', () => {
  it('disables the native editor while a response is pending', () => {
    const answerState = makeAskState()
    const { getByTestId } = render(() => (
      <ControlRequestContent request={makeEditorRequest('draft')} answerState={answerState} agentProvider={AgentProvider.PI} />
    ))
  ...
  it.each(['  first\n  second\n', ''])('retains the edited value across remount: %j', (value) => {
    ...
    const renderEditor = () => render(() => (
      <PiControlContent request={makeEditorRequest('initial')} answerState={answerState} />
    ))
  ...
  it('send ships the textarea contents as a value response', async () => {
    ...
    const { getByTestId } = render(() => (
      <>
        <PiControlContent request={makeEditorRequest()} answerState={answerState} />
        <PiControlActions
