...
import { CONTROL_ALLOW_CHOICE_ID, createControlChoice, sendResponse } from '../../controls/types'
import {
  ALLOW_AS_LABEL,
      ...
      fallback={(
        <PermissionRequestContent
          ...
          source={{
            ...
            input: method() === 'item/permissions/requestApproval' ? permissions() : undefined,
          }}
        />
      )}
    >
      <Match when={toolName() === 'CodexPlanModePrompt'}>
        <PlanApprovalContent request={props.request} />
      </Match>
    </Switch>
  )
}
...
const CodexPlanModePromptActions: Component<ActionsProps> = (props) => {
  ...
  const handleApprove = () => sendResponse(
    ...
    { planApproval: { permissionMode: planApproval.permissionMode() ?? '', clearContext: planApproval.clearContext() } },
  )
  ...
  return (
    <ControlDecisionFooter
      ...
      negativeAction={{
        label: 'Reject',
        testId: 'control-deny-btn',
        onSelect: () => sendResponse(props.onRespond, buildDenyResponse(props.request.requestId, '')),
      }}
