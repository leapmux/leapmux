...
const COPILOT_EXTRA_NOTIF_TYPES = new Set(['agent_error'])

function isCopilotNotifThread(wrapper: { messages: unknown[] } | null): boolean {
  return isNotificationThreadWrapper(wrapper, COPILOT_EXTRA_NOTIF_TYPES, (t, st) =>
    t === 'system' && st !== 'init' && st !== 'task_notification')
}

function CopilotSettingsPanel(props: ProviderSettingsPanelProps): JSX.Element {
  const menuId = createUniqueId()
  const currentModel = () => props.model || defaultModelId(props.availableModels) || DEFAULT_COPILOT_MODEL
  const currentMode = () => props.permissionMode || COPILOT_MODE_AGENT
...
function CopilotTriggerLabel(props: ProviderSettingsPanelProps): JSX.Element {
  const currentModel = () => props.model || defaultModelId(props.availableModels) || DEFAULT_COPILOT_MODEL
  const currentMode = () => props.permissionMode || COPILOT_MODE_AGENT
  return (
    <>
...
function classifyCopilotMessage(
  ...
  if (wrapper) {
    if (isCopilotNotifThread(wrapper))
  ...
  if (sessionUpdate === 'usage_update' || sessionUpdate === 'available_commands_update'
    || sessionUpdate === 'user_message_chunk' || sessionUpdate === 'config_option_update') {
    return { kind: 'hidden' }
  }
...
const copilotPlugin: ProviderPlugin = {
  ...
  SettingsPanel: CopilotSettingsPanel,
  settingsTriggerLabel: CopilotTriggerLabel,
}

registerProvider(AgentProvider.COPILOT_CLI, copilotPlugin)
