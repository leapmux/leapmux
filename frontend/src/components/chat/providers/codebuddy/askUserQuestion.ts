...
/**
 * CodeBuddy's question tool name.
 *
 ...
 * input with an `answers` record added, keyed by the question text.
 */
const CODEBUDDY_ASK_USER_QUESTION = 'AskUserQuestion'

/**
 * Whether one control payload is CodeBuddy's question prompt.
 *
 ...
 */
export function codebuddyIsAskUserQuestion(payload: Record<string, unknown>): boolean {
  return getToolName(payload) === CODEBUDDY_ASK_USER_QUESTION
}
 ...
 * `<For>`.
 */
export function codebuddyAskUserQuestions(payload: Record<string, unknown>): ControlQuestion[] {
