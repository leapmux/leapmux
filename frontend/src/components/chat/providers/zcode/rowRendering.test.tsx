...
describe('zcode tool execution renderer', () => {
  it('shows the grep path below the pattern without repeating the arguments as JSON', () => {
    const { container } = render(() => <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Grep, { pattern: 'answer', path: '/project/src' })} context={{ workingDir: '/project' }} />)
  ...
  it('shows a glob header without a repeated JSON body', () => {
    const { container } = render(() => <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Glob, { pattern: '*.ts', path: '/project/src' })} context={{ workingDir: '/project' }} />)
  ...
  it('renders a Bash command in the title', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Bash, { command: 'ls -la /tmp' })} />
    ))
  ...
  it('renders a Read with the file path and, when present, the requested range', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Read, {
        file_path: '/tmp/a.ts',
        offset: 10,
        limit: 5,
      })}
  ...
  it('renders a Write and an Edit with the file path', () => {
    const { container: write } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Write, { file_path: '/tmp/new.ts' })} />
    ))
    expect(write.textContent).toContain('/tmp/new.ts')

    const { container: edit } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Edit, { file_path: '/tmp/a.ts' })} />
    ))
  ...
  it('renders a subagent spawn with its description and prompt', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Agent, {
  ...
  it('renders a Grep with its pattern', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.Grep, { pattern: 'TODO' })} />
    ))
  ...
  it('renders an unknown tool by its name', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled('SomeToolAddedLater', { arg: 1 })} />
    ))
...
describe('zcode TodoWrite renderer', () => {
  it('draws the list as a checklist rather than as raw JSON', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.TodoWrite, {
        todos: [
  ...
  it('shows the cleared state for an emptied list', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.TodoWrite, { todos: [] })} />
    ))
  ...
  it('falls back to the generic row when the input carries no todos', () => {
    const { container } = render(() => (
      <ZCodeRowView parsed={scheduled(ZCODE_TOOL.TodoWrite, { note: 'nothing' })} />
    ))
