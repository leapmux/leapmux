...
import { renderThreadHasIcon, renderThreadText } from '~/test-support/messageRenderProbes'
import { describeZCodeNotification, zcodeNotificationEntry } from './notification'
...
describe('zcodeNotificationEntry', () => {
  it('maps a described row to a single text entry', () => {
    expect(zcodeNotificationEntry(event(ZCODE_EVENT.TurnSteerDrained)))
      .toEqual([{ kind: 'text', text: 'Queued input delivered to the agent' }])
  })

  // A frame ZCode cannot read produces NO entry rather than a null that asks a
  // shared switch to try next: that switch is gone, and a notification which draws
  // nothing is the safe answer -- the row already reached the transcript.
  it('states nothing exactly when the describer states nothing', () => {
    expect(zcodeNotificationEntry(event(ZCODE_EVENT.PermissionResolved))).toEqual([])
    expect(zcodeNotificationEntry(event('settings_changed'))).toEqual([])
  })
})
...
/**
 * The decision is an app-server WORD, so a plain object answers `Object.prototype`.
 *
 * The table holds functions, so that answer was then CALLED: `toString` drew
 * `[object Undefined]` as the whole sentence, and `__proto__` threw and took the
 * message into the ErrorBoundary.
 */
describe('a decision word that names an Object.prototype member', () => {
  it.each(['toString', 'constructor', 'valueOf', 'hasOwnProperty', '__proto__'])('reports %s as unreadable', (decision) => {
    expect(describeZCodeNotification(event(ZCODE_EVENT.PermissionResolved, {
      decision,
      toolName: ZCODE_TOOL.Read,
    }))).toBeNull()
