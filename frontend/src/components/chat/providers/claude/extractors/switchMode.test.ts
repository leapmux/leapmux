...
import { claudeSwitchModePayload } from './switchMode'
...
describe('claudeSwitchModePayload', () => {
  ...
  it('words the refusal of a plan on the request', () => {
    const payload = claudeSwitchModePayload({}, row(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE), refused(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE))
  ...
  it('keeps every other field the request already stated', () => {
    ...
    const payload = claudeSwitchModePayload(request, row(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE), refused(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE))
  ...
  it('words no refusal for another mode switch', () => {
    const payload = claudeSwitchModePayload({}, row(CLAUDE_TOOL_NAMES.ENTER_PLAN_MODE), refused(CLAUDE_TOOL_NAMES.ENTER_PLAN_MODE))
  ...
  it('words no refusal for a plan the reader approved', () => {
    const payload = claudeSwitchModePayload({}, row(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE), approved(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE, { filePath: '/repo/.plans/one.md' }))
    expect(payload.request.declinedTitle).toBeUndefined()
  ...
  it('words no refusal while the call runs', () => {
    const payload = claudeSwitchModePayload({}, row(CLAUDE_TOOL_NAMES.EXIT_PLAN_MODE), undefined)
    expect(payload.request.declinedTitle).toBeUndefined()
