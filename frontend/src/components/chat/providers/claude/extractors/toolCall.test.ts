...
import { CLAUDE_TOOL_READERS, claudePayload } from './toolCall'
...
describe('CLAUDE_TOOL_READERS', () => {
  ...
  // EMPTY kind -- a `search` row with none of the fields the kind declares.
  it('answers each kind at the key that states it', () => {
    for (const kind of TOOL_KINDS)
      expect(CLAUDE_TOOL_READERS[kind](SMOKE_FACTS).kind, kind).toBe(kind)
  })

  // The label carries the kind, because the empty kind titles a case with nothing at
  // all and a reader cannot tell which row of the report it is.
  it.each(SHARED_REQUEST_KINDS.map(kind => [kind || 'the empty kind', kind] as const))(
...
describe('claudePayload', () => {
  // An MCP wire name carries its server and its tool inside itself, and the name table
  // holds no entry for one -- so the name table alone answers the empty kind.
  it('routes an MCP wire name to the mcp kind', () => {
    const payload = claudePayload(requestRow('mcp__docs__lookup', { query: 'renderer' }), undefined, {})
  ...
  it('takes the empty kind for a tool no vocabulary lists', () => {
    const payload = claudePayload(requestRow('ToolNobodyNames', { anything: 1 }), undefined, {})
    expect(payload.kind).toBe('')
    expect(payload.kind === '' && payload.request).toStrictEqual({ args: { anything: 1 } })
  })
  ...
  it('states no title for a TodoWrite row', () => {
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.TODO_WRITE, { todos: [] }), undefined, {})
    expect(payload.kind).toBe('todo')
 ...
 * EMPTY kind instead, with none of the fields the kind declares, and the hiding two
...
describe('a ToolSearch call', () => {
  it('reads the query into the declared search request', () => {
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.TOOL_SEARCH, { query: 'select:Read,Glob' }), undefined, {})
  ...
  it('leaves the matched tool names unread rather than drawing them as files', () => {
    ...
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.TOOL_SEARCH, { query: 'select:Read,Glob' }), answered, {})
    expect(payload.kind).toBe('search')
  ...
  it('states the reason alone for a probe the tool failed', () => {
    ...
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.TOOL_SEARCH, { query: 'select:Read' }), failed, {})
  ...
  it('answers an empty body for the tool_reference blocks a real frame carries', () => {
    ...
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.TOOL_SEARCH, { query: 'select:Read,Glob' }), answered ?? undefined, {})
...
describe('a Claude file change', () => {
  it.each([
    ...
    expect(claudePayload(requestRow(toolName, { file_path: '/project/a.ts' }), undefined, {}).kind).toBe(kind)
  })
  ...
  it('states the landed diff of an answered edit', () => {
    ...
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.EDIT, { file_path: '/project/a.ts', old_string: 'before', new_string: 'after' }), answered, {})
  ...
  it('keeps the file a failed edit asked to change', () => {
    ...
    const payload = claudePayload(requestRow(CLAUDE_TOOL_NAMES.EDIT, { file_path: '/project/a.ts', old_string: 'before', new_string: 'after' }), failed, {})
    expect(payload.result).toStrictEqual({ failure: true, text: REASON })
 ...
 *
 * Its pictures ride INSIDE the result content, which is what `ToolCallCommon.images`
 * states for the generic trio: `claudeToolCallIR` empties the envelope's own list for
 * those kinds.
 */
describe('a generic Claude call', () => {
  it('states the words and the pictures in one content list', () => {
    ...
    const payload = claudePayload(requestRow('ToolNobodyNames'), answered, {})
  ...
  // A `FailedResult` holds text alone, so a failed call that returned PICTURES keeps
