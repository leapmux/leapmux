...
describe('claudeBashFromToolResult', () => {
  ...
  it('extracts stdout/stderr/interrupted/persisted bookkeeping', () => {
    const source = claudeBashFromToolResult({
      toolUseResult: {
        stdout: 'STDOUT',
        stderr: 'STDERR',
        interrupted: true,
        persistedOutputPath: '/tmp/out',
    ...
    expect(source.persistedOutputPath).toBe('/tmp/out')
    expect(source.persistedOutputSize).toBe(4096)
  })
