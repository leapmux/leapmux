...
import { claudeTaskPayload } from './task'
...
function outcomeOf(status: string): string | undefined {
  const payload = claudeTaskPayload(request, args, row({ task: { status, task_id: 'task-42' } }))
 ...
 */
describe('claudeTaskPayload outcome', () => {
  it.each([
    ['completed', 'completed'],
    ['failed', 'failed'],
    ['error', 'failed'],
  ...
  it('states no result for a call that has not answered', () => {
    const payload = claudeTaskPayload(request, args, undefined)
 ...
 */
describe('claudeTaskPayload failure rung', () => {
  ...
  it('states the reason alone for a TaskOutput the tool failed', () => {
    const payload = claudeTaskPayload(request, args, failedRow(undefined))
  ...
  it('states the reason alone for a TaskStop the tool failed', () => {
    ...
    const payload = claudeTaskPayload(request, stopArgs, failedRow({}))
    expect(payload.result).toStrictEqual({ failure: true, text: 'No such task' })
  })
  ...
  it('states the reason alone for a failed stop that carries a payload', () => {
    const stopArgs = { ...args, toolName: 'TaskStop' } as ClaudeToolRow
    const payload = claudeTaskPayload(request, stopArgs, failedRow({ message: 'Task already ended' }))
    expect(payload.result).toStrictEqual({ failure: true, text: 'No such task' })
  })
  ...
  it('keeps the stop card of a call the tool did not fail', () => {
    const stopArgs = { ...args, toolName: 'TaskStop' } as ClaudeToolRow
    const payload = claudeTaskPayload(request, stopArgs, row({ message: 'Task stopped' }))
