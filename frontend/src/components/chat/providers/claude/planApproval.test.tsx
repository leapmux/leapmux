...
describe('claude plan approval content', () => {
  it('does not request another plan decision after the response', () => {
    const request = { ...makeRequest(), responseState: ControlResponseState.DELIVERED }
    render(() => <ClaudePlanApprovalContent request={request} />)
    expect(screen.getByText('Plan approval', { exact: true })).toBeVisible()
    expect(screen.queryByText('Plan Ready for Review', { exact: true })).toBeNull()
    expect(screen.queryByText('The agent finished planning and is ready to proceed.')).toBeNull()
    expect(screen.queryByText('The agent requests approval to proceed.')).toBeNull()
  })
  ...
  it('shows fallback message when no permissions are requested', () => {
    ...
    expect(screen.getByText('The agent requests approval to proceed.')).toBeInTheDocument()
  })
