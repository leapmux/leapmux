...
describe('claude Task rows before their result lands', () => {
  ...
  it('draws no row for an unresolved TaskGet', () => {
    const row = providerRowIr(AgentProvider.CLAUDE_CODE, useRow(CLAUDE_TOOL_NAMES.TASK_GET, { task_id: 7 }), { role: 'opener' })
  ...
  it('draws the task once the result lands beside it', () => {
    ...
    const row = providerRowIr(AgentProvider.CLAUDE_CODE, payload, { role: 'opener', result: input(result) })
  ...
  it.each([CLAUDE_TOOL_NAMES.TASK_CREATE, CLAUDE_TOOL_NAMES.TASK_UPDATE])('still draws an unresolved %s', (name) => {
    const row = providerRowIr(AgentProvider.CLAUDE_CODE, useRow(name, { subject: 'Inspect sample' }), { role: 'opener' })
    expect(row?.kind).toBe('tool')
  })
})
...
describe('claude Task rows and a sibling from another call', () => {
  it('ignores a result whose tool-use id names another call', () => {
    ...
    const row = providerRowIr(AgentProvider.CLAUDE_CODE, payload, { role: 'opener', result: input(stranger) })
    expect(row).toEqual({ kind: 'hidden' })
  })
})
...
describe('claude ExitPlanMode rows', () => {
  ...
  it.each([
    ...
    const row = providerRowIr(AgentProvider.CLAUDE_CODE, useRow(name, { name: 'feature-a' }), { role: 'opener' })
    const call = row?.kind === 'tool' ? row.call : null
