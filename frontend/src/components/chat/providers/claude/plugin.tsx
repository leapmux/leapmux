...
import { CONTROL_DECISION_WORDS, controlBehaviorDisplay } from '../../persistedControlResponse'
...
const claudeCodePlugin: Provider = {
  ...
  // permission -- so the request decides which pair the saved row shows.
  controlResponseDisplay: withElicitationResponse(claudeElicitation, cr => controlBehaviorDisplay(
    cr.response,
    getToolName(cr.request ?? {}) === CLAUDE_TOOL.EXIT_PLAN_MODE
      ? CONTROL_DECISION_WORDS.plan
      : CONTROL_DECISION_WORDS.permission,
  )),
