...
export function cursorStoredToolPresentation(tool: Record<string, unknown>, model: ToolPresentation, supplemental: Record<string, unknown> | undefined): ToolPresentation | null {
  ...
  if (name === 'Read' && typeof success.content === 'string') {
    ...
    // body then states the file is empty -- of a file it never read (RL-037).
    const refused = !success.content && !!output
    return {
      ...restored,
      kind: 'read',
      input: { ...restored.input, path },
      output: refused ? output : success.content,
      body: { type: 'read', source: readFileSourceFromContent({
        filePath: path,
        content: success.content,
        startLine,
        totalLines: pickNumber(success, 'totalLines', undefined),
        ...
        fallbackContent: refused ? output : undefined,
      }) },
    }
  }
