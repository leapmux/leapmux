...
function cursorAnswerLines(
  ...
  for (const answer of answers) {
    ...
    // Cursor's answer carries the reader's own words beside the option ids, and they read
    // back as one more answer to the same question. A question answered with typed text
    // ALONE is still an answer, so this is what keeps it from being dropped below.
    const typed = pickString(answer, 'freeformText', '').trim()
    if (typed)
      mapped.push(typed)
    if (mapped.length > 0)
