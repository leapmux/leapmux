...
describe('cursorControlResponseDisplay', () => {
  describe('ask_question', () => {
    ...
    // Cursor's answer carries `freeformText` beside the option ids, so a saved decision
    // has to read it back. A question answered with typed text alone is still an answer.
    // See RL-006.
    it('reads a typed answer back beside the options it accompanied', () => {
      const response = questionOutcome({
        outcome: 'answered',
    ...
    it('reads a typed answer back when no option was selected', () => {
      const response = questionOutcome({
        outcome: 'answered',
