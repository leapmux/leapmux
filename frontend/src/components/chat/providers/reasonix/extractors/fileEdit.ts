...
function spanText(lines: string[], requested: string | undefined, fuzzy: boolean, mayBeEmpty: boolean): string | undefined {
  ...
  if (requested !== undefined && !fuzzy) {
    ...
    // A split answers its row count, so the undefined checks are the type-level guard alone.
    const [head, tail] = fragments
    if (fragments.length === 2 && head !== undefined && tail !== undefined
      && encoded.startsWith(head) && encoded.endsWith(tail)
      && encoded.length >= head.length + tail.length) {
      return normalized
    }
  }
...
export function reasonixEditReceipt(output: string, filePath: string, input: Record<string, unknown> = {}): FileEditDiff[] | null {
  ...
  // `parseReceipts` answers a list only when it holds one; the fallback is the type-level guard alone.
  const total = receipts[0]?.total ?? 0
  ...
  for (let index = 1; index <= total; index++) {
    ...
    sources.push({ filePath, structuredPatch: null, oldStr, newStr, showLineNumbers: false, ...(notes.length ? { notice: notes.join('; ') } : {}) })
  }
  return sources
}
