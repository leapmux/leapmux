...
function nativeGuidance(text: string, reference?: string): { forkedFrom?: string } | null {
  ...
  const forkedFrom = fork[1]
  return value.slice(firstBreak + 1) === expected ? { ...(forkedFrom !== undefined ? { forkedFrom } : {}) } : null
}
...
export function reasonixAgentResult({ toolName, input, output: fullOutput, originalOutput, status }: ReasonixAgentOutput): AgentRun {
  ...
  return {
    ...
    ...(outcome === 'unknown' ? { statusLabel: nativeStatus === 'partial' ? 'partial' : 'returned a result' } : {}),
    outcome,
    metadata,
