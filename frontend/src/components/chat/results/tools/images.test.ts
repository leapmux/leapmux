...
describe('imagesForIR over merged calls', () => {
  it('numbers a read call\'s own pictures from zero', () => {
    const call = toolCallIr('read', { images: [picture('a'), picture('b')] })
    expect(imagesForIR(toolRow(call)).map(p => p.data)).toEqual(['a', 'b'])
  })

  it('numbers a generic result\'s inline pictures from zero', () => {
    const call = toolCallIr('mcp', { result: { content: [
      { type: 'text', text: 'before' },
      { type: 'image', source: picture('one') },
      { type: 'image', source: picture('two') },
    ] } })
    expect(imagesForIR(toolRow(call)).map(p => p.data)).toEqual(['one', 'two'])
  })
  ...
  it('lists nothing on a request row whose result row is beside it', () => {
    const call = toolCallIr('read', { images: [picture('a')] })
    const row = toolRow(call, 'request', { result: true })
    expect(imagesForIR(row)).toEqual([])
  })
  ...
  it('lists the pictures of a request row that stands alone', () => {
    ...
    expect(imagesForIR(row).map(p => p.data)).toEqual(['a'])
  })
})
