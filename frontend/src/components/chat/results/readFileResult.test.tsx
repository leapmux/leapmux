...
import { ReadFileResultBody, readFileSourceFromContent } from './readFileResult'
...
describe('readFileResultBody with no content', () => {
  ...
  it('states that the result carried nothing, not that the file is empty', () => {
    const { container } = render(() => (
      <ReadFileResultBody source={readFileSourceFromContent({ filePath: 'bigfile.txt', content: '' })} />
    ))
  ...
  it('prefers whatever the result did carry', () => {
    const source = {
      ...readFileSourceFromContent({ filePath: 'bigfile.txt', content: '' }),
  ...
  it('renders real content rather than the notice', () => {
    const { container } = render(() => (
      <ReadFileResultBody source={readFileSourceFromContent({ filePath: 'a.txt', content: 'alpha' })} />
    ))
