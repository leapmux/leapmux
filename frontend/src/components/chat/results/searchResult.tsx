...
export function SearchResultBody(props: {
  ...
  const contentLines = createMemo(() => content().split('\n'))
  const summary = createMemo(() => summaryFor(props.source))
  const isCollapsed = () => !expanded()
    && (filenames().length > COLLAPSED_RESULT_ROWS
      || contentLines().length > COLLAPSED_RESULT_ROWS)

  const displayFilenames = createMemo(() => {
    if (expanded() || filenames().length <= COLLAPSED_RESULT_ROWS)
      return filenames()
    return filenames().slice(0, COLLAPSED_RESULT_ROWS)
  })
  const displayContent = createMemo(() => {
    if (expanded() || !content())
      return content()
    if (contentLines().length <= COLLAPSED_RESULT_ROWS)
      return content()
    return contentLines().slice(0, COLLAPSED_RESULT_ROWS).join('\n')
  })
