...
function renderCard(initial?: ToolProgressEntry) {
  ...
  const context: ToolLayoutContext = { textSelectionActive: selecting, toolProgress: { liveTail: progress } }
  const result = render(() => (
    <ToolUseLayout
      icon={SquareTerminal}
      toolName="Bash"
...
describe('ToolUseLayout running-tool badge', () => {
  it('renders the elapsed time in the card header', () => {
    const { container } = renderCard({ elapsedSeconds: 90 })
   ...
   * selection the user holds across the title or the summary would collapse.
   *
  ...
  it('replaces no node outside the badge when the progress updates', () => {
    const { container, getByText, getByTestId, setProgress } = renderCard({ elapsedSeconds: 30 })
  ...
  it('writes nothing at all while a text selection is live', () => {
    const { container, setProgress, setSelecting } = renderCard({ elapsedSeconds: 30 })
    setSelecting(true)
    setProgress({ elapsedSeconds: 60 })
    expect(container.textContent).toContain('30s')
  ...
  it('holds every node of the card and honours the selection over a live store', () => {
    ...
    const [selecting, setSelecting] = createSignal(false)
    const context: ToolLayoutContext = { textSelectionActive: selecting, toolProgress: { liveTail: () => store.get('a1', SPAN) } }
