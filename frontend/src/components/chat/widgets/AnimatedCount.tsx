...
//
// The roll is forward-only because both supported counters increase within a
// display lifecycle. If a value does
...
const DigitColumn: Component<{ digit: number, enter: boolean, paused: boolean }> = (props) => {
  ...
  const placeInstant = (offsetCells: number) => {
    ...
    stripEl.style.transition = props.paused || prefersReducedMotion() ? 'none' : ROLL_TRANSITION
  }
  ...
  createEffect(() => {
    ...
    if (props.paused || prefersReducedMotion()) {
...
const RollingNumber: Component<{ value: string, animateEntry: boolean, paused: boolean }> = (props) => {
  ...
  return (
    <Index each={chars()}>
      {ch => (
        ...
        >
          <DigitColumn digit={Number(ch())} enter={props.animateEntry} paused={props.paused} />
        </Show>
      )}
    </Index>
  )
...
/**
 * An animated count whose digits roll while its number format stays stable.
 * The component crossfades when the format family changes.
 *
 ...
 */
export const AnimatedCount: Component<{
  display: string
  unit: string
  family?: string
  rootClass?: string
  ...
  const familyKey = createMemo(() => [formatted().family])
  ...
  // updates.
  const exiting = useFadingSnapshots(formatted, {
    shouldSnapshot: (prev, next) => prev.family !== next.family,
        ...
        <For each={familyKey()}>
          {() => (
            <span class={styles.liveLayer} aria-hidden="true">
              <RollingNumber value={display()} animateEntry={true} paused={props.paused === true} />
            </span>
          )}
        </For>
        <For each={exiting}>
          {snap => (
            <span class={styles.exitingLayer} data-testid="odo-exiting" aria-hidden="true">
              <RollingNumber value={snap.value.display} animateEntry={false} paused={props.paused === true} />
            </span>
          )}
        </For>
      </span>
