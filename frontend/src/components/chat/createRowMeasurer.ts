...
export interface RowMeasurerDeps {
   ...
   * a row that grows post-mount (async syntax highlighting, expand/collapse) updates
   * the offset map and re-pins scrollTop in the SAME frame it grew. Deferring to rAF
   * instead paints one frame with the row at its new size but siblings (and scrollTop)
   * at stale offsets -- a visible vertical wiggle while scrolling. Injected for tests.
   */
...
export function createRowMeasurer(deps: RowMeasurerDeps): RowMeasurer {
  ...
  const flush = () => {
    ...
    for (const el of pending) {
      ...
      if (id === undefined || !el.isConnected)
        continue
      reads.push([id, (el as HTMLElement).getBoundingClientRect().height])
    }
    pending.clear()
  ...
  const attachRow = (id: string, el: HTMLElement) => {
    ...
    deps.measure(id, el.getBoundingClientRect().height)
    ro?.observe(el)
  }
  ...
  const detachRow = (el: HTMLElement) => {
    ...
    // re-estimate + visible re-pin). The newer element's later detach clears it.
  ...
  return { attachRow, detachRow, flushNow: flush, dispose }
}
