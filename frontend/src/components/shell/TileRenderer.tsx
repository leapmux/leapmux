...
export function createTileRenderer(opts: TileRendererOpts) {
  ...
  const contexts = createMemo(mapArray(contextKeys, (key) => {
    ...
    const resolver = createMessageContextResolver({
      ...
      spanMessage: (identity, side) => chatStore.getSpanMessage(agentId, identity, side),
      messageBySeq: seq => chatStore.getLoadedMessageBySeq(agentId, seq),
      fetchSpan: async (identity, signal) => {
        if (!workerId)
          throw new Error('The worker is unavailable')
        return (await getAgentSpanMessages(workerId, { agentId, ...identity }, { signal })).messages
      },
