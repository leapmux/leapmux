...
export function openSubagentTab(
  ...
  if (spec && spec.tombstoneAt != null) {
    ...
    if (!tileId || !layoutStore.hasProjectedTile(tileId))
      return 'no-tile'
    // Same worker resolution as the open branch: a registry row that carries no
    // parentAgentId (Claude/ACP never set it -- the owner IS the parent) or
    // whose parent tab is unplaced falls back to the active tab in the target
    // tile. Without this fallback a closed-then-reopened child tab is unopenable
    // for the providers that never set parentAgentId, while the same row opens
    // fine the first time.
