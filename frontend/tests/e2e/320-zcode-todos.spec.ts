...
const ZCODE = AgentProvider.ZCODE

zcodeTest.describe('tracks the ZCode to-do list', () => {
  zcodeTest('the sidebar follows each list the agent writes, and keeps it after a reload', async ({ authenticatedZCodeWorkspace, page, modelScript }) => {
    void authenticatedZCodeWorkspace
    await modelScript.queue(
      {
        toolCalls: [updateTodosToolCall(ZCODE, 'todos-first', [
    ...
    await modelScript.queue(
      {
        toolCalls: [updateTodosToolCall(ZCODE, 'todos-second', [
          { step: 'Inspect the repository', status: 'completed' },
